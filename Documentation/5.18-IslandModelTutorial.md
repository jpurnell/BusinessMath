...
print("Ring topology result: \(result.value)")
```
...
print("Fully connected result: \(result.value)")
```
...
print("Expected return: \(String(format: "%.1f%%", portReturn * 100))")
print("Risk (volatility): \(String(format: "%.1f%%", portRisk * 100))")
print("Sharpe ratio: \(String(format: "%.3f", sharpe))")
```
...
for (i, area) in result.solution.toArray().enumerated() {
    print("  Member \(i+1): \(String(format: "%.0f", area)) mm²")
}
print("Total weight: \(String(format: "%.2f", result.value)) kg")
```
...
print("Total evaluations: \(result.totalEvaluations)")
```
...
let result = try! optimizer.minimize(highdim, from: VectorN(Array(repeating: 5.0, count: 20)))

print("High-dimensional optimization:")
print("  Best value: \(String(format: "%.6f", result.value))")
print("  Total evaluations: \(result.totalEvaluations)")
...
print("  Best island: \(result.bestIslandIndex)")
print("  Best fitness: \(String(format: "%.6f", result.bestFitness))")
...
for (i, fitness) in result.islandFitnesses.enumerated() {
    ...
    print("  Island \(i): \(String(format: "%.6f", fitness))\(marker)")
}
```
...
print("  Result: \(String(format: "%.6f", singleResult.value))")
print("  Time: \(String(format: "%.3f", singleTime))s")
print("\nIsland Model (4×50):")
print("  Result: \(String(format: "%.6f", islandResult.value))")
print("  Time: \(String(format: "%.3f", islandTime))s")
```
...
// Results will now be reproducible (even on same optimizer instance!)
// Island Model creates fresh GA instances for each run, so RNG resets
...
// Note: Unlike PSO/DE, Island Model doesn't need separate instances
// for reproducibility because it creates new islands each run
```
