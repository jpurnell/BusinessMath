...
func example1_BasicExponentialDecay() {
	...
	for (name, trueVal, recoveredVal) in params {
		...
		print("  \(name.padding(toLength: 11, withPad: " ", startingAt: 0)) | \(trueVal.number().padding(toLength: 6, withPad: " ", startingAt: 0)) | \(recoveredVal.number().padding(toLength: 9, withPad: " ", startingAt: 0)) | \(absError.number().padding(toLength: 9, withPad: " ", startingAt: 0)) | \(relError.percent().padding(toLength: 8, withPad: " ", startingAt: 0)) | \(status)")
	}
	print()
...
func example2_CustomerRetentionModel() {
	...
	print("True Model:")
	print("  Initial retention:  \(initialRetention.percent())")
	print("  Monthly churn rate: \(monthlyChurnRate.percent())")
	print("  12-month retention: \((initialRetention * exp(-monthlyChurnRate * 12)).percent())")
	print()
	...
	print("  Initial retention:  \(a.percent())")
	print("  Monthly churn rate: \(b.percent())")
	print("  12-month retention: \((a * exp(-b * 12)).percent())")
	print()
...
func example3_DataRangeImpact() {
	...
	for test in tests {
		...
		guard let (_, recoveredB, _) = ExponentialDecayModel.fit(data: data) else {
			print("  \(test.name.padding(toLength: 13, withPad: " ", startingAt: 0)) | \(test.n) | \("FAILED".padding(toLength: 19, withPad: " ", startingAt: 0)) | \("N/A".padding(toLength: 9, withPad: " ", startingAt: 0)) | \("✗ FAIL")")
			continue
		}
		
		let relError = abs(recoveredB - trueB) / trueB
		let status = relError <= 0.20 ? "✓ PASS" : "✗ FAIL"
		print("  \(test.name.padding(toLength: 13, withPad: " ", startingAt: 0)) | \(test.n) | \(recoveredB.number().padding(toLength: 19, withPad: " ", startingAt: 0)) | \(relError.percent().padding(toLength: 9, withPad: " ", startingAt: 0)) | \(status)")
	}
...
func example4_ComparisonToReciprocalModel() {
	...
	if let (aEst, bEst, _) = ExponentialDecayModel.fit(data: expData) {
		...
		print("  Parameter a: \(aError.percent()) error")
		print("  Parameter b: \(bError.percent()) error")
	} else {
		print("  Fitting failed")
	}
	print()
...
func example5_WhenExponentialFittingFails() {
	...
	if let (aTrue, bTrue, _) = ExponentialDecayModel.fit(data: noisyData) {
		...
		print("  Parameter a error: \(relErrorA.percent())")
		print("  Parameter b error: \(relErrorB.percent())")
