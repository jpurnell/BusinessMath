...
func example4_StepByStepWorkflow() {
	...
	do {
		...
		for (name, trueVal, recoveredVal) in params {
			...
			print("  \(name.padding(toLength: 9, withPad: " ", startingAt: 0)) | \(trueVal.number(5).padding(toLength: 7, withPad: " ", startingAt: 0)) | \(recoveredVal.number(5).padding(toLength: 9, withPad: " ", startingAt: 0)) | \(absError.number(3).padding(toLength: 9, withPad: " ", startingAt: 0)) | \(relError.percent(4).padding(toLength: 10, withPad: " ", startingAt: 0))  \(status)")
		}
		print()
...
func example5_SampleSizeEffect() {
	...
	for n in sampleSizes {
		do {
			...
			print("  \("n".padding(toLength: 3, withPad: " ", startingAt: 0)) | \(status.padding(toLength: 7, withPad: " ", startingAt: 0)) | \(report.relativeErrors["a"]!.percent()), \(report.relativeErrors["b"]!.percent()), \(report.relativeErrors["sigma"]!.percent())")
