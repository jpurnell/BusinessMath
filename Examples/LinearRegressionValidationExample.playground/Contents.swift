...
func example1_PerfectLinearRegression() {
	...
	for (name, trueVal, recoveredVal) in params {
		...
		print("  \(name.padding(toLength: 10, withPad: " ", startingAt: 0)) | \(trueVal.number().paddingLeft(toLength: 6)) | \(recoveredVal.number().paddingLeft(toLength: 9)) | \(absError.number().paddingLeft(toLength: 9)) | \(relError.percent().paddingLeft(toLength: 9)) | \(status)")
	}
	print()
...
func example3_ComparingModels() {
	...
	print("  Intercept: \(intError.percent()) error")
	print("  Slope:     \(slopeError.percent()) error")
	print()
...
func example4_SystematicValidation() {
	...
	for test in tests {
		...
		print("  \(test.name.padding(toLength: 16, withPad: " ", startingAt: 0)) | \(status.paddingLeft(toLength: 7)) | \(avgError.percent())")
	}
