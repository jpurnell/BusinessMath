...
func gradientDescentComparisonExample() throws {
    ...
    print("Minimizing Rosenbrock function from \(initial.components)")
    ...
    let basicResult = try basicGD.minimize(rosenbrock, from: initial)
    print("   Solution: \(basicResult.solution.components.map { String(format: "%.4f", $0) })")
    ...
    let momentumGD = MomentumGradientDescent<VectorN<Double>>(
        learningRate: 0.001,
        momentum: 0.9,
        maxIterations: 10000
    )
    let momentumResult = try momentumGD.minimize(rosenbrock, from: initial)
    print("   Solution: \(momentumResult.solution.components.map { String(format: "%.4f", $0) })")
    ...
    let adam = AdamOptimizer<VectorN<Double>>(
        learningRate: 0.01,
        maxIterations: 10000
    )
    let adamResult = try adam.minimize(rosenbrock, from: initial)
    print("   Solution: \(adamResult.solution.components.map { String(format: "%.4f", $0) })")
...
func newtonRaphsonExample() throws {
    ...
    print("Minimizing quadratic function from \(initial.components.map { Int($0) })")
    print()
    ...
    let newtonResult = try newton.minimize(quadratic, from: initial)
    print("   Solution: \(newtonResult.solution.components.map { String(format: "%.6f", $0) })")
    print("   Value: \(String(format: "%.10f", newtonResult.value))")
    ...
    let bfgsResult = try bfgs.minimize(quadratic, from: initial)
    print("   Solution: \(bfgsResult.solution.components.map { String(format: "%.6f", $0) })")
    print("   Value: \(String(format: "%.10f", bfgsResult.value))")
...
func parameterFittingExample() throws {
    ...
    // Optimize with BFGS
    let optimizer = MultivariateNewtonRaphson<VectorN<Double>>(method: .bfgs)
    let result = try optimizer.minimize(
        leastSquares,
        from: VectorN([0.0, 0.0, 0.0])  // Start from zero
    )
    ...
    print()
    print("Sum of squared errors: \(String(format: "%.4f", result.value))")
...
func multiDimensionalExample() throws {
    ...
    let result = try optimizer.minimize(sphere, from: initial)

    print("Results:")
    print("  Final value: \(String(format: "%.10f", result.value))")
    print("  Iterations: \(result.iterations)")
    print("  Converged: \(result.converged)")
    print()
    print("Solution (should be near zero):")
    for (i, value) in result.solution.components.enumerated() {
        print(String(format: "  x%d = %.6f", i+1, value))
    }
