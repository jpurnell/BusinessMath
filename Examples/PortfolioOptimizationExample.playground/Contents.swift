...
func basicPortfolioExample() throws {
	...
	print("   Expected Return: \(minVar.expectedReturn.percent())")
	print("   Risk (Std Dev): \(minVar.volatility.percent())")
	print("   Weights:")
	for (i, weight) in minVar.weights.toArray().enumerated() {
		if weight > 0.01 {
			print("\(assets[i].paddingLeft(toLength: 12)): \(weight.percent())")
		}
	}
	print()
	...
	print("   Expected Return: \(maxSharpe.expectedReturn.percent())")
	print("   Risk (Std Dev): \(maxSharpe.volatility.percent())")
	print("   Weights:")
	...
	print("   Expected Return: \(targetRet.expectedReturn.percent()) (target: 12.0%%)")
	print("   Risk (Std Dev): \(targetRet.volatility.percent())")
	print("   Weights:")
...
func efficientFrontierExample() throws {
	...
	print("\("Risk".paddingLeft(toLength: 8)) | \("Return".paddingLeft(toLength: 8)) | \("Sharpe (rf=2%)".paddingLeft(toLength: 14))")
	print(String(repeating: "-", count: 36))
	...
	for portfolio in frontier.portfolios {
		...
		print("\(risk.percent().paddingLeft(toLength: 8)) | \(ret.percent().paddingLeft(toLength: 8)) | \(sharpe.number().paddingLeft(toLength: 13))")
	}
	...
	print("  Risk: \(minRisk.volatility.percent()), Return: \(minRisk.expectedReturn.percent())")

	let maxSharpe = frontier.maximumSharpePortfolio
	print("Maximum Sharpe Portfolio:")
	print("  Risk: \(maxSharpe.volatility.percent()), Return: \(maxSharpe.expectedReturn.percent()), Sharpe: \(maxSharpe.sharpeRatio.number())")
...
func riskParityExample() throws {
	...
	for (i, name) in assets.enumerated() {
		let vol = sqrt(covariance[i][i])
		print("  \(name): \(vol.percent()) volatility")
	}
	print()
	...
	for (i, weight) in riskParity.weights.toArray().enumerated() {
		print("  \(assets[i]): \(weight.percent())")
	}

	print()
	print("Portfolio Risk: \(riskParity.volatility.percent())")
	print("Portfolio Return: \(riskParity.expectedReturn.percent())")
	print()
...
func constrainedPortfolioExample() throws {
	...
	print("   Sharpe: \(longOnly.sharpeRatio.number()), Return: \(longOnly.expectedReturn.percent()), Risk: \(longOnly.volatility.percent())")
	print("   Weights: [\(longOnly.weights.toArray().map{ "\($0.percent())" }.joined(separator: ", "))]")
	print()
	...
	print("   Sharpe: \(longShort.sharpeRatio.number()), Return: \(longShort.expectedReturn.percent()), Risk: \(longShort.volatility.percent())")
	print("   Weights: [\(longShort.weights.toArray().map{ "\($0.percent())" }.joined(separator: ", "))]")
	print()
	...
	print("   Sharpe: \(boxConstrained.sharpeRatio.number()), Return: \(boxConstrained.expectedReturn.percent()), Risk: \(boxConstrained.volatility.percent())")
	print("   Weights: [\(boxConstrained.weights.toArray().map{ "\($0.percent())" }.joined(separator: ", "))]")
	print()
...
func realWorldPortfolioExample() throws {
	...
	for (i, name) in assets.enumerated() {
		let ret = expectedReturns[i]
		let vol = sqrt(covariance[i][i])
		print("  \(name.paddingLeft(toLength: 14)): \(ret.percent()) return, \(vol.percent()) vol")
	}
	print()
	...
	print("  Expected Return: \(conservative.expectedReturn.percent())")
	print("  Risk: \(conservative.volatility.percent())")
	print("  Allocation:")
	for (i, weight) in conservative.weights.toArray().enumerated() {
		if weight > 0.01 {
			let allocation = 1_000_000 * weight
			print("    \(assets[i].paddingLeft(toLength: 14)): \(allocation.currency()) (\(weight.percent()))")
		}
	}
	print()
	...
	print("  Expected Return: \(moderate.expectedReturn.percent())")
	print("  Risk: \(moderate.volatility.percent())")
	print("  Allocation:")
	...
	print("  Expected Return: \(aggressive.expectedReturn.percent())")
	print("  Risk: \(aggressive.volatility.percent())")
	print("  Allocation:")
