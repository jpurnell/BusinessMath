...
func electronicsManufacturingExample() throws {
	...
	for product in products.sorted(by: { $0.name > $1.name }) {
		...
		print("\(product.name.padding(toLength: 16, withPad: " ", startingAt: 0)) \(quantity.number().paddingLeft(toLength: 10)) \(revenue.currency().paddingLeft(toLength: 13)) \(cost.currency().paddingLeft(toLength: 13)) \(profit.currency().paddingLeft(toLength: 14)) \(margin.percent().paddingLeft(toLength: 13))")
	}

	print(String(repeating: "-", count: 96))
	print("\("TOTAL".padding(toLength: 18, withPad: " ", startingAt: 0))            \(plan.revenue.currency()) \(plan.costs.currency().paddingLeft(toLength: 13)) \(plan.profit.currency().paddingLeft(toLength: 14)) \((plan.profit / plan.revenue).percent().paddingLeft(toLength: 13))")
	print(String(repeating: "-", count: 96))
	print()
	...
	for (resource, utilization) in plan.resourceUtilization.sorted(by: { $0.key < $1.key }) {
		...
		print("  \(resource.padding(toLength: 20, withPad: " ", startingAt: 0)) \(used.number().paddingLeft(toLength: 5)) / \(capacity.number().paddingLeft(toLength: 5))  - \(utilization.percent().paddingLeft(toLength: 10)) \(bar)")
	}
	...
	for product in products {
		...
		let marginPercent = (margin / product.pricePerUnit)
		...
		print("  \(product.name):")
		print("    Margin: \(margin.currency()) (\(marginPercent.percent()))")
...
func resourceConstrainedExample() throws {
	...
	for (resource, utilization) in sortedUtilization {
		...
		print("  \(resource.padding(toLength: 20, withPad: " ", startingAt: 0)) \(percentage.percent()) \(marker)")
	}
	print()
