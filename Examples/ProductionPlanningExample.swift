...
func electronicsManufacturingExample() throws {
    ...
    print(String(format: "%-20s %10s %12s %12s %12s %10s",
                  "Product", "Quantity", "Revenue", "Cost", "Profit", "Margin"))
    print(String(repeating: "-", count: 80))
    ...
    for product in products.sorted(by: { $0.name > $1.name }) {
        ...
        print(String(format: "%-20s %10.0f $%11.0f $%11.0f $%11.0f %9.1f%%",
                      product.name, quantity, revenue, cost, profit, margin))
    }

    print(String(repeating: "-", count: 80))
    print(String(format: "%-20s            $%11.0f $%11.0f $%11.0f %9.1f%%",
                  "TOTAL", plan.revenue, plan.costs, plan.profit,
                  (plan.profit / plan.revenue) * 100))
    print(String(repeating: "-", count: 80))
    print()
    ...
    for (resource, utilization) in plan.resourceUtilization.sorted(by: { $0.key < $1.key }) {
        ...
        print(String(format: "  %-20s %5.0f / %5.0f (%3d%%) %s",
                      resource, used, capacity, percentage, bar))
    }
    ...
    for product in products {
        ...
        print("  \(product.name):")
        print(String(format: "    Margin: $%.0f (%.1f%%)", margin, marginPercent))
        print(String(format: "    Margin per assembly hour: $%.2f", marginPerHour))
    }
...
func objectiveComparisonExample() throws {
    ...
    print(String(format: "%-20s %15s %15s %15s %15s",
                  "Objective", "Product A Qty", "Product B Qty", "Total Revenue", "Total Profit"))
    print(String(repeating: "-", count: 80))
    ...
    for objective in objectives {
        ...
        print(String(format: "%-20s %15.0f %15.0f $%14.0f $%14.0f",
                      objectiveName, qtyA, qtyB, plan.revenue, plan.profit))
    }
...
func resourceConstrainedExample() throws {
    ...
    for (resource, utilization) in sortedUtilization {
        ...
        print(String(format: "  %-20s %3d%%%s", resource, percentage, marker))
    }
    print()
