...
func example1_BasicFinancialModel() {
    ...
    print("Revenue: $\(formatCurrency(revenue))")
    print("Costs: $\(formatCurrency(costs))")
    print("Profit: $\(formatCurrency(profit))")
    print()
}
...
func example6_InvestmentAnalysis() {
    ...
    print("Initial Cost: $\(formatCurrency(investment.initialCost))")
    print("Discount Rate: \(investment.discountRate * 100)%")
    print("NPV: $\(formatCurrency(investment.npv))")

    if let irr = investment.irr {
        print("IRR: \(formatPercent(irr))")
    }

    if let payback = investment.paybackPeriod {
        print("Payback Period: \(payback) periods")
    }
    print()
}
