...
func capitalBudgetingExample() throws {
    ...
    for option in result.selectedOptions.sorted(by: { $0.name < $1.name }) {
        ...
        print("  ✓ \(option.name)")
        print("    Allocation: \(Int(allocation * 100))%")
        print("    Funding: $\(Int(funding))")
        print("    Expected Value: $\(Int(option.expectedValue))")
...
func marketingAllocationExample() throws {
    ...
    for option in result.selectedOptions.sorted(by: {
        ...
        print("  \(option.name):")
        print("    Allocation: \(Int(allocation * 100))%")
        print("    Spend: $\(Int(spend))")
        print("    Expected Revenue: $\(Int(expectedRevenue))")
        print("    ROI: \(String(format: "%.1f", roi))x")
        print()
    }

    print("Total Expected Revenue: $\(Int(result.totalValue))")
    print("Total Spend: $\(Int(result.totalResourcesUsed["spend"] ?? 0))")
    let overallROI = result.totalValue / (result.totalResourcesUsed["spend"] ?? 1)
    print("Overall ROI: \(String(format: "%.1f", overallROI))x")
