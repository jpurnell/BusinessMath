...
func constructionExample() {
	...
	print("  Linear space [0, 10]: \(linspace.toArray())")

	let logspace = VectorN<Double>.logSpace(from: 1.0, to: 100.0, count: 3)
	print("  Log space [1, 100]: \(logspace.toArray())")
...
func portfolioWeightsExample() {
	...
	for (i, asset) in assets.enumerated() {
		print("  \(asset.paddingLeft(toLength: 5)): \(weights[i].percent())")
	}
	print()
	...
	for (i, asset) in assets.enumerated() {
		print("  \(asset.paddingLeft(toLength: 5)): \(returns[i].percent())")
	}
	...
	print("Portfolio Return: \(portfolioReturn.percent())")
	...
	for (i, asset) in assets.enumerated() {
		print("  \(asset.paddingLeft(toLength: 5)): \(weights[i].percent()) × \(returns[i].percent()) = \(contributions[i].percent())")
	}
...
func normalizationExample() {
	...
	print("  Z-scores: [\(zScores.toArray().map({ $0.number(2,)}).joined(separator: ", "))]")
