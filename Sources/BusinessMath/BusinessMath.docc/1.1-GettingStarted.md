...
let quarterlyRevenue: [Double] = [100, 120, 110, 150, 105, 125, 115, 160]  // Q4 holiday spike

let quarterlySeries = TimeSeries(
...
let indices = try seasonalIndices(timeSeries: quarterlySeries, periodsPerYear: 4)
// Result: [~0.85, ~1.01, ~0.91, ~1.23] (Q4 is 23% above average)

// Remove seasonality to see underlying trend
let deseasonalized = try seasonallyAdjust(timeSeries: quarterlySeries, indices: indices)
...
```swift
// 1. Load historical data (eight quarters of revenue)
let historicalPeriods = (0..<8).map { Period.quarter(year: 2023 + $0/4, quarter: ($0 % 4) + 1) }
let historicalRevenue: [Double] = [100, 120, 110, 150, 105, 125, 115, 160]
let revenueHistory = TimeSeries(periods: historicalPeriods, values: historicalRevenue)

// 2. Extract seasonality
let revenueIndices = try seasonalIndices(timeSeries: revenueHistory, periodsPerYear: 4)

// 3. Deseasonalize
let deseasonalizedRevenue = try seasonallyAdjust(timeSeries: revenueHistory, indices: revenueIndices)

// 4. Fit trend to deseasonalized data
var revenueTrend = LinearTrend<Double>()
try revenueTrend.fit(to: deseasonalizedRevenue)

// 5. Project trend forward (nil if the model has not been fit)
let trendForecast = try revenueTrend.project(periods: 4)

// 6. Reapply seasonality
if let trendForecast {
    let revenueForecast = try applySeasonal(timeSeries: trendForecast, indices: revenueIndices)
    print("Forecast:", revenueForecast.valuesArray)
}
```
...
// Anchored to a fixed instant rather than `Date()`: separate `Date()` calls are
// separate instants, so the gaps carry microseconds of run-to-run drift and every
// figure below moves in its low-order digits.
let valuationDate = Date(timeIntervalSince1970: 1_767_225_600)  // 2026-01-01T00:00:00Z
let dates = [
    valuationDate,                                        // initial investment
    valuationDate.addingTimeInterval(365 * 86400 * 1),    // Year 1
    valuationDate.addingTimeInterval(365 * 86400 * 2),    // Year 2
    valuationDate.addingTimeInterval(365 * 86400 * 3),    // Year 3
    valuationDate.addingTimeInterval(365 * 86400 * 4),    // Year 4
    valuationDate.addingTimeInterval(365 * 86400 * 5)     // Year 5
]
let investmentCashFlows = [-250000.0, 100_000, 150_000, 200_000, 250_000, 300_000]

// Calculate XNPV (irregular intervals)
let npvValue = try xnpv(rate: 0.10, dates: dates, cashFlows: investmentCashFlows)

// Calculate XIRR
let irrValue = try xirr(dates: dates, cashFlows: investmentCashFlows)

// Payback period
let payback = paybackPeriod(cashFlows: investmentCashFlows)

// Discounted payback period
let discountedPayback = discountedPaybackPeriod(rate: 0.10, cashFlows: investmentCashFlows)
```
