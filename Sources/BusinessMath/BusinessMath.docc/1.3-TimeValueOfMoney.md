...
// Result: ~1,219,971 (over $1 million!)
```
...
print("\(planA > planB ? "Plan A" : "Plan B") wins!")
...
```swift
let principalCumulative = 300_000.0
let rateCumulative = 0.06 / 12
let periodsCumulative = 360
...
let firstYearInterest = cumulativeInterest(
    ...
    startPeriod: 1,
    endPeriod: 12,
    totalPeriods: periodsCumulative,
    presentValue: principalCumulative,
    futureValue: 0,
    type: .ordinary
)
...
let cashFlowsNPV: [Double] = [-100_000, 50_000, 80_000]

let tsNPV = TimeSeries(periods: periodsNPV, values: cashFlowsNPV)
let npvValue = npv(rate: 0.10, timeSeries: tsNPV)
// Result: ~11,570
```
...
```swift
let cashFlowsPI = [-100_000.0, 30_000, 40_000, 50_000, 60_000]

let pi = profitabilityIndex(rate: 0.10, cashFlows: cashFlowsPI)
// Result: ~1.39
...
```swift
let cashFlowsPayback = [-100_000.0, 30_000, 40_000, 50_000, 60_000]

// Simple payback (ignoring time value)
let payback = paybackPeriod(cashFlows: cashFlowsPayback)
// Result: 3 periods (recovered after 3rd payment)

// Discounted payback (considering time value)
let discountedPayback = discountedPaybackPeriod(rate: 0.10, cashFlows: cashFlowsPayback)
...
```swift
let cashFlowsIRR = [-100_000.0, 30_000, 40_000, 50_000, 60_000]

let internalRateOfReturn = try irr(cashFlows: cashFlowsIRR)
// Result: ~24.9%
...
```swift
let cashFlowsIRRCheck = [-100_000.0, 30_000, 40_000, 50_000, 60_000]
let internalRateOfReturnCheck = try irr(cashFlows: cashFlowsIRRCheck)

// Verify: NPV at IRR should be ~0
let npvAtIRR = npv(discountRate: internalRateOfReturnCheck, cashFlows: cashFlowsIRRCheck)
// Result: ~-2.5e-11 — a relative residual of about 1.1e-16 on a $100,000 outlay, which
// is the last bit of a Double. This read ~0.000008 until `irr` stopped measuring
// convergence on the NPV residual in currency units and started measuring it on the
// Newton correction to the rate; the residual is now as near zero as the arithmetic
// allows, at any cash-flow scale rather than only at this one.
```
...
```swift
let cashFlowsMIRR = [-100_000.0, 30_000, 40_000, 50_000, 60_000]

// Finance negative flows at 8%, reinvest positive flows at 12%
let modifiedIRR = try mirr(
    cashFlows: cashFlowsMIRR,
    financeRate: 0.08,
...
do {
    ...
    print(error.errorDescription ?? "IRR calculation failed")
...
do {
    ...
    print(error.errorDescription ?? "IRR calculation failed")
    if let suggestions = error.recoverySuggestion {
...
//
// Anchored to a fixed instant rather than `Date()`, for the same reason as the
// settlement date in 3.10-BondValuationGuide. Four separate `Date()` calls are four
// separate instants: the gaps between them are 100, 250 and 400 days *plus* a few
// microseconds of drift that differs on every run, so the discounted value moved in
// its low-order digits and the figure below was not reproducible.
let valuationDate = Date(timeIntervalSince1970: 1_767_225_600)  // 2026-01-01T00:00:00Z
let dates = [
	valuationDate,                                  // initial investment
	valuationDate.addingTimeInterval(100 * 86400),  // 100 days later
	valuationDate.addingTimeInterval(250 * 86400),  // 250 days later
	valuationDate.addingTimeInterval(400 * 86400)   // 400 days later
]
let cashFlowsIrregular = [-100_000.0, 30_000, 50_000, 40_000]

let xnpvValue = try xnpv(rate: 0.10, dates: dates, cashFlows: cashFlowsIrregular)
// Result: ~12,100 (accounts for actual timing)

// Compare to regular NPV (assumes annual periods)
let regularNPV = npv(discountRate: 0.10, cashFlows: cashFlowsIrregular)
...
```swift
// Fixed dates, not `Date()`. XIRR answers a question about specific dates, so reading
// the first one from the clock made both figures below change between runs.
let xirrStart = Calendar.current.date(from: DateComponents(year: 2024, month: 1, day: 1)) ?? Date()
let datesXIRR = [
	xirrStart,                                        // Initial investment
	xirrStart.addingTimeInterval(100 * 86400),        // 100 days later
	xirrStart.addingTimeInterval(250 * 86400),        // 250 days later
	xirrStart.addingTimeInterval(400 * 86400)         // 400 days later
]
let cashFlowsXIRR = [-100_000.0, 30_000, 50_000, 40_000]

// Find IRR for irregular cash flows
let xirrValue = try xirr(dates: datesXIRR, cashFlows: cashFlowsXIRR)
// Result: ~29.4%

// Verify: XNPV at XIRR should be ~0
let xnpvAtXIRR = try xnpv(rate: xirrValue, dates: datesXIRR, cashFlows: cashFlowsXIRR)
// Result: ~0.0000001
```
...
// Fixed anchor, same reason as the XNPV example above.
let seriesADate = Date(timeIntervalSince1970: 1_767_225_600)  // 2026-01-01T00:00:00Z
let vcDates = [
    seriesADate,                                   // Series A
    seriesADate.addingTimeInterval(365 * 86400),   // Series B (1 year)
    seriesADate.addingTimeInterval(750 * 86400),   // Series C (2+ years)
    seriesADate.addingTimeInterval(1825 * 86400)   // Exit (5 years)
]
let vcCashFlows = [-5_000_000.0, -3_000_000, -2_000_000, 40_000_000]

let vcXIRR = try xirr(dates: vcDates, cashFlows: vcCashFlows)
// Result: ~37.2% (excellent return)

let vcXNPV = try xnpv(rate: 0.20, dates: vcDates, cashFlows: vcCashFlows)
// NPV at 20% hurdle rate ~7.2mm
```
...
guard rate > -1.0 && rate < 10.0 else {
    ...
    throw BusinessMathError.outOfRange(
        ...
        min: -1.0,
        max: 10.0,
        context: "discount rate per period"
    )
}
