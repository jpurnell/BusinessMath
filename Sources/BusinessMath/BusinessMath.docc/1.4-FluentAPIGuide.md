...
- Create scenarios using ``ScenarioSetBuilder``
...
// `company` is the Entity built under "Basic Model Construction" below
let quarters = [
...
let revenueValues: [Double] = [120_000, 135_000, 150_000, 165_000]
let cogsValues: [Double] = [48_000, 54_000, 60_000, 66_000]

var model = FinancialModel()
model.entity = company
model.revenueComponents.append(
    RevenueComponent(name: "Product Revenue", periods: quarters, values: revenueValues)
)
...
let priceValues: [Double] = [10.0, 10.0, 10.0, 10.0]
let quantityValues: [Double] = [12_000, 13_500, 15_000, 16_500]

let fluentModel = buildModel(for: company) {
    Revenue {
        Product("Product Revenue")
            .price(periods: quarters, values: priceValues)
            .quantity(periods: quarters, values: quantityValues)
    }

    Costs {
        FixedCost("COGS", periods: quarters, value: 50_000)
    }
}
```
...
let basicModel = buildModel(for: company) {
    Revenue {
...
// Access results
let basicRevenue = basicModel.calculateRevenue()  // 50,000 (50 × 1,000)
let basicCosts = basicModel.calculateCosts(revenue: basicRevenue)  // 40,000 (20k fixed + 20k variable)
let basicProfit = basicModel.calculateProfit()  // 10,000
```
...
```swift
// `company` is the Entity introduced above
let periodsProduct = [
	Period.year(2023),
	Period.year(2024),
	Period.year(2025),
...
let modelProduct = buildModel(for: company) {
	Revenue {
		// Product with time series for price and quantity
		Product("Widget")
			.price(periods: periodsProduct, values: [10.0, 10.0, 10.5, 10.5])
			.quantity(periods: periodsProduct, values: [10_000, 11_000, 11_500, 12_000])

		Product("Gadget")
			.price(periods: periodsProduct, values: [25.0, 25.0, 26.0, 26.0])
			.quantity(periods: periodsProduct, values: [2_000, 2_200, 2_300, 2_400])
	}

	Costs {
		// Fixed costs with time series
		FixedCost("Rent", periods: periodsProduct, value: 5_000)
		FixedCost("Salaries", periods: periodsProduct, value: 15_000)
...
let revenue2024 = modelProduct.totalRevenue(for: .year(2024))
let expenses2024 = modelProduct.totalExpenses(for: .year(2024))
let profit2024 = modelProduct.profit(for: .year(2024))
...
```swift
let modelMultiProduct = buildModel(for: company) {
	Revenue {
		// Hardware products
		Product("Laptop")
			.price(1200)
...
print(modelMultiProduct.calculateRevenue().currency())
...
let modelConditionalComponents = buildModel(for: company) {
	Revenue {
		// Always included
...
let arrayBuiltModel = buildModel(for: company) {
	Revenue {
...
let investmentAutoCalc = buildInvestment {
...
print("NPV: \(investmentAutoCalc.npv.currency())")              // ~$17,000
print("IRR: \(((investmentAutoCalc.irr ?? 0) * 100).number())%")        // ~17.1%
print("Payback: \(investmentAutoCalc.paybackPeriod ?? 0) years")  // 3 years
print("ROI: \(investmentAutoCalc.roi * 100)%")        // ~50%
```
...
let investmentDates = buildInvestment {
...
print("NPV: \(investmentDates.npv.currency(0))")
print("IRR: \((investmentDates.irr ?? 0).percent(1))")
```
...
// Compare metrics
print("Project A: NPV = \(investmentA.npv.currency()), IRR = \(((investmentA.irr ?? 0) * 100).number())%")
print("Project B: NPV = \(investmentB.npv.currency()), IRR = \(((investmentB.irr ?? 0) * 100).number())%")
...
```swift
let scenarioAdjusted = buildScenario {
...
let domesticRevenue = scenarioAdjusted.parameters["Domestic Revenue"]  // 1,000,000
let opexIncrease = scenarioAdjusted.parameters["Operating Expenses"]   // 50,000
```
...
let scenariosThreeWay = ScenarioSet.standardThreeWay(
...
// Access scenarios
for scenario in scenariosThreeWay.scenarios {
	if let revenue = scenario.parameters["revenue"],
	   let growth = scenario.parameters["growth"] {
		print("\(scenario.name): Revenue = \(revenue.currency(0)), Growth = \(growth.percent(1))")
	}
}
...
let sensitivitySet = ScenarioSet {
	for change in revenueChanges {
		ScenarioNamed("Revenue \(change > 0 ? "+" : "")\(change.percent(1))") {
...
let monthlyRevenue = buildTimeSeries(startingAt: jan) {
...
let revenueByPeriod = TimeSeries {
...
```swift
let labeledRevenue = buildTimeSeries(startingAt: jan) {
...
print(labeledRevenue.label(for: jan) ?? "No label")  // "January"
```
...
let historicalRevenue = buildTimeSeries(startingAt: jan) {
...
// Or with labels
let labeledHistoricalRevenue = buildTimeSeries(startingAt: jan) {
...
let seasonalRevenue = buildTimeSeries(startingAt: jan) {
	for month in 1...12 {
...
let growthRevenue = buildTimeSeries(startingAt: jan) {
...
let growthModel = buildModel(for: company) {
	Revenue {
		// Use projection builder for growing revenue
		let baseRevenue = 100_000.0
		let growthRate = 0.10

		for (index, _) in annualPeriods.enumerated() {
	...
	Costs {
		...
		// Fixed costs
		FixedCost("Operating Expenses", periods: annualPeriods, value: 20_000)
	}
}
```
...
```swift
	...
	let bundleModel = buildModel(for: company) {
		Revenue {
...
let fluentPerfModel = buildModel(for: company) {
    Revenue {
        Product("Sales")
            .price(periods: quarters, values: [100, 110, 120, 130])
            .quantity(periods: quarters, values: [1000, 1100, 1200, 1300])
    }
    Costs {
        FixedCost("Overhead", periods: quarters, value: 10_000)
    }
}

// Traditional (direct component assembly)
var traditionalPerfModel = FinancialModel()
traditionalPerfModel.entity = company
traditionalPerfModel.revenueComponents.append(
    RevenueComponent(name: "Sales", periods: quarters,
                     values: [100_000, 121_000, 144_000, 169_000])
)
traditionalPerfModel.costComponents.append(
    CostComponent(name: "Overhead", periods: quarters,
                  values: Array(repeating: 10_000, count: 4))
)
...
The model builders are non-throwing: a `buildModel` closure cannot contain `try`, so
validate the inputs before you open the builder.

```swift
// Validate before building - the builder closure cannot throw
guard periods.count == values.count else {
    fatalError("Periods and values must match")
}

let validatedModel = buildModel(for: company) {
    Revenue("Sales", periods: periods, values: values)
    FixedCost("Overhead", periods: periods, value: 10_000)
}
```

Construction that *does* throw — ``Account`` validates its name, roles and time
series — composes with `Result`:
...
case .success(let account):
    print("Built account: \(account.name)")
case .failure(let error):
    print("Error building account: \(error)")
}
```

### Testing Builder-Based Code

Test fluent APIs like any other code. These examples belong in a test target — they use
`swift-testing` and a `@testable` import:

```swift
import Testing
@testable import BusinessMath

@Suite("Fluent API builders")
struct FluentAPIBuilderTests {
...
let monthlyModel = buildModel(for: company) {
	// Revenue streams
	Product("Widget Pro")
...
// Analyze results
for month in months {
	let revenue = monthlyModel.totalRevenue(for: month)
	let expenses = monthlyModel.totalExpenses(for: month)
...
for scenario in comparisonScenarios {
...
for (index, investment) in ranked.enumerated() {
	...
	print("   IRR: \(((investment.irr ?? 0) * 100).number())%")
	print("   Payback: \((investment.paybackPeriod ?? .infinity).number()) years")
	print()
}

// Example Output:
// Investment Rankings by NPV:
// 1. Project B - Market Expansion
//    NPV: $32,658.29
//    IRR: 18.1%
//    Payback: 3 years
// 
// 2. Project C - Cost Reduction
//    NPV: -$262.96
//    IRR: 9.701%
//    Payback: 2.5 years
// 
// 3. Project A - Equipment Upgrade
//    NPV: -$1,183.32
//    IRR: 9.15%
//    Payback: 2.571 years
