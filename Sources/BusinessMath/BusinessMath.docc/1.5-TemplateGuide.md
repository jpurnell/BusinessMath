...
let advancedMRRProjection = advancedSaaSModel.projectMRR(months: 36)

// Analyze customer count trends
for month in [1, 12, 24, 36] {
    let customers = advancedSaaSModel.calculateCustomerCount(forMonth: month)
    let mrr = advancedSaaSModel.calculateMRR(forMonth: month)
...
for (name, advancedSaaSModel) in scenarios {
	let mrr = advancedSaaSModel.calculateMRR(forMonth: 12)
...
let subscriptionBoxModel = SubscriptionBoxModel(
...
if let months = extendedSaaSModel.monthsToTarget(targetMRR: 100_000) {
    print("Reach $100k MRR in \(months) months")
}

if let burnMultiple = extendedSaaSModel.calculateBurnMultiple(monthlyBurn: 50_000) {
...
for churn in churnRates {
	...
	let month24MRR = extendedSaaSModel.calculateMRR(forMonth: 24)
