...
let modelSaaS = SaaSModel(
	...
	let mrrProjection = try modelSaaS.projectMRR(months: 36)


	// Calculate key metrics
	let saasLTV = try modelSaaS.lifetimeValue().value      // Customer Lifetime Value
	let saasEconomics = try modelSaaS.acquisitionMetrics() // nil without a positive CAC
	let arr = try modelSaaS.calculateMRR(forMonth: 12) // Annual Recurring Revenue


	print("Year 1 ARR: \(arr.currency(0))")
	print("LTV: \(saasLTV.currency())")
	if let saasEconomics {
		print("LTV:CAC: \(saasEconomics.ratio.number(1))x")  // healthy is above 3.0
	}


	// Rule of 40 check
	let growthRate = try modelSaaS.calculateGrowthRate(from: 1, to: 12)
...
let advancedSaaSModel = SaaSModel(
    initialMRR: 50_000,
...
let advancedMRRProjection = try advancedSaaSModel.projectMRR(months: 36)

// Analyze customer count trends
for month in [1, 12, 24, 36] {
    let customers = try advancedSaaSModel.calculateCustomerCount(forMonth: month)
    let mrr = try advancedSaaSModel.calculateMRR(forMonth: month)
...
let cohortRetention = (1...12).map { month in
    1.0 - (advancedSaaSModel.churnRate * Double(month))
}
...
for (name, advancedSaaSModel) in scenarios {
	let mrr = try advancedSaaSModel.calculateMRR(forMonth: 12)
	let ltv = try advancedSaaSModel.lifetimeValue().value
	let economics = try advancedSaaSModel.acquisitionMetrics()

print("\(name) Case: (\(advancedSaaSModel.churnRate.percent(1)) Churn, \(advancedSaaSModel.newCustomersPerMonth.number(0)) New/month, \(advancedSaaSModel.customerAcquisitionCost!.currency(0)) CAC)")
	print("  MRR: \(mrr.currency(0))")
	print("  LTV: \(ltv.currency())")
	if let economics {
		print("  LTV:CAC: \(economics.ratio.number(1))x")
	}
	print()
}
```
...
// Calculate key metrics
let storeTotalRevenue = retailModel.calculateTotalRevenue()
let inventoryTurns = retailModel.calculateInventoryTurns()
let grossMargin = 1.0 - retailModel.costOfGoodsSold

print("Monthly Revenue: \(storeTotalRevenue.currency())")
print("Inventory Turns: \(inventoryTurns.number())x per year")
print("Gross Margin: \(grossMargin * 100)%")

// Store performance metrics
let revenuePerSqFt = retailModel.calculateRevenuePerSquareFoot(squareFootage: 2_000)
...
print("\nTotal Revenue: \(allStoresTotalRevenue.currency())")
print("Average per Store: \((allStoresTotalRevenue / Double(stores.count)).currency())")
```

### Inventory Management

```swift
let inventoryRetailModel = RetailModel(
	numberOfStores: 5,
...
let monthlyRevenue = inventoryRetailModel.calculateTotalRevenue()
let monthlyCOGS = monthlyRevenue * inventoryRetailModel.costOfGoodsSold
...
print("Inventory per Store: \((optimalInventory / Double(inventoryRetailModel.numberOfStores ?? 0)).currency())")
```
...
for (name, inventoryRetailModel) in scenariosManufacturing {
	let units = inventoryRetailModel.actualProduction ?? 0
	let utilization = inventoryRetailModel.calculateCapacityUtilization()
	let unitCost = inventoryRetailModel.calculateUnitCost()
	let revenue = Double(units) * inventoryRetailModel.sellingPricePerUnit
...
let subscriptionBoxModel = try SubscriptionBoxModel(
...
// Project growth
let subscriberProjection = subscriptionBoxModel.projectSubscribers(months: 24)

// Calculate metrics
let month12Subscribers = subscriptionBoxModel.calculateSubscribers(forMonth: 12)
let month12Revenue = subscriptionBoxModel.calculateRevenue(forMonth: 12)
let subscriptionLTV = try subscriptionBoxModel.lifetimeValue().value
let subscriptionEconomics = try subscriptionBoxModel.acquisitionMetrics()

print("Month 12 Projections:")
print("Subscribers: \(month12Subscribers.number())")
print("Monthly Revenue: \(month12Revenue.currency())")
print("LTV: \(subscriptionLTV.currency())")
if let subscriptionEconomics {
	print("LTV:CAC: \(subscriptionEconomics.ratio.number())x")
}
```
...
```swift
// Create a subscription box model
let modelCohortRetention = try SubscriptionBoxModel(
...
let standardPricingExperiment = try SubscriptionBoxModel(
...
let premiumPricingExperiment = try SubscriptionBoxModel(
	initialSubscribers: 500,
...
for (name, subscriptionBoxModel) in tiers {
	let subs = subscriptionBoxModel.calculateSubscribers(forMonth: 12)
	let revenue = subscriptionBoxModel.calculateRevenue(forMonth: 12)
	let grossMargin = (subscriptionBoxModel.monthlyBoxPrice - subscriptionBoxModel.costOfGoodsPerBox - subscriptionBoxModel.shippingCostPerBox) / subscriptionBoxModel.monthlyBoxPrice

	let tierLTV = try subscriptionBoxModel.lifetimeValue().value

	print("\n\(name) Tier:")
	print("  Subscribers: \(subs.number(0))")
	print("  Revenue: \(revenue.currency())")
	print("  Gross Margin: \(grossMargin.percent())")
	print("  LTV: \(tierLTV.currency())")
}
```
...
A template is immutable, so that does not compile. Build a new instance instead, carrying over
the values you want to keep:

```swift
let highGrowthModel = SaaSModel(
...
extension SaaSModel {
    ...
    func monthsToTarget(targetMRR: Double) throws -> Int? {
        for month in 1...120 {  // Search up to 10 years
            let mrr = try calculateMRR(forMonth: month)
            if mrr >= targetMRR {
                return month
            }
        }
    ...
    func calculateBurnMultiple(monthlyBurn: Double) throws -> Double? {
        let startMRR = initialMRR
        let endMRR = try calculateMRR(forMonth: 12)
...
let extendedSaaSModel = SaaSModel(
    initialMRR: 10_000,
    churnRate: 0.03,
...
if let months = try extendedSaaSModel.monthsToTarget(targetMRR: 100_000) {
    print("Reach $100k MRR in \(months) months")
}

if let burnMultiple = try extendedSaaSModel.calculateBurnMultiple(monthlyBurn: 50_000) {
...
let subscriptionRevenueCombo = try SubscriptionBoxModel(
	initialSubscribers: 2000,
	monthlyBoxPrice: 29.99,
...
for year in 1...3 {
	let month = year * 12
	let mrr = try modelProjection.calculateMRR(forMonth: month)
	let arr = try modelProjection.calculateARR()
	let customers = try modelProjection.calculateCustomerCount(forMonth: month)
	// An em dash rather than a number where the ratio is undefined — a model with no
	// positive acquisition cost has no LTV:CAC, and printing zero would read as a very
	// bad one rather than as an absent one.
	let economics = try modelProjection.acquisitionMetrics()
	let ratio = economics.map { "\($0.ratio.number(1))x" } ?? "—"

	print("\(year)    | \(mrr.currency(0)) | \(arr.currency(0)) | \(customers.number(0).paddingLeft(toLength: 9)) | \(ratio)")
}
```
...
for churn in churnRates {
	...
	let month24MRR = try extendedSaaSModel.calculateMRR(forMonth: 24)
