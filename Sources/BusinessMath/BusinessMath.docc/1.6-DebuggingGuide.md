...
(Note: example is structured to run in a playground, but that is not a requirement)

```swift
import PlaygroundSupport
// Keep playground running until async work completes
...
// Set log level
logger.log(level: .debug)  // Options: .debug, .info, .warning, .error, .critical
