...
```swift
let cycleDebugger = ModelDebugger()
let cycles = await cycleDebugger.detectCircularDependencies(in: model)
...
```swift
let dependencyDebugger = ModelDebugger()
let dependencyCycles = await dependencyDebugger.detectCircularDependencies(in: model)
