...
do {
    throw OpaqueError(message: "Invalid input")
} catch let error as OpaqueError {
    // Everything the handler has to work with is this one string
    print(error.message)
    // "Invalid input"  — which parameter? what would have been valid? how do I fix it?
}
```
...
do {
    ...
    // "Invalid input: Discount rate must be between 0 and 1 (provided: -0.5) (expected: 0.0 to 1.0)"

    print(error.recoverySuggestion!)
    // "Please provide a value within the range: 0.0 to 1.0"

    print(error.code)
    // "E001"

    print(error.helpAnchor!)
    // "https://businessmath.com/errors/E001"
}
```
...
A shared error type means one handler covers the lot. `report` below is that handler, and running
every call through it also shows where the categories are not what you would guess:
`logarithmicMean` rejects a non-positive argument as `invalidInput` (E001) *before* it reaches a
division, so it is the one call here that does not report E003.
...
func report(_ label: String, _ call: () throws -> Double) {
    do {
        print("\(label): returned \(try call()) — no error raised")
    } catch let error as BusinessMathError {
        print("\(label): \(error.code) — \(error.errorDescription!)")
    } catch {
        print("\(label): unexpected \(type(of: error))")
    }
}

// Statistical functions
report("harmonicMean") { try harmonicMean([60.0, 40.0, 0.0]) }          // contains zero (reciprocal calculation)
report("correlationCoefficient") { try correlationCoefficient(xSeries, ySeries) }  // either array has zero variance
report("coefficientOfSkew") { try coefficientOfSkew(flatValues) }       // standard deviation is zero
report("contraharmonicMean") { try contraharmonicMean(5.0, -5.0) }      // sum equals zero
report("logarithmicMean") { try logarithmicMean(-1.0, 2.0) }            // E001, not E003 — see above
report("weightedAverage") { try weightedAverage(flatValues, weights: [0.0, 0.0, 0.0]) }  // weights sum to zero

// Financial ratios (standalone functions)
report("currentRatio") { try currentRatio(currentAssets: 100.0, currentLiabilities: 0.0) }
report("quickRatio") { try quickRatio(currentAssets: 100.0, inventory: 20.0, currentLiabilities: 0.0) }
report("debtToEquity") { try debtToEquity(totalLiabilities: 100.0, shareholderEquity: 0.0) }
report("interestCoverage") { try interestCoverage(earningsBeforeInterestAndTax: 100.0, interestExpense: 0.0) }
report("roi") { try roi(gainFromInvestment: 100.0, costOfInvestment: 0.0) }
report("roe") { try roe(netIncome: 100.0, shareholderEquity: 0.0) }
report("profitMargin") { try profitMargin(netIncome: 100.0, revenue: 0.0) }
report("inventoryTurnover") { try inventoryTurnover(costOfGoodsSold: 100.0, averageInventory: 0.0) }
report("assetTurnover") { try assetTurnover(netSales: 100.0, averageTotalAssets: 0.0) }
```

### Numerical Instability (E004)

For numerical precision issues — a calculation whose *method* has broken down, as distinct from
one that merely failed to finish (E002).

``irr(cashFlows:guess:tolerance:maxIterations:)`` raises it when the derivative of NPV collapses
below 1e-6, which leaves the Newton-Raphson step undefined. That happens when one cash flow sits
far enough in the future that its discount factor dominates the sum:

```swift
// A single inflow 400 periods out: at the default 10% guess the discount factor is ~1e16,
// so dNPV/dr underflows the step floor while NPV is still ~-1.
...
do {
    ...
    if case .numericalInstability(let message, let suggestions) = error {
        print(message)
        // "IRR: Derivative of NPV is too small at rate 0.1 - the Newton-Raphson step is numerically unstable"
        for suggestion in suggestions { print("  • \(suggestion)") }
    }
}
```

It is also the case to reach for in your own guards around a calculation that can overflow:
...
Three scenario APIs raise this themselves, when a driver name does not appear in the base case's
`driverOverrides`:
...
Each checks every name before running any projection, so a typo costs nothing and one run reports
all of them. The `reason` lists the driver names that do exist, which is usually enough to spot the
typo without opening the scenario. These names are not otherwise validated anywhere: an unknown name
is *inserted* into the overrides rather than replacing anything, the builder never reads it, and the
result is a curve, a table or a bar that is silently constant.

It is also the right error for invalid driver *configuration*. Driver initializers do not validate
their inputs, so wrap construction in a factory that does:
...
do {
    ...
    if case .circularDependency(let path) = error {
        ...
        // • Using the prior period's value for the account that closes the loop
        // • Introducing an intermediate value
        //
        // FormulaEvaluator.accountNames(in:) reports which accounts a formula reads
        // without evaluating it, which is enough to locate the cycle.
        //
...
throws `BusinessMathError.inconsistentData` (E202) for the first offending period, with both sides
of the equation and the difference in the message. Catch it and re-add it to the same aggregator if
you want one combined report.
...
// Usage: log where the context is, rethrow so the caller still decides what to do.
// Logging is not handling — a layer that swallows the error here leaves the caller
// with a wrong answer and no way to know.
func loggedNPV(rate: Double, flows: [Double]) throws -> Double {
    do {
        return try calculateNPV(rate: rate, flows: flows)
    } catch {
...
do {
    print("NPV: \(try loggedNPV(rate: rate, flows: flows))")
} catch let error as BusinessMathError {
    // The caller decides: here, fall back to an undiscounted total and say so.
    print("NPV unavailable (\(error.code)); falling back to sum of flows: \(flows.reduce(0, +))")
}
```
...
The difference shows up in `recoverySuggestion`, which is what a caller actually surfaces:
...
do {
    ...
    print("GOOD:")
    print(error.recoverySuggestion!)
    // Names the three things a caller can change
}
...
do {
    ...
    print("BAD:")
    print(error.recoverySuggestion!)
    // The one suggestion is "Try different values" — which values? in which direction?
}
```
