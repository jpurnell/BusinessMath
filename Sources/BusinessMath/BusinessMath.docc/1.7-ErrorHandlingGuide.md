...
do {
	...
	throw BusinessMathError.invalidInput(message: "Discount rate must be non-negative", value: "-0.5", expectedRange: "≥ 0.0")
...
try harmonicMean([60, 40, 0])      // Throws: contains zero (reciprocal calculation)
try correlationCoefficient(x, y)   // Throws: if either array has zero variance
try coefficientOfSkew(values)      // Throws: if standard deviation is zero
try contraharmonicMean(5, -5)      // Throws: sum equals zero
try logarithmicMean(-1, 2)         // Throws: negative or zero values
try weightedAverage(values, weights: [0, 0, 0])  // Throws: weights sum to zero

// Financial ratios (standalone functions)
try currentRatio(currentAssets: 100, currentLiabilities: 0)   // Throws
try quickRatio(currentAssets: 100, inventory: 20, currentLiabilities: 0)  // Throws
try debtToEquity(totalDebt: 100, totalEquity: 0)  // Throws
try interestCoverage(ebit: 100, interestExpense: 0)  // Throws
try roi(netProfit: 100, investment: 0)  // Throws
try roe(netIncome: 100, shareholdersEquity: 0)  // Throws
try profitMargin(netIncome: 100, revenue: 0)  // Throws
try inventoryTurnover(cogs: 100, averageInventory: 0)  // Throws
try assetTurnover(revenue: 100, averageAssets: 0)  // Throws
```
