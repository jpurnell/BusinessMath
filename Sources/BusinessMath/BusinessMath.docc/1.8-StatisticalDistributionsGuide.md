...
// Spread/dispersion
let salesStdDev = stdDev(salesData)     // 11.76
let salesVariance = variance(salesData) // 138.29
...
let avgSpeed = try harmonicMean(speeds)  // 42.86 mph (correct for average speed)

// Weighted average
// Throws if sum of weights is zero
let grades = [85.0, 90.0, 78.0]
let weights = [0.3, 0.5, 0.2]  // 30%, 50%, 20%
let finalGrade = try weightedAverage(grades, weights: weights)  // 85.9
```
...
let normalSamples = (0..<1000).map { _ in normalDist.random() }
...
let chiSquaredSample: Double = distributionChiSquared(degreesOfFreedom: df)

// Student's t (small sample sizes)
let tSample: Double = distributionT(degreesOfFreedom: df)

// F-distribution (comparing variances)
let fSample: Double = distributionF(df1: 5, df2: 10)
