...
for (volume, profit) in profitTable {
	...
	print("\(volume.number(0).paddingLeft(toLength: 8))\(revenue.currency().paddingLeft(toLength: 15))\(totalCosts.currency().paddingLeft(toLength: 16))\(profit.currency().paddingLeft(toLength: 16)) \(status)")
}
...
--------	-----------		-----------		-------------
   1,000     $25,000.00      $65,000.00    ($40,000.00) ✗
   2,000     $50,000.00      $80,000.00    ($30,000.00) ✗
   3,000     $75,000.00      $95,000.00    ($20,000.00) ✗
   4,000    $100,000.00     $110,000.00    ($10,000.00) ✗
   5,000    $125,000.00     $125,000.00           $0.00 ✓
   6,000    $150,000.00     $140,000.00      $10,000.00 ✓
   7,000    $175,000.00     $155,000.00      $20,000.00 ✓
   8,000    $200,000.00     $170,000.00      $30,000.00 ✓
   9,000    $225,000.00     $185,000.00      $40,000.00 ✓
  10,000    $250,000.00     $200,000.00      $50,000.00 ✓
...
let monthlyFixedCostsPricingStrategy = 100_000.0
let variableCostPerUnitPricingStrategy = 30.0
...
calculate: { price, volume in
	...
	let totalCosts = monthlyFixedCostsPricingStrategy + (variableCostPerUnitPricingStrategy * volume)
	return revenue - totalCosts
}
)
...
print("=========================================")
print("Fixed Costs: \(monthlyFixedCostsPricingStrategy.currency())")
print("Variable Cost: \(variableCostPerUnitPricingStrategy.currency())/unit")
print()

// Print header row with volume scenarios
var headerStringPricingStrategy = "Price".padding(toLength: 10, withPad: " ", startingAt: 0)
for volume in volumeScenarios {
headerStringPricingStrategy += "\(volume.number())".paddingLeft(toLength: 14)
}
print(headerStringPricingStrategy)
...
print("\nOptimal Strategy:")
print(
"Price: \(optimalPrice.currency()), Volume: \(optimalVolume.number()) units"
)
...
Price           2,000.00      2,500.00      3,000.00      3,500.00      4,000.00
================================================================================
...
Optimal Strategy:
Price: $60.00, Volume: 4,000.00 units
...
```swift
// Base investment
let initialCostInvestmentScenario = 50_000.0
let baseRevenueInvestmentScenario = 20_000.0
let years = 5

// Scenarios
let growthRatesInvestmentScenario = Array(stride(from: 0.05, through: 0.25, by: 0.05))  // 5% to 25% annual growth
let discountRatesInvestmentScenario = Array(stride(from: 0.08, through: 0.16, by: 0.02)) // 8% to 16% required return

let npvMatrix = DataTable<Double, Double>.twoVariable(
	rowInputs: growthRatesInvestmentScenario,
	columnInputs: discountRatesInvestmentScenario,
	calculate: { growth, discount in
		// Generate cash flows with growth
		var flows = [-initialCostInvestmentScenario]  // Initial investment
		var revenue = baseRevenueInvestmentScenario
...
print("=========================")
print("Initial Investment: \(initialCostInvestmentScenario.currency())")
print("Base Revenue: \(baseRevenueInvestmentScenario.currency())")
print("Period: \(years) years")
print()

// Print header
var headerStringInvestmentScenario = "Growth\\Discount".padding(toLength: 16, withPad: " ", startingAt: 0)
for discount in discountRatesInvestmentScenario {
	headerStringInvestmentScenario += "\(discount.percent())".paddingLeft(toLength: 16)
}
print(headerStringInvestmentScenario)
print(String(repeating: "=", count: 96))

// Print rows
for (rowIndex, growth) in growthRatesInvestmentScenario.enumerated() {
	var rowString = "\(growth.percent())".padding(toLength: 16, withPad: " ", startingAt: 0)
//		print(String(format: "%.0f%%\t\t", growth * 100), terminator: "")

	for colIndex in 0..<discountRatesInvestmentScenario.count {
...
var totalScenarios = growthRatesInvestmentScenario.count * discountRatesInvestmentScenario.count
...
let maxMonthlyPaymentLoanAffordability = 2_000.0
let loanTermMonthsLoanAffordability = 360  // 30 years

// Scenarios
let loanAmountsLoanAffordability = Array(stride(from: 200000.0, through: 450000, by: 50000))
let interestRatesLoanAffordability = Array(stride(from: 0.03, through: 0.060, by: 0.005))

let paymentMatrix = DataTable<Double, Double>.twoVariable(
	rowInputs: loanAmountsLoanAffordability,
	columnInputs: interestRatesLoanAffordability,
	calculate: { loanAmount, annualRate in
		let monthlyRate = annualRate / 12.0
		return payment(
			presentValue: loanAmount,
			rate: monthlyRate,
			periods: loanTermMonthsLoanAffordability,
			futureValue: 0,
			type: .ordinary
		)
	}
...
print("============================================")
print("Maximum Affordable Payment: \(maxMonthlyPaymentLoanAffordability.currency())")
print("Term: 30 years")
print()

// Print header
var headerStringLoanAffordability = "Loan Amount".padding(toLength: 16, withPad: " ", startingAt: 0)
for rate in interestRatesLoanAffordability {
	headerStringLoanAffordability += "\(rate.percent(1))".paddingLeft(toLength: 13, withPad: " ")
}
print(headerStringLoanAffordability)
print(String(repeating: "=", count: 107))

// Print rows with affordability indicators
for (rowIndex, amount) in loanAmountsLoanAffordability.enumerated() {
	var rowString = "\((amount/1000).currency(0))k".padding(toLength: 16, withPad: " ", startingAt: 0)
	for colIndex in 0..<interestRatesLoanAffordability.count {
		let monthlyPmt = paymentMatrix[rowIndex][colIndex]
		let affordable = monthlyPmt <= maxMonthlyPaymentLoanAffordability ? "✓" : "✗"
...
for (colIndex, rate) in interestRatesLoanAffordability.enumerated() {
	var maxAffordable = 0.0

	for (rowIndex, amount) in loanAmountsLoanAffordability.enumerated() {
		let monthlyPmt = paymentMatrix[rowIndex][colIndex]
		if monthlyPmt <= maxMonthlyPaymentLoanAffordability {
			maxAffordable = amount
		}
	}

	print("At \(rate.percent(1)): \(maxAffordable.currency(0))")
}
```
...
Loan Affordability Matrix (Monthly Payment)
============================================
Maximum Affordable Payment: $2,000.00
Term: 30 years

Loan Amount              3.0%         3.5%         4.0%         4.5%         5.0%         5.5%         6.0%
===========================================================================================================
$200k                $843.21✓     $898.09✓     $954.83✓   $1,013.37✓   $1,073.64✓   $1,135.58✓   $1,199.10✓
$250k              $1,054.01✓   $1,122.61✓   $1,193.54✓   $1,266.71✓   $1,342.05✓   $1,419.47✓   $1,498.88✓
$300k              $1,264.81✓   $1,347.13✓   $1,432.25✓   $1,520.06✓   $1,610.46✓   $1,703.37✓   $1,798.65✓
$350k              $1,475.61✓   $1,571.66✓   $1,670.95✓   $1,773.40✓   $1,878.88✓   $1,987.26✓   $2,098.43✗
$400k              $1,686.42✓   $1,796.18✓   $1,909.66✓   $2,026.74✗   $2,147.29✗   $2,271.16✗   $2,398.20✗
$450k              $1,897.22✓   $2,020.70✗   $2,148.37✗   $2,280.08✗   $2,415.70✗   $2,555.05✗   $2,697.98✗
...
**Insight:** Rate increases above 4.0% significantly reduce buying power - from $450k to $350k.
...
```swift
// Create a data table
let ratesFormatting = [0.03, 0.04, 0.05, 0.06, 0.07]
let paymentTableFormatting = DataTable<Double, Double>.oneVariable(
	inputs: ratesFormatting,
	calculate: { rate in
		payment(
...
```swift
let pricesTwoVar = [10.0, 12.0, 14.0]
let volumesTwoVar = [100.0, 200.0, 300.0]
let fixedCostTwoVar = 500.0
let variableCostTwoVar = 5.0

let profitMatrixTwoVar = DataTable<Double, Double>.twoVariable(
	rowInputs: pricesTwoVar,
	columnInputs: volumesTwoVar,
	calculate: { price, volume in
		(price * volume) - fixedCostTwoVar - (variableCostTwoVar * volume)
	}
)

let csvTwoVar = DataTable.toCSV(
	profitMatrixTwoVar,
	rowInputs: pricesTwoVar,
	columnInputs: volumesTwoVar
)
...
let consoleVolumes = [100.0, 200.0, 300.0]
let fixedCost = 500.0
let variableCost = 5.0

let consoleProfitMatrix = DataTable<Double, Double>.twoVariable(
	rowInputs: prices,
	columnInputs: consoleVolumes,
	calculate: { price, volume in
...
let irregularCashFlows = [-investmentAmount, 25_000.0, 30_000.0, 35_000.0, 40_000.0]

// XNPV sensitivity table
let xnpvTable = DataTable<Double, Double>.oneVariable(
	inputs: rateScenarios,
	calculate: { rate in
		try! xnpv(rate: rate, dates: dates, cashFlows: irregularCashFlows)
	}
)
...
let goodRates = [0.05, 0.06, 0.07, 0.08, 0.09]  // ±2% from 7% base

// ✗ Bad: Too wide, includes unrealistic scenarios
let badRates = [0.01, 0.10, 0.20, 0.30, 0.40]
```
...
let goodPrices = Array(stride(from: 10.0, through: 20.0, by: 1.0))

// ✗ Bad: Too coarse, might miss optimal point
let badPrices = [10.0, 20.0]
```
...
let validatedProfitMatrix = DataTable<Double, Double>.twoVariable(
    rowInputs: prices,
    columnInputs: consoleVolumes,
    calculate: { price, volume in
...
print("IRR (approximate): \(irrApprox.percent(2))")
```
...
let optimalPricePoint = DataTable<Double, Double>.oneVariable(
...
if let optimal = optimalPricePoint {
	print("Optimal price: \(optimal.input.currency())")
	print("Maximum profit: \(optimal.output.currency(0))")
}
```
...
let riskAcceptableScenarios = npvMatrix.flatMap { $0 }
    .filter { $0 > 0 }
    .count

let riskTotalScenarios = growthRatesInvestmentScenario.count * discountRatesInvestmentScenario.count
let successRate = Double(riskAcceptableScenarios) / Double(riskTotalScenarios)

print("Success rate: \(successRate.percent(0))")
```
