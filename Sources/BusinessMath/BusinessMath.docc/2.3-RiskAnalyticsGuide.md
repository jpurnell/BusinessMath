...
for scenario in scenarios {
    ...
    print("  Shocks:")
    // `shocks` is a dictionary; sort by driver so the report reads the same every run.
    for (driver, shock) in scenario.shocks.sorted(by: { $0.key < $1.key }) {
        let pct = shock * 100
...
// 250 simulated daily returns. Drawn through an explicitly seeded generator rather than
// the free `distributionNormal(mean:stdDev:)`, which has no seed parameter and so would
// produce a different sample on every run.
let returns: [Double] = {
	var rng = SplitMix64(seed: 8_675_309)
	let draw = DistributionNormal(spReturnMean, spReturnStdDev)
	return (0..<250).map { _ in draw.next(using: &rng) }
}()

	// The series starts on a fixed date rather than `Date()`, so the periods — and every
	// risk figure derived from them — are the same on any day the example is run.
	let seriesStart = Date(timeIntervalSince1970: 1_735_776_000)  // 2025-01-02T00:00:00Z
	let periods = (0..<spReturns.count).map { Period.day(seriesStart.addingTimeInterval(Double($0) * 86400)) }
...
// CVaR is the expected loss if we're in the worst 5%
let expectedShortfallLoss = abs(riskMetrics.cvar95) * portfolioValue
print("  If in worst 5% of days, expect to lose: \(expectedShortfallLoss.currency())")
```
...
// Add current snapshot. Stamp it with the last period in the return series rather than
// `Date()`: the metrics describe that day, and a fixed stamp keeps the example reproducible.
let asOfDate = seriesStart.addingTimeInterval(Double(spReturns.count - 1) * 86_400)
