...
// generator, and re-seeding it with 2401 reproduces the run exactly. Drop the
// generator and call `revenueDistribution.random()` for a fresh draw each run.
var revenueGenerator = Xoshiro256StarStar(seed: 2401)
let revenueDistribution = DistributionNormal(1_000_000, 100_000)  // (mean, stdDev)

// Run a Monte Carlo simulation
var revenueValues: [Double] = []
for _ in 0..<1_000 {
    // Simulate revenue with uncertainty
    let revenue = revenueDistribution.next(using: &revenueGenerator)
...
**Output:**
```
Histogram (20 bins, 1000 samples):

[  672833 -   703844):                                                        2 (0.2%)
[  703844 -   734854):                                                        2 (0.2%)
[  734854 -   765864):  ██                                                    7 (0.7%)
[  765864 -   796875):  ██                                                    8 (0.8%)
[  796875 -   827885):  █████                                                16 (1.6%)
[  827885 -   858895):  ███████████                                          34 (3.4%)
[  858895 -   889906):  ███████████████████                                  57 (5.7%)
[  889906 -   920916):  ███████████████████████                              68 (6.8%)
[  920916 -   951927):  ██████████████████████████████████████              112 (11.2%)
[  951927 -   982937):  ███████████████████████████████████████             115 (11.5%)
[  982937 -  1013947):  ██████████████████████████████████████████████████  145 (14.5%)
...
```
...
var durationGenerator = Xoshiro256StarStar(seed: 2402)
let durationDistribution = DistributionNormal(200, 14)  // (mean, stdDev)
let projectDurations = (0..<500).map({ _ in durationDistribution.next(using: &durationGenerator) })
...
// Risk analysis
let deadlineDays = 210.0  // Date we committed to the client
...
Price    ◄█████████████████████████|█████████████████████████► Impact: 40000 133.3%
		   10000                 30000                 50000)
Cost     ◄          ███████████████|███████████████          ► Impact: 24000 80.0%
		   18000                 30000                 42000)
Volume   ◄               ██████████|██████████               ► Impact: 16000 53.3%
		   22000                 30000                 38000)
OpEx     ◄                       ██|███                      ► Impact: 4000 13.3%
		   28000                 30000                 32000)
Tax Rate ◄                         |                         ► Impact: 0 0.0%
		   30000                 30000                 30000)
```
...
for (i, input) in tornado.inputs.prefix(3).enumerated() {
	...
	print("\(i+1). \(input): ±\(percentImpact.percent()) impact on output")
}
...
// Setup periods and entity
	let q1CV = Period.quarter(year: 2024, quarter: 1)
	let q2CV = Period.quarter(year: 2024, quarter: 2)
	let q3CV = Period.quarter(year: 2024, quarter: 3)
	let q4CV = Period.quarter(year: 2024, quarter: 4)
	let periodsCV = [q1CV, q2CV, q3CV, q4CV]
let entityCV = Entity(id: "CS123", name: "Coffee Shop")

// Create base case scenario
	var driverOverridesCV: [String: AnyDriver<Double>] = [:]
	driverOverridesCV["Price"] = AnyDriver(DeterministicDriver(name: "Price", value: 100.0))
	driverOverridesCV["Volume"] = AnyDriver(DeterministicDriver(name: "Volume", value: 1000.0))
	driverOverridesCV["Cost"] = AnyDriver(DeterministicDriver(name: "Cost", value: 60.0))
	driverOverridesCV["OpEx"] = AnyDriver(DeterministicDriver(name: "OpEx", value: 10_000.0))

	let baseCaseCV = FinancialScenario(
		name: "Base Case",
		description: "Standard operating scenario",
		driverOverrides: driverOverridesCV
	)
...
let builderCV: ScenarioRunner.StatementBuilder = { drivers, periods in
	...
	let revenueAccount = try Account(entity: entityCV, name: "Revenue", incomeStatementRole: .revenue, timeSeries: revenueSeries)
	let cogsAccount = try Account(entity: entityCV, name: "COGS", incomeStatementRole: .costOfGoodsSold, timeSeries: cogsSeries)
	let opexAccount = try Account(entity: entityCV, name: "OpEx", incomeStatementRole: .operatingExpenseOther, timeSeries: opexSeries)
	...
	let assetAccount = try Account(entity: entityCV, name: "Cash", balanceSheetRole: .cashAndEquivalents, timeSeries: netSeries)
	let equityAccount = try Account(entity: entityCV, name: "Equity", balanceSheetRole: .commonStock, timeSeries: netSeries)
	let balanceSheet = try BalanceSheet(
		entity: entityCV,
		periods: periods,
	...
	let cashAccount = try Account(entity: entityCV, name: "Cash", cashFlowRole: .otherOperatingActivities, timeSeries: netSeries)
	let cashFlowStatement = try CashFlowStatement(
		entity: entityCV,
		periods: periods,
		accounts: [cashAccount]
...
let tornadoCV = try runTornadoAnalysis(
	...
	return projection.incomeStatement.netIncome[q1CV]!
}
...
// Simulate the top driver with uncertainty (±20% standard deviation),
// from its own seeded generator so the histogram below reproduces exactly
let baseValue = 100.0  // Base price from our scenario
var priceGenerator = Xoshiro256StarStar(seed: 2403)
let priceDistribution = DistributionNormal(baseValue, baseValue * 0.2)  // (mean, stdDev)

var netIncomeResults: [Double] = []
for _ in 0..<10_000 {
	let simulatedPrice = priceDistribution.next(using: &priceGenerator)
...
print("\nKey Insights:")
print("- \(topDriverCV) has ±\(tornadoCV.impacts[topDriverCV]!.currency(0)) impact (tornado)")
...
`String.write(toFile:atomically:encoding:)` does not create intermediate
directories: if `outputs/` does not already exist it fails with
`NSCocoaErrorDomain Code=4 ... No such file or directory`. Create the directory
first. These paths are relative, so they resolve against the process's working
directory — pass an absolute path if you need the files somewhere specific.

```swift
import Foundation

// Create the output directory before writing into it
let outputDirectory = "outputs"
try FileManager.default.createDirectory(
	atPath: outputDirectory,
	withIntermediateDirectories: true
)

// Save the histogram built earlier to a file
let histogramPlot = plotHistogram(histogram)

let histogramPath = "\(outputDirectory)/revenue_distribution.txt"
try histogramPlot.write(toFile: histogramPath, atomically: true, encoding: .utf8)
print("Histogram saved to \(histogramPath)")

// Save the tornado diagram built earlier to a file
let tornadoPath = "\(outputDirectory)/sensitivity_analysis.txt"
...
- Risk (95% VaR): \(results.valueAtRisk(confidenceLevel: 0.95).currency(0))
"""

try report.write(toFile: "\(outputDirectory)/full_report.txt", atomically: true, encoding: .utf8)
```
