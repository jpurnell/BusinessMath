...
```swift
let automatedReport = try ReciprocalParameterRecoveryCheck.run(
    trueA: 0.2,
    trueB: 0.3,
    trueSigma: 0.2,
    n: 100,
...
```swift
// Test boundaries
try ReciprocalParameterRecoveryCheck.run(trueA: 0.001, trueB: 0.001, trueSigma: 0.001)  // Near zero
try ReciprocalParameterRecoveryCheck.run(trueA: 10.0, trueB: 10.0, trueSigma: 1.0)      // Large values

// Test high noise
try ReciprocalParameterRecoveryCheck.run(trueA: 0.2, trueB: 0.3, trueSigma: 2.0)        // SNR << 1
```
