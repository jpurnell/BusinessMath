...
let verification = 100_000.0 * pow(1.1447, 3.0)
// Result: ~150,000 ✓
```
...
// `project(periods:)` returns nil when the model was not fitted to a TimeSeries.
guard let linearForecast = try linearTrend.project(periods: 6) else {
...
guard let exponentialForecast = try exponentialTrend.project(periods: 5) else {
	fatalError("exponentialTrend was not fitted to a TimeSeries")
}
...
guard let logisticForecast = try logisticTrend.project(periods: 12) else {
	fatalError("logisticTrend was not fitted to a TimeSeries")
}
...
guard let customForecast = try customTrend.project(periods: 12) else {
	fatalError("customTrend was not fitted to a TimeSeries")
}
...
```swift
let comparisonHistorical = TimeSeries(periods: periods, values: values)

// Fit multiple models
var comparisonLinear = LinearTrend<Double>()
try comparisonLinear.fit(to: comparisonHistorical)

var comparisonExponential = ExponentialTrend<Double>()
try comparisonExponential.fit(to: comparisonHistorical)

var comparisonLogistic = LogisticTrend<Double>(capacity: 10_000)
try comparisonLogistic.fit(to: comparisonHistorical)

// Compare on holdout data
let trainData = comparisonHistorical.range(from: periods[0], to: periods[8])
let testData = comparisonHistorical.range(from: periods[9], to: periods[11])

// Fit on training data, evaluate on test data
var linearModel = LinearTrend<Double>()
try linearModel.fit(to: trainData)
guard let holdoutLinearForecast = try linearModel.project(periods: 3) else {
	fatalError("linearModel was not fitted to a TimeSeries")
}

// Calculate error metrics using the built-in API
let linearMetrics = testData.forecastError(against: holdoutLinearForecast)
print("Linear Model:")
...
guard let holdoutExponentialForecast = try exponentialModel.project(periods: 3) else {
	fatalError("exponentialModel was not fitted to a TimeSeries")
}
let expMetrics = testData.forecastError(against: holdoutExponentialForecast)
print("\nExponential Model:")
...
let quarterlyPeriods = (0..<12).map { Period.quarter(year: 2022 + $0/4, quarter: ($0 % 4) + 1) }
let revenue: [Double] = [100, 120, 110, 150,  // 2022
                         105, 125, 115, 160,  // 2023
                         110, 130, 120, 170]  // 2024

let ts = TimeSeries(periods: quarterlyPeriods, values: revenue)
...
guard let trendForecast = try trend.project(periods: 4) else {
	fatalError("trend was not fitted to a TimeSeries")
}
...
```swift
// 1. Load historical data — three years of quarterly revenue
let historicalPeriods = (0..<12).map { Period.quarter(year: 2022 + $0/4, quarter: ($0 % 4) + 1) }
let historicalRevenue: [Double] = [820_000, 910_000, 855_000, 1_180_000,   // 2022
                                   880_000, 975_000, 915_000, 1_265_000,   // 2023
                                   945_000, 1_045_000, 980_000, 1_350_000] // 2024

let workflowHistorical = TimeSeries(periods: historicalPeriods, values: historicalRevenue)

// 2. Extract seasonal pattern
let workflowIndices = try seasonalIndices(timeSeries: workflowHistorical, periodsPerYear: 4)

// 3. Deseasonalize to reveal underlying trend
let workflowDeseasonalized = try seasonallyAdjust(timeSeries: workflowHistorical, indices: workflowIndices)

// 4. Fit trend model to deseasonalized data
var workflowTrend = LinearTrend<Double>()
try workflowTrend.fit(to: workflowDeseasonalized)

// 5. Project trend forward
let forecastPeriods = 4  // Next 4 quarters
guard let workflowTrendForecast = try workflowTrend.project(periods: forecastPeriods) else {
    fatalError("workflowTrend was not fitted to a TimeSeries")
}

// 6. Reapply seasonality to trend forecast
let workflowSeasonalForecast = try applySeasonal(timeSeries: workflowTrendForecast, indices: workflowIndices)

// 7. Present forecast with confidence intervals
let trendCI = try workflowTrend.projectWithConfidence(
    periods: forecastPeriods,
    confidenceLevel: 0.95
)

for i in 0..<forecastPeriods {
    let period = workflowSeasonalForecast.periods[i]
    let forecast = workflowSeasonalForecast.valuesArray[i]
    // Scale CI bounds by same seasonal factor
    let seasonalFactor = workflowSeasonalForecast.valuesArray[i] / workflowTrendForecast.valuesArray[i]
...
let assumedGrowthRate = 0.50  // 50% per year
let projection10y = currentRevenue * pow(1 + assumedGrowthRate, 10)
// Result: $57.7M
...
```swift
// Split historical data — two years of monthly observations
let allPeriods = (0..<24).map { Period.month(year: 2023 + $0/12, month: ($0 % 12) + 1) }
let allValues: [Double] = (0..<24).map { 100.0 + 5.0 * Double($0) }
...
let backtestTrain = allData.range(from: allPeriods[0], to: trainEnd)
let backtestTest = allData.range(from: trainEnd + 1, to: allPeriods.last!)

// Fit on training data
var model = LinearTrend<Double>()
try model.fit(to: backtestTrain)
...
// The same history as before, now extended with the latest six months
let newPeriods = (0..<30).map { Period.month(year: 2023 + $0/12, month: ($0 % 12) + 1) }
let newValues: [Double] = (0..<30).map { 100.0 + 5.0 * Double($0) }
