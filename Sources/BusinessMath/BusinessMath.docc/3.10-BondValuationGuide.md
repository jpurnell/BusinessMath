...
// Settlement date is fixed rather than `Date()` so every price, yield, duration and
// spread printed in this guide is reproducible — with a live clock they would change
// every day the example is run.
//
// The calendar is fixed for the same reason. `Calendar.current` carries the reader's
// time zone, so a date built with it is a different instant in Tokyo than in New York —
// and a bond's coupon schedule is a schedule of calendar dates, not of local midnights.
...
print("Price: \(marketPrice.currency())")
// Output: Price: $1,043.66 (trades at premium since coupon > yield)

// Calculate current yield
let currentYield = bond.currentYield(price: marketPrice)
print("Current Yield: \(currentYield.percent())")
...
### The price depends on the day-count convention, and says which one it used

`Bond` takes a `dayCount:` that defaults to **ACT/365**, and the figure above is that
default. It is not the only defensible answer for these inputs:

| Convention | Price | Why it differs |
|---|---|---|
| ACT/365 *(default)* | $1,043.66 | 1,826 actual days over a 365-day year is 5.0027 years — the 2028 leap day makes it fractionally more than five |
| **30/360** | **$1,043.76** | every month is 30 days, so this is *exactly* five years |
| ACT/ACT (ISDA) | $1,043.79 | actual days over each year's own length |
| ACT/360 | $1,040.51 | the same days over a 360-day year, so 5.07 years and materially more discounting |

**30/360 is the one that reproduces the textbook formula.** Price the same bond by hand as
ten half-year periods — `Σ 30/(1.025)^k + 1000/(1.025)^10` — and you get **1043.7603**,
which is the 30/360 column to ten significant figures. That is not a coincidence: the
whole-period formula every textbook prints *assumes* 30/360 without saying so, because
assuming it is what makes "five years" mean exactly ten periods.

So if you are checking this library against a worked example in a textbook, pass
`dayCount: .thirty360` and the numbers will agree. If you are checking it against a
counterparty, pass whatever basis the bond is quoted on.

> Before 2026-09-13 this guide printed **$1,043.82**, from a `365.25` constant applied to a
> raw seconds interval. That is the mean length of a Gregorian year, and it is not a
> day-count convention — no standard names it and no counterparty settles on it. The figure
> was never canonical; it was an artefact of the arithmetic. See ``DayCountConvention``.
...
let analysisCalendar = Calendar.current
// Same fixed settlement date as Step 1 (2026-01-01T00:00:00Z), so the figures below
// are reproducible instead of drifting with the wall clock.
let analysisToday = today

	// Step 1: Credit Analysis
let analysisCreditModel = CreditSpreadModel<Double>()
let companyZScore = 2.2
let companyPD = analysisCreditModel.defaultProbability(zScore: companyZScore)
...
let companySpread = analysisCreditModel.creditSpread(
...
// Step 3: Bond Pricing
let analysisMaturity = analysisCalendar.date(byAdding: .year, value: 7, to: analysisToday)!
...
let analysisBond = Bond(
	...
	paymentFrequency: .semiAnnual,
	issueDate: analysisToday
)

let riskFree = 0.030  // 3.0% Treasury
let fairYield = riskFree + companySpread
let fairValue = analysisBond.price(yield: fairYield, asOf: analysisToday)
...
let analysisCallDate = analysisCalendar.date(byAdding: .year, value: 3, to: analysisToday)!
...
// Step 5: Risk Metrics
let bondDuration = analysisBond.macaulayDuration(yield: fairYield, asOf: analysisToday)
let bondConvexity = analysisBond.convexity(yield: fairYield, asOf: analysisToday)
...
let analysisMarketPrice = 1015.00  // Hypothetical market price

print("\nInvestment Decision:")
print("  Market Price: \(analysisMarketPrice.currency(2))")
print("  Fair Value: \(callableValue.currency(2))")

if callableValue > analysisMarketPrice {
	let upside = ((callableValue / analysisMarketPrice) - 1.0)
	print("  Assessment: UNDERVALUED by \(upside.percent(1))")
	print("  Recommendation: BUY")
} else {
	let downside = (1.0 - (callableValue / analysisMarketPrice))
...
- Explore **<doc:2.3-RiskAnalyticsGuide>** for portfolio-level risk management
- See **<doc:1.3-TimeValueOfMoney>** for foundational present value concepts
- Review **<doc:3.8-InvestmentAnalysis>** for asset allocation strategies
- Study **<doc:4.2-ScenarioAnalysisGuide>** for stress testing bond portfolios
