...
// project(periods:) returns an optional: nil when the model was never fitted
// to a TimeSeries (and so has no period information to project from).
guard let validationForecast = try validationModel.project(periods: 6) else {
	fatalError("LinearTrend was not fitted to a TimeSeries")
}
...
The ``ForecastErrorMetrics`` struct returned by `forecastError(against:)` provides multiple accuracy metrics:
...
### Letting the Fit Choose Them

`fitETS(seasonality:config:)` searches for α, β and γ rather than asking you for them, and
reports how well the answer fits. It minimises the in-sample sum of squared one-step-ahead
residuals — the standard objective for ETS — over a simplex search seeded by a coarse scan,
and returns a model that is trained and ready to forecast.

```swift
let shape: [Double] = [0, 8, 15, 19, 20, 18, 12, 4, -6, -14, -19, -20]
let fitMonths = (0..<48).map { Period.month(year: 2022 + $0 / 12, month: $0 % 12 + 1) }
let fitValues: [Double] = (0..<48).map { index in
    let trend: Double = 100.0 + 0.5 * Double(index)
    return trend + shape[index % 12]
}
let fitSeries = TimeSeries(periods: fitMonths, values: fitValues)

let fit = try fitSeries.fitETS(seasonality: .detect)

print("α=\(fit.alpha) β=\(fit.beta) γ=\(fit.gamma)")
print("cycle=\(fit.seasonLength) detected=\(fit.seasonalityWasDetected)")
print("in-sample SMAPE=\(fit.errors.smape)")
print("converged=\(fit.convergence.converged) after \(fit.convergence.evaluations) evaluations")

// The fitted model is already a Forecaster, so the evaluation tier accepts it directly.
let fittedForecast = try fit.model.trainedForecast(from: fitSeries, horizon: 12)
print(fittedForecast.valuesArray.count)
```

**The seasonality argument is a choice, not a switch.** ``ETSSeasonality`` has three cases:

- `.none` — non-seasonal. Level and trend only; the search covers two parameters, and gamma
  is reported as zero because with a cycle length of 1 it has nothing to smooth.
- `.detect` — the default. Delegates to ``TimeSeries/detectedSeasonLength(maxLag:)``, which is
  the strongest autocorrelation lag `h ≥ 2` that clears the white-noise band `1.96/√n`, and
  falls back to non-seasonal when nothing does.
- `.periods(n)` — an explicit cycle length. Detection never overrides it. `fit.seasonalityWasDetected`
  is how you tell the two apart afterwards.

Detection is available on its own, without fitting anything:

```swift
let acfMonths = (0..<36).map { Period.month(year: 2022 + $0 / 12, month: $0 % 12 + 1) }
let acfValues: [Double] = (0..<36).map { index in
    let swing: Double = (index % 12) < 6 ? 15.0 : -15.0
    return 100.0 + swing
}
let acfSeries = TimeSeries(periods: acfMonths, values: acfValues)
print(acfSeries.detectedSeasonLength() ?? 1)
```

`fit.errors` carries the in-sample accuracy of the fit — ``ETSFitErrors`` holds MAE, RMSE,
MAPE, SMAPE and an optional MASE. MASE is `nil` when the naive scale is degenerate, because
a constant series has no benchmark to be scaled against, and every value is a **ratio**
rather than a percentage.

Two caveats worth stating plainly. In-sample accuracy measures the fit, not the forecast; for
out-of-sample accuracy use ``TimeSeries/backtest(_:config:)`` on `fit.model`. And the search
is deterministic by construction — a coarse scan and a simplex from a fixed initial geometry
have no randomness in them — so the same series fits to the same parameters every time,
without seeding.

### Grid Search for Optimal Parameters

Kept for the case where you want to score candidate parameters against a **held-out** window
rather than against the in-sample residuals `fitETS` minimises.
...
Common seasonal patterns. For `Double` models `alpha`, `beta`, and `gamma`
default to 0.2, 0.1, and 0.1, so only `seasonalPeriods` is required:

```swift
let dailyWithWeekly = HoltWintersModel<Double>(seasonalPeriods: 7)      // Weekly pattern
let weeklyWithMonthly = HoltWintersModel<Double>(seasonalPeriods: 4)    // Monthly pattern
let monthlyWithAnnual = HoltWintersModel<Double>(seasonalPeriods: 12)   // Annual pattern
let quarterlyWithAnnual = HoltWintersModel<Double>(seasonalPeriods: 4)  // Annual pattern
```
...
try linear.fit(to: salesTimeSeries)
...
try exponential.fit(to: salesTimeSeries)
...
try logistic.fit(to: salesTimeSeries)
