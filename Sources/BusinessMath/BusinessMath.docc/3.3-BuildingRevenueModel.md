...
// `project(periods:)` returns an optional — it is nil when the model was fitted
// from raw values rather than a TimeSeries, so it has no period information.
guard let trendForecast = try linearModel.project(periods: forecastPeriods) else {
...
guard let testForecast = try testModel.project(periods: 2) else {
    fatalError("testModel was not fitted to a TimeSeries")
}
...
guard let conservativeForecast = try conservativeModel.project(periods: forecastPeriods) else {
	fatalError("conservativeModel was not fitted to a TimeSeries")
}
...
guard let optimisticForecast = try optimisticModel.project(periods: forecastPeriods) else {
	fatalError("optimisticModel was not fitted to a TimeSeries")
}
...
func buildRevenueModel() throws {
	...
	guard let trendForecast = try model.project(periods: forecastPeriods) else {
