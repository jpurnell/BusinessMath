...
A single-period summary needs one `OperationalMetrics` value; the multi-period
report later in this guide needs one for *every* period. Build the full set now,
so each quarter has metrics of its own:

```swift
let mrrByQuarter = [300_000.0, 330_000.0, 360_000.0, 345_000.0]
let customersByQuarter = [250.0, 268.0, 287.0, 281.0]

let quarterlyMetrics = periods.indices.map { index in
	OperationalMetrics<Double>(
		entity: entity,
		period: periods[index],
		metrics: [
			"monthly_recurring_revenue": mrrByQuarter[index],
			"customer_count": customersByQuarter[index],
			"average_revenue_per_user": mrrByQuarter[index] / customersByQuarter[index]
		]
	)
}
```
...
let summaries = try periods.indices.map { index in
	try FinancialPeriodSummary(
		...
		incomeStatement: incomeStatement,
		balanceSheet: balanceSheet,
		operationalMetrics: quarterlyMetrics[index]
	)
}
...
let metricsTimeSeries = try OperationalMetricsTimeSeries(
    metrics: quarterlyMetrics  // One OperationalMetrics per period
)
...
let revenuePerCustomer = periods.map { period in
    let mrr = mrrSeries![period]!
    let customers = customerSeries![period]!
...
for (index, quarter) in quarters.enumerated() {
    ...
    let quarterlyIncomeStatement = try IncomeStatement(
        entity: company,
        periods: [quarter],
        accounts: [revenue]  // ... plus expense accounts
    )
    incomeStatements.append(quarterlyIncomeStatement)

    // Create balance sheet (simplified) — every index you read later must be
    // appended here, or `balanceSheets[$0]` below traps on an empty array
    let cashAccount = try Account(
        entity: company,
        name: "Cash",
        balanceSheetRole: .cashAndEquivalents,
        timeSeries: TimeSeries(periods: [quarter], values: [500 * Double(index + 1)])
    )
    // Give the sheet a current liability: liquidity ratios divide by
    // current liabilities, and a sheet with none produces infinities
    let payablesAccount = try Account(
        entity: company,
        name: "Accounts Payable",
        balanceSheetRole: .accountsPayable,
        timeSeries: TimeSeries(periods: [quarter], values: [200 * Double(index + 1)])
    )
    let equityAccount = try Account(
        entity: company,
        name: "Equity",
        balanceSheetRole: .commonStock,
        timeSeries: TimeSeries(periods: [quarter], values: [300 * Double(index + 1)])
    )
    let quarterlyBalanceSheet = try BalanceSheet(
        entity: company,
        periods: [quarter],
        accounts: [cashAccount, payablesAccount, equityAccount]
    )
    balanceSheets.append(quarterlyBalanceSheet)
...
let shopCoSummaries = try quarters.indices.map {
    try FinancialPeriodSummary(
        entity: company,
        period: quarters[$0],
...
let shopCoReport = try MultiPeriodReport(entity: company, periodSummaries: shopCoSummaries)

// 6. Analyze results
print("Revenue Growth: \(shopCoReport.revenueGrowth())")
print("Margin Trend: \(shopCoReport.operatingMarginTrend())")
print("Leverage Trend: \(shopCoReport.debtToEBITDATrend())")

// 7. Export as JSON (Codable support)
let encoder = JSONEncoder()
let jsonData = try encoder.encode(shopCoReport)
```
...
```swift
// This throws MultiPeriodReportError.entityMismatch:
// the second summary belongs to ShopCo, not to Acme.
do {
    let mismatchedReport = try MultiPeriodReport(
        entity: entity,
        periodSummaries: [q1Summary, shopCoSummaries[0]]  // Error!
    )
    print("Unexpectedly accepted \(mismatchedReport.periodCount) summaries")
} catch let error as MultiPeriodReportError {
    print("Rejected: \(error)")  // Rejected: All period summaries must belong to the same entity
}
```
...
if let peRatio = q1Summary.peRatio {
...
if let inventoryTurnover = q1Summary.inventoryTurnoverRatio {
...
```swift
let derivedRevenuePerCustomer = q1Metrics.derived(
