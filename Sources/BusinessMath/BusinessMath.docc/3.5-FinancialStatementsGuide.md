...
let incomeStmt = try IncomeStatement(
    entity: acme,
    periods: periods,
    accounts: [revenueAccount, cogs, depreciation]  // Uses incomeStatementRole
)
...
// Use in BOTH statements
let workingCapitalBalanceSheet = try BalanceSheet(
    entity: acme,
    periods: periods,
    accounts: [cash, inventory]  // Uses balanceSheetRole
)

let workingCapitalCashFlowStmt = try CashFlowStatement(
    entity: acme,
    periods: periods,
...
let integratedDepreciation = try Account(
    entity: acme,
    name: "Depreciation",
...
let integratedInventory = try Account(
    entity: acme,
    name: "Inventory",
...
// Inventory has to be paid for out of something. Carrying it draws down cash by the
// same amount, so the balance sheet still balances once inventory is added.
let integratedCash = try Account(
    entity: acme,
    name: "Cash and Equivalents",
    balanceSheetRole: .cashAndEquivalents,
    timeSeries: TimeSeries(periods: periods, values: [400_000, 490_000, 630_000, 770_000])
)

// 2. Build all three statements from one master list of accounts.
//    Each initializer requires every account it is handed to carry that statement's
//    role, so filter the master list down before passing it in.
let allAccounts = [
    revenueAccount, cogs, salary, marketing, integratedDepreciation, interestExpense, incomeTax,
    integratedCash, receivables, integratedInventory, ppe,
    payables, longTermDebt,
    commonStock, retainedEarnings
]

let integratedIncomeStatement = try IncomeStatement(
    entity: acme,
    periods: periods,
    accounts: allAccounts.filter { $0.incomeStatementRole != nil }
)

let integratedBalanceSheet = try BalanceSheet(
    entity: acme,
    periods: periods,
    accounts: allAccounts.filter { $0.balanceSheetRole != nil }
)

// For cash flow, add net income account
let integratedNetIncomeAccount = try Account(
    entity: acme,
    name: "Net Income",
    cashFlowRole: .netIncome,
    timeSeries: integratedIncomeStatement.netIncome
)

let integratedCashFlowStatement = try CashFlowStatement(
    entity: acme,
    periods: periods,
    accounts: (allAccounts + [integratedNetIncomeAccount, capex, dividends])
        .filter { $0.cashFlowRole != nil }
)
...
let projection = CompanyProjection(
    entity: acme,
    periods: periods,
    incomeStatement: integratedIncomeStatement,
    balanceSheet: integratedBalanceSheet,
    cashFlowStatement: integratedCashFlowStatement
)
...
let baseProjection = try runner.run(
	...
	// 3. Sample drivers for each period. `Revenue` and `COGS Ratio` are probabilistic,
	//    so draw them through a seeded generator: the same seed replays the same
	//    projection, which is what makes a scenario reviewable rather than merely plausible.
	var generator = Xoshiro256StarStar(seed: 20_250_101)
	var revenueValues: [Double] = []
	var cogsValues: [Double] = []

	for (index, period) in periods.enumerated() {
		let baseRevenue = try drivers["Revenue"]!.sample(for: period, using: &generator)
		let growth = try drivers["Growth"]!.sample(for: period, using: &generator)
		let periodRevenue = baseRevenue * pow(1 + growth, Double(index))

		let cogsRatio = try drivers["COGS Ratio"]!.sample(for: period, using: &generator)

		revenueValues.append(periodRevenue)
		cogsValues.append(periodRevenue * cogsRatio)
	}
...
```swift
// Base Case: reuse `baseProjection` from the ScenarioRunner example above
...
// Calculate expected value (probability-weighted across all scenarios)
let expectedRevenue = scenarios.expectedValue { $0["revenue"] ?? 0 } ?? 0
