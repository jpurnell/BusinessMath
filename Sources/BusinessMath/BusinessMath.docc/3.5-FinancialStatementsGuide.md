...
let categoryMaxLength = (expensesByCategory.keys.map({$0.description.lengthOfBytes(using: .utf8)}).max() ?? 0)

for (category, accounts) in expensesByCategory {
	let total = accounts.reduce(0.0) { sum, account in
		sum + (account.timeSeries[q1] ?? 0.0)
	}
	print("\(category.paddingLeft(toLength: categoryMaxLength)): \(total.currency())")
}
