...
let intrinsicValue = try utilityStock.valuePerShare()
...
let techValue = try techStock.valuePerShare()
...
let emergingValue = try emergingStock.valuePerShare()
...
let totalEquityValue = try fcfeModel.equityValue()

// Value per share (100M shares outstanding)
let sharesOutstanding = 100.0
let fcfeSharePrice = try fcfeModel.valuePerShare(sharesOutstanding: sharesOutstanding)
...
let riEquityValue = try riModel.equityValue()
let riSharePrice = try riModel.valuePerShare(sharesOutstanding: 100.0)
...
for growth in growthRange {
	...
	print("  Growth Rate: \(growth.percent(1).paddingLeft(toLength: 5)) →  Value: \(try model.valuePerShare().currency(2))")
}
...
let reitValue = try reitValuation.valuePerShare()
...
- ``ResidualIncomeModel``
- ``enterpriseValueFromFCFF(freeCashFlowToFirm:wacc:terminalGrowthRate:)``
