...
> Important: every number above is exact and repeats on every run — `seed: 4108` fixes the
> draws, so the mean, standard deviation and interval are pinned figures you can check this
> example against. `Used GPU` is the one line that is hardware-dependent: it reports what the
> machine actually had, not what the code asked for.
>
> This example deliberately does **not** print its own elapsed time. It used to, and that one
> `Compute time:` line was the only thing in a 232-line article that could not be reproduced —
> a wall-clock reading no seed can stabilise, which cost the whole article its verifiability
> for the sake of a debug-build number the prose already told you not to trust. The figures
> worth quoting are the release-build ones below.
