...
let baseOpEx = DeterministicDriver(name: "Operating Expenses", value: 200_000)
...
let builder: ScenarioRunner.StatementBuilder = { drivers, periods in
 // Extract primitive driver values. A driver a scenario never set is a modelling
 // mistake — name it and throw, rather than trapping on `!` and losing which one
 // was missing. `StatementBuilder` is throwing precisely so this can surface.
 func value(of name: String) throws -> Double {
  guard let driver = drivers[name] else {
   throw ScenarioError.unknownInput(scenario: "statement builder", inputName: name)
  }
  return driver.sample(for: periods[0])
 }

 let revenue = try value(of: "Revenue")
 let cogsRate = try value(of: "COGS Rate")
 let opex = try value(of: "Operating Expenses")
...
//
// `runFinancialSimulation(scenario:entity:periods:iterations:builder:)` samples its
// drivers through `Driver.sample(for:)`, which owns its randomness — the function has
// no `seed:` parameter, so every run of it produces different figures. Where you need
// the numbers to reproduce (a board pack, a regression test, this page), drive the
// loop yourself: `ProbabilisticDriver` conforms to ``SeedableDriver``, so
// `sample(for:using:)` draws every value from a generator you seed once.
var simulationRNG = Xoshiro256StarStar(seed: 4213)
var projections: [FinancialProjection] = []
projections.reserveCapacity(10_000)

for _ in 0..<10_000 {
	let revenueDraw = try uncertainRevenue.sample(for: q1, using: &simulationRNG)
	let cogsRateDraw = try uncertainCOGSRate.sample(for: q1, using: &simulationRNG)

	// One iteration's drawn values, held fixed while the statements are built
	var drawnOverrides: [String: AnyDriver<Double>] = [:]
	drawnOverrides["Revenue"] = AnyDriver(DeterministicDriver(name: "Revenue", value: revenueDraw))
	drawnOverrides["COGS Rate"] = AnyDriver(DeterministicDriver(name: "COGS Rate", value: cogsRateDraw))
	drawnOverrides["Operating Expenses"] = AnyDriver(baseOpEx)

	projections.append(try runner.run(
		scenario: FinancialScenario(
			name: uncertainScenario.name,
			description: uncertainScenario.description,
			driverOverrides: drawnOverrides
		),
		entity: company,
		periods: quarters,
		builder: builder
...
let simulation = FinancialSimulation(projections: projections)
...
var gpuSim = MonteCarloSimulation(
    ...
    enableGPU: true,
    seed: 4207,  // reproducible; omit for a fresh draw
...
// Elapsed wall-clock time is deliberately not printed here. `Date().timeIntervalSince(_:)`
// cannot be seeded, so a timing line changes the output of this example on every run and
// on every machine. Time it on your own hardware when the speed is what you are testing;
// the figures below are about the result, not the clock.
...
**Expected output** (reproducible from `seed: 4207`):
```
=== GPU-Accelerated Monte Carlo (100,000 iterations) ===
Used GPU: true
...
After-Tax Net Income:
  Mean: $157,948
  StdDev: $53,313
  90% CI: [$73,513, $248,816]

Risk of Loss: 0.06%
Base case after-tax: $158,000
Monte Carlo mean: $157,948
```

The Monte Carlo mean lands within $52 of the base case's $158,000, which is what you
should expect: the three inputs are symmetric around their base values, and the tax
rule only bends the distribution in the small tail where net income turns negative —
6 runs in 10,000 here.

**Key advantages of expression models:**
- **Fast at high iteration counts**: 100K iterations in well under a second (vs 5-10 seconds on CPU); time it on your own hardware rather than trusting a number printed on this page
...
```swift
let uncertainRevenueComposed = ProbabilisticDriver<Double>.normal(
	name: "Revenue", mean: 1_000_000, stdDev: 100_000
)
let uncertainCOGSRateComposed = ProbabilisticDriver<Double>.normal(
	name: "COGS Rate", mean: 0.60, stdDev: 0.05
)
