...
```swift
let mathFunctionsModel = try MonteCarloExpressionModel { builder in
...
```swift
let conditionalModel = try MonteCarloExpressionModel { builder in
...
```swift
let minMaxModel = try MonteCarloExpressionModel { builder in
...
let callOption = try MonteCarloExpressionModel { builder in
...
var simulation = MonteCarloSimulation(
    iterations: 100_000,
    enableGPU: true,
    seed: 4401,  // reproducible; omit for a fresh draw
...
```swift
let portfolioModel = try MonteCarloExpressionModel { builder in
    // 5-asset portfolio
...
```swift
let compoundingModel = try MonteCarloExpressionModel { builder in
...
```swift
let portfolioVarianceModel = try MonteCarloExpressionModel { builder in
    let w1 = builder[0]
    let w2 = builder[1]
...
let goodGPUModel = try MonteCarloExpressionModel { builder in
    let units = builder[0]
    let price = builder[1]
...
var goodGPUSim = MonteCarloSimulation(
    iterations: 500_000,  // Large iteration count
    enableGPU: true,
    seed: 4402,
    expressionModel: goodGPUModel
)
...
let poorGPUModel = try MonteCarloExpressionModel { builder in
    builder[0] + builder[1]  // Just addition
}

var poorGPUSim = MonteCarloSimulation(
    iterations: 10_000,  // Small iteration count
    enableGPU: true,
    seed: 4403,
    expressionModel: poorGPUModel
)
...
let inlinedTaxModel = try MonteCarloExpressionModel { builder in
    let revenue = builder[0]
    let taxRate = builder[1]
...
let reusableTaxFunction = ExpressionFunction(inputs: 2) { builder in
    let income = builder[0]
    let rate = builder[1]
    return income * rate
}

let expressionTaxModel = try MonteCarloExpressionModel { builder in
    let revenue = builder[0]
    let taxRate = builder[1]
    let taxes = reusableTaxFunction.call(revenue, taxRate)  // ✓ GPU-compatible!
...
Task {
    do {
        ...
        // Warm-up
        var warmupSim = MonteCarloSimulation(iterations: 1000, enableGPU: true, seed: 4404, expressionModel: model)
        ...
        // Measure GPU
        let gpuStart = Date()
        // Same seed on both sides, so the two runs differ by execution path
        // alone rather than by which samples each happened to draw.
        var gpuSim = MonteCarloSimulation(iterations: 100_000, enableGPU: true, seed: 4405, expressionModel: model)
        ...
        // Measure CPU
        let cpuStart = Date()
        var cpuSim = MonteCarloSimulation(iterations: 100_000, enableGPU: false, seed: 4405, expressionModel: model)
...
```swift
let completeModel = try MonteCarloExpressionModel { builder in
...
var portfolioSim = MonteCarloSimulation(
    iterations: 100_000,
    enableGPU: true,
    seed: 4406,
...
for i in 0..<3 {
    portfolioSim.addInput(SimulationInput(
        name: "Weight\(i)",
        distribution: DistributionNormal(0.33, 0.05)  // ~33% each with variation
    ))
}

let portfolioResults = try portfolioSim.run()

print("Expected Final Wealth: \(portfolioResults.statistics.mean.currency(0))")
print("95% Confidence: \(portfolioResults.percentiles.p5.currency(0)) - \(portfolioResults.percentiles.p95.currency(0))")
print("Executed on: \(portfolioResults.usedGPU ? "GPU ⚡" : "CPU")")
...
```swift
let operationCountModel = try MonteCarloExpressionModel { builder in
...
var sim = MonteCarloSimulation(iterations: 100_000, seed: 4407) { inputs in
...
let gpuExpressionModel = try MonteCarloExpressionModel { builder in
...
var gpuSim = MonteCarloSimulation(
    ...
    enableGPU: true,
    seed: 4407,
    expressionModel: gpuExpressionModel
)
```
...
let expressionCallModel = try MonteCarloExpressionModel { builder in
...
let fixedArrayModel = try MonteCarloExpressionModel { builder in
