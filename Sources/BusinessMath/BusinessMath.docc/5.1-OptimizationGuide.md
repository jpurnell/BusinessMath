...
```swift
let goalSeekOptimizer = GoalSeekOptimizer<Double>(
	target: 0.0,
	tolerance: 0.01
)
...
let goalSeekResult = goalSeekOptimizer.optimize(
	objective: profit,
...
if goalSeekResult.converged {
	print("Breakeven with constraints: \(goalSeekResult.optimalValue.currency())")
}
```
...
let adamOptimizer = MultivariateGradientDescent<VectorN<Double>>(
	learningRate: 0.01,
	maxIterations: 10000
)

let result = try adamOptimizer.minimizeAdam(
	function: rosenbrock,
...
print("Solution: \(bfgsResult.solution.toArray())")  // ~[0, 0] — the minimum of x² + y²
...
print("Solution: \(constrainedResult.solution.toArray())")  // ~[0.5, 0.5]

// Shadow price (Lagrange multiplier)
if let lambda = constrainedResult.lagrangeMultipliers.first {
...
// Define a 3-asset portfolio
let inequalityExpectedReturns = VectorN([0.08, 0.12, 0.15])  // 8%, 12%, 15%

// Covariance matrix (annualized)
let inequalityCovarianceMatrix = [
    [0.0400, 0.0100, 0.0080],  // Asset 1: 20% volatility
    [0.0100, 0.0900, 0.0200],  // Asset 2: 30% volatility
    [0.0080, 0.0200, 0.1600]   // Asset 3: 40% volatility
]

// Portfolio variance function: w^T * Σ * w
let inequalityPortfolioVariance: @Sendable (VectorN<Double>) -> Double = { weights in
    var variance = 0.0
    for i in 0..<3 {
        for j in 0..<3 {
			variance += weights[i] * weights[j] * inequalityCovarianceMatrix[i][j]
        }
    }
    return variance
}

// Minimize risk subject to constraints
let inequalityPortfolioOptimizer = InequalityOptimizer<VectorN<Double>>()

let inequalityResult = try inequalityPortfolioOptimizer.minimize(
	inequalityPortfolioVariance,
	from: VectorN([0.4, 0.4, 0.2]),
	subjectTo: [
		// Target return: portfolio return ≥ 10%
		.inequality { w in
			let ret = w.dot(inequalityExpectedReturns)
...
print("Optimal weights: \(inequalityResult.solution.toArray())")
let optimalVariance = inequalityPortfolioVariance(inequalityResult.solution)
let optimalReturn = inequalityResult.solution.dot(inequalityExpectedReturns)
print("Portfolio risk (volatility): \(sqrt(optimalVariance).percent(2))")
print("Portfolio return: \(optimalReturn.percent(2))")
```
...
let projects = [
	...
	AllocationOption(
		id: "cloud_migration",
		name: "C",
		expectedValue: 700_000,
		resourceRequirements: ["budget": 550_000],
		strategicValue: 9.0
	)
]

let resourceOptimizer = ResourceAllocationOptimizer()

let resourceResult = try resourceOptimizer.optimize(
	options: projects,
	objective: .maximizeWeightedValue(strategicWeight: 0.3),
	constraints: [
		.totalBudget(1_000_000),
		.requiredOption(optionId: "cloud_migration"),
		.mutuallyExclusive(["proj_a", "proj_b"])
	]
)

print("Selected projects: \(resourceResult.selectedOptions.map { $0.name })")
print("Total value: \(resourceResult.totalValue.currency())")
```
...
let products = [
	...
	ManufacturedProduct(
		...
		name: "Basic",
		pricePerUnit: 150,
		variableCostPerUnit: 130,
		demand: .range(min: 300, max: 900),
		resourceRequirements: [
			"assembly_hours": 3.0,
			"testing_hours": 3.0
		]
...
// `productionQuantities` is a dictionary, so sort by product id before printing —
// otherwise the order changes on every run and the output is not reproducible.
print("Production quantities: \(productionPlan.productionQuantities.sorted(by: { $0.key < $1.key }).map({$0.key + ": " + $0.value.number(0)}))")
print("Profit: \(productionPlan.profit.currency(0))")
```
...
// `optimizedDrivers` is a dictionary; sort by driver name so the report reads the
// same way every time.
for (name, value) in driverResult.optimizedDrivers.sorted(by: { $0.key < $1.key }) {
	print("  \(name): \(value.number(2))")
}

print("\nTarget achievement:")
print("  All targets met: \(driverResult.feasible)")

// Show the achieved metrics
let driverOptimizedMetrics = driverResult.achievedMetrics
print("\nAchieved metrics:")
if let mrr = driverOptimizedMetrics["mrr"] {
	print("  MRR: \(mrr.currency(0))")
}
if let ratio = driverOptimizedMetrics["ltv_cac_ratio"] {
...
- Explore <doc:5.2-PortfolioOptimizationGuide> for detailed portfolio optimization techniques
- Learn <doc:4.2-ScenarioAnalysisGuide> for optimizing across multiple scenarios
- See <doc:2.3-RiskAnalyticsGuide> for comprehensive risk analysis
