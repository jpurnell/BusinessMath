...
let smallProblem: @Sendable (VectorN<Double>) -> Double = { v in
	let x = v[0], y = v[1]
	return (x - 1) * (x - 1) + (y - 2) * (y - 2)
}

let smallProblemOptimizer = selectOptimizer(problemSize: 2)
let smallProblemResult = try smallProblemOptimizer.minimize(smallProblem, from: VectorN([0.0, 0.0]))

print("Solution: [\(smallProblemResult.solution[0].rounded()), \(smallProblemResult.solution[1].rounded())]")  // Near [1, 2]
```
...
// Use the factory
let factoryObjective: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }
let factoryOptimizer = OptimizerFactory.create(strategy: .adaptive, dimension: 2)
let factoryResult = try factoryOptimizer.minimize(factoryObjective, from: VectorN([3.0, 4.0]))

print("Objective value: \(factoryResult.objectiveValue)")
```
...
let constrainedQuadratic: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }
...
let portfolioOptimizer: any MultivariateOptimizer<VectorN<Double>> = InequalityOptimizer(
	maxIterations: 100
)
...
for (i, weight) in portfolioResult.solution.toArray().enumerated() {
	print("  Asset \(i+1): \(weight.percent())")
}

let portfolioReturn = expectedReturns.dot(portfolioResult.solution)
let portfolioStdDev = sqrt(portfolioResult.objectiveValue)
...
let playgroundQuadratic: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }
let optimizer1: any MultivariateOptimizer<VectorN<Double>> = MultivariateGradientDescent(
	learningRate: 0.01,
	maxIterations: 1000
)
let result1 = try optimizer1.minimize(playgroundQuadratic, from: VectorN([5.0, 5.0]))
...
var playgroundAlgorithms: [(name: String, optimizer: any MultivariateOptimizer<VectorN<Double>>)] = [
	("Gradient Descent", MultivariateGradientDescent(learningRate: 0.001, maxIterations: 10000)),
	("Newton-Raphson", MultivariateNewtonRaphson(maxIterations: 100, tolerance: 1e-6)),
	("Adaptive", AdaptiveOptimizer(maxIterations: 1000))
]
for (name, optimizer) in playgroundAlgorithms {
		let result = try optimizer.minimize(playgroundQuadratic, from: VectorN([3.0, 4.0]))
