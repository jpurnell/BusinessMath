...
// Use the factory
let factoryObjective = { (v: VectorN<Double>) -> Double in v.dot(v) }
...
let constrainedQuadratic = { (v: VectorN<Double>) -> Double in v.dot(v) }
...
let playgroundQuadratic = { (v: VectorN<Double>) -> Double in v.dot(v) }
