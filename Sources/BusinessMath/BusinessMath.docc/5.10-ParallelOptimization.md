...
By running multiple optimization attempts from different random starting points simultaneously, dramatically increase your chances of finding the global optimum instead of getting stuck in a local minimum.
...
// Await the call. Wrapping it in a detached `Task { }` is a trap: nothing waits on that
// task, so a script or command-line program can reach the end of its top-level code and
// exit before the optimization has printed anything at all.
let quickStartResult = try await optimizer.optimize(
...
print("Best solution: \(quickStartResult.solution)")           // Near [1.0, 1.0]
print("Objective value: \(quickStartResult.objectiveValue)")   // Near 0.0
print("Success rate: \(quickStartResult.successRate)")         // most starts converge
print("Starts tried: \(quickStartResult.allResults.count)")    // 12
```

That's it! The optimizer:
1. Generated 12 random starting points in [-2, 2] × [-2, 2]
2. Ran 12 gradient descent optimizations **in parallel**
3. Automatically selected the best result
4. Tracked which attempts converged

> Important: **Every optimizer in this article passes `seed: 42`, and that is why its
> figures reproduce.** ``ParallelOptimizer`` draws its starting points from
> `Double.random(in: 0...1)`, so an unseeded run gives different solutions, success rates
> and "best starting point" on every invocation — sometimes by many orders of magnitude.
> `ParallelOptimizer.init(algorithm:numberOfStarts:maxIterations:tolerance:seed:)` takes a
> seed for exactly this reason; omit it and you get the system generator.
>
> The figures below are therefore one *reproducible* sample rather than a converged
> answer. Seeding fixes which starting points are drawn; it does not make any one draw
> representative, and a different seed will move every number on this page.
...
A simplified sketch of ``ParallelOptimizer``'s internals — `runOptimization` is a
private helper, so the sketch is not compilable on its own.
...
let rastriginOptimizer = ParallelOptimizer<VectorN<Double>>(
...
let narrowValleyRosenbrock: @Sendable (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
    return (1 - x) * (1 - x) + 100 * (y - x*x) * (y - x*x)
}
...
**Tuning Strategy:**
```swift
// The number of starts is set on the optimizer, not on optimize(...)
let tuningRegion = (lower: VectorN([-2.0, -2.0]), upper: VectorN([2.0, 2.0]))

// Start conservative
let conservativeOptimizer = ParallelOptimizer<VectorN<Double>>(
    algorithm: .gradientDescent(learningRate: 0.01),
    numberOfStarts: 10,
    maxIterations: 500,
    seed: 42
)
let conservativeResult = try await conservativeOptimizer.optimize(
    objective: rosenbrock,
    searchRegion: tuningRegion,
    constraints: []
)
print("Success rate: \(conservativeResult.successRate)")  // 1.0 here; on a harder problem, e.g. 0.4

// If success rate < 0.5, increase starts
let thoroughOptimizer = ParallelOptimizer<VectorN<Double>>(
    algorithm: .gradientDescent(learningRate: 0.01),
    numberOfStarts: 30,
    maxIterations: 500,
    seed: 42
)
let thoroughResult = try await thoroughOptimizer.optimize(
    objective: rosenbrock,
    searchRegion: tuningRegion,
    constraints: []
)
print("Success rate: \(thoroughResult.successRate)")  // also 1.0 — 10 starts were already enough
...
```swift
typealias Algorithm = ParallelOptimizer<VectorN<Double>>.Algorithm

// GRADIENT DESCENT: General purpose, reliable
let gradientDescentAlgorithm: Algorithm = .gradientDescent(learningRate: 0.01)
// Best for: Most problems
// Speed: Fast
// Robustness: High

// NEWTON-RAPHSON: Fast convergence when near optimum
let newtonRaphsonAlgorithm: Algorithm = .newtonRaphson
...
let initialGuess = VectorN(Array(repeating: 0.0, count: 10))
...
let conservativeMaxIterations = 1000

// BALANCED (default recommendation):
let balancedMaxIterations = 500

// AGGRESSIVE (when you need speed):
let aggressiveMaxIterations = 100
...
let portfolioOptimizer = ParallelOptimizer<VectorN<Double>>(
    ...
    numberOfStarts: 10,       // Many starts for robustness
    // For the constrained algorithms, `maxIterations` is the *outer* augmented-Lagrangian
    // count, and each outer step runs its own inner solve of up to 1,000 BFGS iterations.
    // A few dozen is the working range; the hundreds that suit gradient descent multiply
    // out to hundreds of millions of objective evaluations here.
    maxIterations: 15,
    tolerance: 1e-6,
    seed: 42
)

let portfolioResult = try await portfolioOptimizer.optimize(
...
print("Optimal allocation: \(portfolioResult.solution)")
print("Minimum risk: \(portfolioResult.objectiveValue)")
print("Success rate: \(portfolioResult.successRate)")
print("Converged starts: \(portfolioResult.allResults.filter { $0.converged }.count)/30")

// Analyze diversity of solutions found
let objectiveValues = portfolioResult.allResults.map { $0.value }
...
> Warning: `Minimum risk` frequently prints a *negative* number here, alongside an
> allocation such as `[-6.33, 6.35, 0.76, 0.22]`. Neither is a portfolio. The objective
> penalises only the sum of the weights, and `[-6.33, 6.35, 0.76, 0.22]` does sum to 1,
> so nothing in the problem statement keeps an individual weight inside `[0, 1]` — the
> `searchRegion` bounds the starting points, not the search. Worse, the correlation term
> `0.5 · w₀ · w₁ · cos(w₀ - w₁)` is unbounded below once the weights are allowed to grow,
> so the "minimum risk" the optimizer finds is a variance that has gone negative rather
> than a diversified allocation. To make this example mean what it says, add
> `-w[i] ≤ 0` and `w[i] - 1 ≤ 0` for each weight to `constraints`, and treat any negative
> risk as a failure rather than a result.
...
let hyperparameterOptimizer = ParallelOptimizer<VectorN<Double>>(
...
let hyperparameterResult = try await hyperparameterOptimizer.optimize(
...
print("Optimal learning rate: \(hyperparameterResult.solution[0])")
print("Optimal momentum: \(hyperparameterResult.solution[1])")
print("Expected validation loss: \(hyperparameterResult.objectiveValue)")
print("Search reliability: \(hyperparameterResult.successRate)")
```

> Warning: Read the reliability line before you read the answer. This example prints
> `Search reliability: 0.0` — *no* start converged, because `learningRate: 0.05` is far
> too large for a landscape whose interesting structure is the `0.1 · sin(50·lr)` term.
> The "optimal learning rate" it reports is wherever each run happened to stop, and it
> lands on a different one of the `sin` term's many troughs every time. A success rate of
> zero means the numbers below it are not an optimum; reduce the learning rate or raise
> `maxIterations` until it is non-zero.
...
let processOptimizer = ParallelOptimizer<VectorN<Double>>(
	algorithm: .inequality,
	// 8 starts and 12 outer iterations, not the 25 and 400 this example carried: each
	// outer step runs an inner BFGS solve of up to 1,000 iterations, so the two numbers
	// multiply. 25 × 400 was 23 seconds of this article's runtime on its own.
	numberOfStarts: 6,
	maxIterations: 8,
	seed: 42
)

let processResult = try await processOptimizer.optimize(
...
print("Optimal temperature: \(processResult.solution[0].number(2))°C")
print("Optimal pressure: \(processResult.solution[1].number(2)) bar")
print("Optimal catalyst ratio: \(processResult.solution[2].number(2))")
print("Maximum yield: \((-processResult.objectiveValue).number(1))")
print("Process robustness: \(processResult.successRate)")

// Analyze alternative optima found
let allYields = processResult.allResults.map { -$0.value }  // Convert back to positive
let sortedYields = allYields.sorted(by: >)
print("\nTop 5 yields found:")
for (i, yield_val) in sortedYields.prefix(5).enumerated() {
	print("  \(i+1). \(yield_val.number(1))")
}
```

> Warning: The yields this example prints — millions, against a `processYield` whose two
> pathways are capped at 80 and 60 — are an artefact, and the reason is worth
> understanding because it is a mistake that is easy to repeat. `searchRegion` decides
> only where the *starting points* are sampled; it does not confine the optimizer. The
> constraint list bounds temperature and pressure but says nothing about catalyst ratio,
> so the search is free to walk to catalyst ≈ -0.05 — a pole of
> `catalyst / (catalyst + 0.05)` — where the objective is unbounded below and "maximum
> yield" becomes whatever the divergence reached before the iteration limit. It also
> explains the printed `Optimal catalyst ratio: -0.10`, which is not a ratio anything can
> have. Adding `catalyst ≥ 0.01` and `catalyst ≤ 0.2` to `processConstraints` is the right
> instinct but not sufficient on its own: with those constraints the augmented-Lagrangian
> inner solve produces no finite candidate at all and `optimize` throws
> `failedToConverge`. A pole inside the feasible closure has to be removed from the
> *objective* — guard the denominator, or reparameterise — not fenced off with a
> constraint.
...
Excerpted from ``ParallelOptimizationResult``. Re-declaring it here would shadow
the type the library vends, so the excerpt is shown rather than compiled.
...
> Warning: The `range` branch above is not a reliable diagnosis, and this example is
> the proof. Run it twice against Rosenbrock and it prints
> `Objective range: 1.6e-15 → All starts converged to same region` one time and
> `Objective range: 9.4e+53 → problem has many local minima` the next. The second
> conclusion is false whichever run produces it: the 2-D Rosenbrock function has exactly
> one minimum, at (1, 1). A huge range means one or two starts *diverged* — gradient
> descent with a fixed learning rate overshoots the valley wall and the objective blows
> up — not that the landscape is multi-modal. Before reading a large range as
> multi-modality, check `converged` on the extreme results and discard the ones that
> never converged; a diverged run contributes an arbitrarily large number to `max()`.
...
2. **Learning rate too large (diverging)**
   ...
   let reducedLearningRateAlgorithm: ParallelOptimizer<VectorN<Double>>.Algorithm =
...
**Symptoms:**
```swift
let divergentObjectives = result.allResults.map { $0.value }
let divergentRange = divergentObjectives.max()! - divergentObjectives.min()!  // Large value (> 10)
```
...
let increasedStarts = 100  // Was 20

// 2. Narrow search region if you know approximate location
let narrowedSearchRegion = (lower: VectorN([0.0, 0.0]), upper: VectorN([2.0, 2.0]))
...
1. **Reduce number of starts**
   ```swift
   let reducedStarts = 10  // Was 50
   ```

2. **Reduce max iterations**
   ```swift
   let reducedMaxIterations = 200  // Was 1000
   ```

3. **Use faster algorithm**
   ```swift
   // Gradient descent is usually faster than Newton-Raphson
   let fasterAlgorithm: ParallelOptimizer<VectorN<Double>>.Algorithm =
...
**Solutions:**
```swift
// Your own (possibly unguarded) computation
@Sendable func yourFunction(_ x: Double, _ y: Double) -> Double {
    (x - 1) * (x - 1) + 100 * (y - x * x) * (y - x * x)
}
...
```swift
func scaleUpIfNeeded(
    objective: @escaping @Sendable (VectorN<Double>) -> Double,
    searchRegion: (lower: VectorN<Double>, upper: VectorN<Double>)
...
// Usage:
let distributionResult = try await optimizer.optimize(
    objective: rosenbrock,
    searchRegion: (
        lower: VectorN([-2.0, -2.0]),
        upper: VectorN([2.0, 2.0])
...
distributionResult.analyzeResults()
```
...
let verifyResult = try verifyOptimizer.minimize(
    function: rosenbrock,
    gradient: { v in try numericalGradient(rosenbrock, at: v) },
...
> Warning: Run this and it prints `✓ Result verified - high confidence in global optimum`
> underneath a parallel result of roughly `4e+43`. Both halves of the comparison are
> wrong, and they are wrong independently of each other.
>
> First, agreement here is close to tautological. The verification re-runs gradient
> descent from `parallelResult.bestStartingPoint` — the *same* starting point the parallel
> run used — so a deterministic optimizer on a deterministic objective retraces the same
> path and the difference is exactly `0.0`. That confirms the optimizer is repeatable, not
> that the answer is a minimum. A verification worth the name starts somewhere else: from
> `parallelResult.solution` with a stricter tolerance, or from a fresh point, and checks
> the gradient norm at the reported solution.
>
> Second, an objective of `4e+43` on Rosenbrock is a diverged run, not an optimum.
> `learningRate: 0.01` with `maxIterations: 500` overshoots the valley wall here; the
> Quick Start above uses `0.001` and 32,768 iterations and reaches ~`1e-12`. Guard the
> claim on the value as well as the difference — `verifyResult.value < 1e-6` — before
> printing anything with a checkmark in it.
...
func benchmarkObjective(
    ...
    // Reported as a bracket rather than a duration. A wall-clock reading differs on every
    // run and every machine, so a figure printed here would be one no reader could
    // reproduce — including the documentation runner, which runs each article twice and
    // compares. Print `timePerEval` yourself when you are sizing a real problem.
    switch estimatedTime {
    case ..<1.0:
        print("Objective is cheap: 10 starts × 500 iterations finishes inside a second.")
    case ..<60.0:
        print("Objective is moderate: 10 starts × 500 iterations runs in seconds.")
    default:
        print("Objective is expensive: 10 starts × 500 iterations runs for minutes — reduce starts, iterations, or the cost of the objective itself.")
    }
}

// Usage:
let myObjective: @Sendable (VectorN<Double>) -> Double = { v in
...
**Optimization Complete**: You now have comprehensive optimization capabilities:
