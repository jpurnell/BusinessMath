...
print("Success rate: \(quickStartResult.successRate)")         // ~0.98 (63 of 64 converged)
print("Starts tried: \(quickStartResult.allResults.count)")    // 64
```

That's it! The optimizer:
1. Generated 64 random starting points in [-2, 2] × [-2, 2]
2. Ran 64 gradient descent optimizations **in parallel**
3. Automatically selected the best result
4. Tracked which attempts converged

> Important: **The figures printed throughout this article are not reproducible.**
> ``ParallelOptimizer`` draws its starting points from the system random number
> generator — `generateStartingPoints(count:region:)` calls `Double.random(in: 0...1)` —
> and neither `ParallelOptimizer.init(algorithm:numberOfStarts:maxIterations:tolerance:)`
> nor ``ParallelOptimizer/optimize(objective:searchRegion:constraints:)`` accepts a
> `seed:`, a `using:` generator, or an explicit list of starting points. Every solution,
> success rate, objective range and "best starting point" below therefore changes on each
> run, sometimes by many orders of magnitude. Compare ``VectorN/random(in:dimension:using:)``,
> which does take a generator. Until ``ParallelOptimizer`` grows the same parameter, treat
> the numbers here as one sample of a distribution, not as values you should expect to
> reproduce.
