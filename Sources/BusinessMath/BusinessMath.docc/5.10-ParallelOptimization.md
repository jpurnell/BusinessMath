...
print("Success rate: \(conservativeResult.successRate)")  // e.g., 0.4 (40%)
...
print("Success rate: \(thoroughResult.successRate)")  // e.g., 0.7 (70%) - better!
...
Task {
    let result = try await portfolioOptimizer.optimize(
...
Task {
    let result = try await hyperparameterOptimizer.optimize(
...
Task {
	let result = try await processOptimizer.optimize(
	...
	print("Optimal temperature: \(result.solution[0].number(2))°C")
	print("Optimal pressure: \(result.solution[1].number(2)) bar")
	print("Optimal catalyst ratio: \(result.solution[2].number(2))")
	print("Maximum yield: \((-result.objectiveValue).number(1))")
