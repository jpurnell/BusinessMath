...
let (row, col, value) = (0, 2, 2.0)
let sparseFromTriplets = SparseMatrix(rows: 3, columns: 3, triplets: [(row, col, value)])

// Operations
let vector = [1.0, 1.0, 1.0]
let result = sparse.multiply(vector: vector)  // O(nnz)
...
let sparseMatrix = SparseMatrix(dense: [[4.0, 1.0], [1.0, 3.0]])
let rhsVector = [1.0, 2.0]
let optionalGuess: [Double]? = nil
...
let portfolioSolver = SparseSolver()
let weights = try portfolioSolver.solve(
    A: covariance,
    b: returns,
...
// Solve flow problem
let flowSolver = SparseSolver()
let flows = try flowSolver.solve(
	A: flowMatrix,
	b: demands,
...
print("Sparsity: \(flowMatrix.sparsity.percent())")
...
```swift
// 500-point discretization
let n = 500
var tridiagonalTriplets: [(Int, Int, Double)] = []

for i in 0..<n {
    tridiagonalTriplets.append((i, i, 2.0))      // Diagonal
    if i > 0 {
        tridiagonalTriplets.append((i, i-1, -1.0)) // Sub-diagonal
    }
    if i < n-1 {
        tridiagonalTriplets.append((i, i+1, -1.0)) // Super-diagonal
    }
}

let A = SparseMatrix(rows: n, columns: n, triplets: tridiagonalTriplets)
...
let heatSolver = SparseSolver()
let temperature = try heatSolver.solve(
    A: A,
    b: b_modified,
    method: .conjugateGradient,
    tolerance: 1e-10
...
print("Solved 500-point heat equation")
print("Temperature range: \(temperature.min()!) to \(temperature.max()!)")
```

Note the size. Storage here is genuinely O(nnz) and a million-point matrix would fit
comfortably — but *solving* is a different budget. A 1-D Laplacian has condition number
O(n²), and unpreconditioned CG needs O(n) iterations to reach a tight tolerance, each
costing O(nnz). The solve is therefore quadratic in `n`: measured on this implementation,
500 points takes about 0.6s, 1,000 about 2.6s and 2,000 about 10s. At `n = 10,000` — the
size this example used to be — it does not throw and it does not stop; it was still running
after fifteen minutes. Nothing is wrong, and that is the trap: the matrix builds instantly,
the memory figures in the table above all hold, and only the *solve* is quadratic. That is
the **No Preconditioning** limitation listed below, met in the one place a reader would
actually meet it, and it is why this example is 500 points rather than 10,000.
...
```swift
// ✓ Use for symmetric positive definite matrices
let cgSolver = SparseSolver()
let cgSolution = try cgSolver.solve(A: A, b: b_modified, method: .conjugateGradient)
```

**Biconjugate Gradient:**
```swift
// ✓ Use for general non-symmetric matrices
let bicgSolver = SparseSolver()
let bicgSolution = try bicgSolver.solve(A: A, b: b_modified, method: .biconjugateGradient)
```
...
**Tight Tolerance:**
```swift
let tightToleranceSolution = try solver.solve(
    A: A,
    b: b_modified,
    method: .conjugateGradient,
...
let previousSolution = [Double](repeating: 0.0, count: b_modified.count)
let warmStartSolution = try solver.solve(
    A: A,
    b: b_modified,
    method: .conjugateGradient,
    initialGuess: previousSolution  // Faster convergence
)
```
...
```swift
do {
    let x = try solver.solve(A: A, b: b_modified, method: .conjugateGradient)

    // Verify solution
    let Ax = A.multiply(vector: x)
    let residual = zip(Ax, b_modified).map { $0 - $1 }.map { $0 * $0 }.reduce(0, +)
