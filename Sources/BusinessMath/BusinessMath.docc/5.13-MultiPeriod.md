...
@Sendable func riskAversion(quarter: Int, totalQuarters: Int) -> Double {
...
for year in stride(from: 0, through: 20, by: 5) {
	let quarter = year * 4
	if quarter < glidePathResult.trajectory.count {
		let weights = glidePathResult.trajectory[quarter].toArray()
...
let maxViolation = convergenceResult.constraintViolations.map { abs($0) }.max() ?? 0.0
