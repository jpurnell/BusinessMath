...
```swift
let boxConstraints: [MultivariateConstraint<VectorN<Double>>] =
    [.budgetConstraint] + MultivariateConstraint<VectorN<Double>>.nonNegativity(dimension: 3)

let boxConvenienceResult = try RobustOptimizer<VectorN<Double>>.optimizeBox(
...
let nominalReturns_worstCasePortfolio = [0.10, 0.12, 0.08, 0.15]  // Tech, Healthcare, Utilities, Energy
let deviations_worstCasePortfolio = [0.03, 0.02, 0.01, 0.04]      // ±3%, ±2%, ±1%, ±4%

// Asset volatilities (for risk penalty)
let volatilities_worstCasePortfolio = [0.15, 0.10, 0.05, 0.20]
...
let covarianceMatrix_worstCasePortfolio = (0..<4).map { i in
	(0..<4).map { j in
...
Optimal weights:
  Asset 1:  2.5%  (Tech - high uncertainty, 15% volatility)
  Asset 2: 40.0%  (Healthcare - medium return, 10% volatility)
  Asset 3: 22.7%  (Utilities - low return, 5% volatility)
  Asset 4: 34.8%  (Energy - highest uncertainty, 20% volatility)

Performance metrics:
  Worst-case return: 8.60%
  Nominal return: 11.10%
  Downside protection: 2.49%

Worst-case scenario (all assets hit lower bounds):
  Asset 1 return: 7.00%  (10% - 3%)
  Asset 2 return: 10.00%  (12% - 2%)
  Asset 3 return: 7.00%  (8% - 1%)
  Asset 4 return: 11.00%  (15% - 4%)
```

**Key Insights:**
- **Utilities (2.5%):** Minimal allocation despite minimal uncertainty (±1%) and volatility (5%) because of lowest return (8%)
- **Healthcare (40%):** Dominates allocation due to balanced risk-return profile (12% return, ±2% uncertainty, 10% volatility)
- **Tech (2.5%):** Minimal allocation despite 10% nominal return due to higher uncertainty (±3%, 15% volatility)
- **Energy (35%):** Strong allocation becuase of highest return (15%) despite extreme uncertainty (±4%, 20% volatility)
...
2. **Concentration Limits Force Diversification:** Without the 40% limit, the optimizer might put everything in Utilities (lowest risk). The constraint forces it to spread across other assets.

3. **Worst-Case Variance Matters:** The optimizer considers variance in EVERY scenario, not just the nominal. High-volatility assets like Energy (20%) get penalized even when they have good worst-case returns (11%).

Without risk constraints, the optimizer would simply put 100% in Asset 4 (highest worst-case return of 11%). The risk penalty and concentration limits create realistic diversification.
**─────────────────────────────────────────────────**
...
let nominalExpectedReturn = nominalWeights.enumerated()
    .map { nominalReturns[$0] * $1 }
    .reduce(0, +)
...
// COMPARISON
print("=== COMPARISON ===")
let expectedReturnLoss = -nominalResult.objectiveValue - (-robustResult.nominalObjective)
...
var scenarioConstraints: [MultivariateConstraint<VectorN<Double>>] = [.budgetConstraint]
scenarioConstraints.append(contentsOf:
    MultivariateConstraint<VectorN<Double>>.nonNegativity(dimension: 4)
)

// Optimize for worst-case scenario
let scenarioResult = try RobustOptimizer<VectorN<Double>>.optimizeDiscrete(
    objective: { weights, returns in
...
let weights = scenarioResult.solution.toArray()
...
print("Worst-case return: \(String(format: "%.2f%%", -scenarioResult.worstCaseObjective * 100))")
```
...
let ellipsoidalOptimizer = RobustOptimizer<VectorN<Double>>(
...
var ellipsoidalConstraints: [MultivariateConstraint<VectorN<Double>>] = [.budgetConstraint]
ellipsoidalConstraints.append(contentsOf:
    MultivariateConstraint<VectorN<Double>>.nonNegativity(dimension: 3)
)

let ellipsoidalResult = try ellipsoidalOptimizer.optimize(
    objective: { weights, returns in
        -weights.dot(VectorN(returns))
    },
    nominalParameters: correlatedReturns,
    initialSolution: VectorN([0.33, 0.33, 0.34]),
    constraints: ellipsoidalConstraints,
    minimize: true
)
...
let ellipsoidalWeights = ellipsoidalResult.solution.toArray()
print("Optimal allocation:")
for (i, name) in ["Tech", "Finance", "Utilities"].enumerated() {
    print("  \(name): \(String(format: "%.1f%%", ellipsoidalWeights[i] * 100))")
}
print()

print("Expected return: \(String(format: "%.2f%%", -ellipsoidalResult.nominalObjective * 100))")
print("Worst-case return (2σ): \(String(format: "%.2f%%", -ellipsoidalResult.worstCaseObjective * 100))")
print()

// Compare with box uncertainty of same "size"
let stdDevs = (0..<historicalCovariance.count).map { sqrt(historicalCovariance[$0][$0]) }
...
print("  Ellipsoid worst-case: \(String(format: "%.2f%%", -ellipsoidalResult.worstCaseObjective * 100))")
print("  Box worst-case: \(String(format: "%.2f%%", -boxResult.worstCaseObjective * 100))")
let worstCaseDifference = -ellipsoidalResult.worstCaseObjective - (-boxResult.worstCaseObjective)
print("  Difference: \(String(format: "%.2f%%", worstCaseDifference * 100))")
```
...
let fastSamples = 30  // ~1-2 seconds

// Balanced (N=100)
let balancedSamples = 100  // ~5-10 seconds

// Accurate (N=500)
let accurateSamples = 500  // ~30-60 seconds
```
...
```swift
let validationResult = try ellipsoidalOptimizer.optimize(
    objective: { weights, returns in -weights.dot(VectorN(returns)) },
    nominalParameters: correlatedReturns,
    initialSolution: VectorN([0.33, 0.33, 0.34]),
    constraints: ellipsoidalConstraints,
...
for (i, param) in validationResult.worstCaseParameters.enumerated() {
    let nominalValue = correlatedReturns[i]
    let deviation = param - nominalValue
    print("  Param \(i): \(param) (nominal: \(nominalValue), deviation: \(deviation))")
}

// Verify they're in the uncertainty set
assert(uncertaintySet.contains(validationResult.worstCaseParameters))
```
...
```swift
let nominalParams = [0.10, 0.12, 0.08]
let worstCaseParams = [0.07, 0.08, 0.06]
let comparisonConstraints: [MultivariateConstraint<VectorN<Double>>] =
    [.budgetConstraint] + MultivariateConstraint<VectorN<Double>>.nonNegativity(dimension: 3)
...
func solveNominal() throws -> VectorN<Double> {
    ...
    return try optimizer.minimize(
        { weights in -weights.dot(VectorN(nominalParams)) },
        from: VectorN([0.33, 0.33, 0.34]),
        subjectTo: comparisonConstraints
    ).solution
}

func solveRobust() throws -> VectorN<Double> {
    try RobustOptimizer<VectorN<Double>>.optimizeBox(
        objective: { weights, returns in -weights.dot(VectorN(returns)) },
        nominal: nominalParams,
        deviations: [0.03, 0.04, 0.02],
        initialSolution: VectorN([0.33, 0.33, 0.34]),
...
// Solve both problems
let nominalSolution = try solveNominal()
let robustSolution = try solveRobust()
...
let tooFewSamples = 10

// Minimum recommended
let minimumRecommendedSamples = 50

// Good balance
let goodBalanceSamples = 100

// High accuracy (slower)
let highAccuracySamples = 500
```
...
```swift
@MainActor func checkConvergenceAndFeasibility() throws {
    let result = try ellipsoidalOptimizer.optimize(
        objective: { weights, returns in -weights.dot(VectorN(returns)) },
        nominalParameters: correlatedReturns,
        initialSolution: VectorN([0.33, 0.33, 0.34]),
        constraints: ellipsoidalConstraints,
...
**Example:**
```swift
let underSampledCount = 10  // NOT ENOUGH for 5+ dimensional uncertainty
```
...
let mismatchedOptimizer = RobustOptimizer<VectorN<Double>>(uncertaintySet: mismatchedSet)

// BUG: Different nominal used!
let mismatchedResult = try mismatchedOptimizer.optimize(
    objective: { weights, returns in -weights.dot(VectorN(returns)) },
...
```swift
// Start from nominal solution, then pass it as `initialSolution:`
let warmStartSolution = try solveNominal()
```
...
```swift
let stableSamples = 200  // More stable
```
