...
let nonNegativityConstraints = MultivariateConstraint<VectorN<Double>>.nonNegativity(dimension: 3)
```
...
let profitFunction: @Sendable (VectorN<Double>) -> Double = { x in
...
// - Minimum order: x₁ ≥ 10
// - Non-negativity: x ≥ 0
let productionConstraints: [MultivariateConstraint<VectorN<Double>>] = [
...
let productionInitial = VectorN([15.0, 20.0])

let productionOptimizer = InequalityOptimizer<VectorN<Double>>()
let productionResult = try productionOptimizer.maximize(
    profitFunction,
    from: productionInitial,
    subjectTo: productionConstraints
)

print("Optimal production plan:")
print("  Product 1: \(productionResult.solution[0].number(1)) units")
print("  Product 2: \(productionResult.solution[1].number(1)) units")
print("  Total profit: $\(productionResult.objectiveValue.number(2))")
```
...
let mixedConstraints: [MultivariateConstraint<VectorN<Double>>] = [
...
```swift
let configuredOptimizer = InequalityOptimizer<VectorN<Double>>(
...
let tooTightConstraints: [MultivariateConstraint<VectorN<Double>>] = [
    .budgetConstraint,                   // Σw = 1
