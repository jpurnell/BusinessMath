...
// This example runs at 800 — just below the 1,000 GPU threshold — because `seed`
// is honoured only on the CPU path today, and the numbers printed below are meant
// to reproduce. Raise it to 2,000 to engage the GPU, and read
// "Seeding and the GPU path" before you rely on a seed at that scale.
...
`seed` is honoured on the **CPU path only**. Once `populationSize` reaches 1,000 on a
Metal-capable machine, selection, crossover, and mutation move to the GPU, and the GPU
kernels take their per-individual random state from the system generator rather than from
the seeded stream. Two runs of the same seeded configuration at population 1,000 or above
therefore produce **different** answers:
...
for population in [999, 1000] {
    let probe = GeneticAlgorithm<VectorN<Double>>(
        config: GeneticAlgorithmConfig(populationSize: population, generations: 60, seed: 8675309),
        searchSpace: seedThresholdSearchSpace
    )
    let probeResult = try probe.minimize(seedThresholdObjective, from: VectorN([5.0, 5.0]))

    // Uncomment, then run this file twice and compare the two outputs:
    //   print("population \(population): \(probeResult.value)")
    // At 999 the two runs print the same number. At 1,000 they do not — which is
    // why the value is not published here as a figure.
    _ = probeResult
}
```
...
- **Regression tests and published figures** must stay below 1,000, which is why every
  example in this tutorial that prints numbers does.
- **GPU-scale runs are still correct** — they converge to the same neighbourhood — they are
  simply not bit-reproducible. Assert on tolerances there, never on exact values.
- ``DifferentialEvolution`` does *not* have this limitation: its GPU path derives its
  per-thread seeds from the seeded generator, so a seeded DE run at population 1,000
  reproduces exactly. See <doc:5.17-DifferentialEvolutionTutorial>.
