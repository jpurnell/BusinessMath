...
let optimizer = DifferentialEvolution<VectorN<Double>>(
    // `.default` with a seed. The preset leaves `seed` nil, which means a fresh
    // random stream — and different numbers — on every run.
    config: DifferentialEvolutionConfig(seed: 31415),
...
// Solution: [1.0000008477334252, 0.9999999588565133]
// Objective value: 3.023004479278227e-10
// Converged: true
```
...
let rand1Optimizer = DifferentialEvolution<VectorN<Double>>(
    config: config,
...
let rastriginResult = try! rand1Optimizer.minimize(rastrigin, from: VectorN([0.0, 0.0]))
// Prints 0.10256348878925614. The global minimum is 0 — rand/1 explores widely and
// gets into the right basin, but does not polish; see best/1 below for the contrast.
print("Rastrigin minimum: \(rastriginResult.value)")
```
...
let best1Config = DifferentialEvolutionConfig(
    populationSize: 50,
    generations: 100,
    mutationFactor: 0.8,
    crossoverRate: 0.9,
...
let best1Optimizer = DifferentialEvolution<VectorN<Double>>(
    config: best1Config,
    searchSpace: [(-10.0, 10.0), (-10.0, 10.0)]
)
...
let sphereResult = try! best1Optimizer.minimize(sphere, from: VectorN([5.0, 5.0]))
print("Sphere minimum: \(sphereResult.value)")  // Should be near 0
print("Iterations: \(sphereResult.iterations)")
```
...
let currentToBest1Config = DifferentialEvolutionConfig(
    populationSize: 60,
    generations: 150,
    mutationFactor: 0.7,
    crossoverRate: 0.95,
...
let currentToBest1Optimizer = DifferentialEvolution<VectorN<Double>>(
    config: currentToBest1Config,
    searchSpace: [(-5.0, 5.0), (-5.0, 5.0)]
)
...
let ackleyResult = try! currentToBest1Optimizer.minimize(ackley, from: VectorN([0.0, 0.0]))
print("Ackley minimum: \(ackleyResult.value)")  // Should be near 0
```
...
let highCROptimizer = DifferentialEvolution<VectorN<Double>>(
    config: highCR,
...
let parabolaResult = try! highCROptimizer.minimize(parabola, from: VectorN([0.0]))
print("Solution: x = \(parabolaResult.solution[0])")  // Should be near 3.0
```
...
let portfolioOptimizer = DifferentialEvolution<VectorN<Double>>(
...
let negativeSharpe: @Sendable (VectorN<Double>) -> Double = { weights in
...
let optimalWeights = portfolioResult.solution.toArray()
let weightSum = optimalWeights.reduce(0.0, +)
for (i, w) in optimalWeights.enumerated() {
    print("  Asset \(i+1): \((w/weightSum).percent(1))")
}
print("Sharpe ratio: \((-portfolioResult.value).number(3))")
```
...
let hyperparamOptimizer = DifferentialEvolution<VectorN<Double>>(
    config: DifferentialEvolutionConfig(
        populationSize: 40,
        generations: 50,
...
let validationError: @Sendable (VectorN<Double>) -> Double = { hyperparams in
...
let hyperparamResult = try! hyperparamOptimizer.minimize(validationError, from: VectorN([0.01, 0.5, 128.0]))

print("Optimal hyperparameters:")
print("  Learning rate: \(hyperparamResult.solution[0].number(4))")
print("  Dropout rate: \(hyperparamResult.solution[1].number(2))")
print("  Batch size: \(Int(round(hyperparamResult.solution[2])))")
...
let beamOptimizer = DifferentialEvolution<VectorN<Double>>(
    config: DifferentialEvolutionConfig(
        populationSize: 50,
        generations: 100,
        seed: 31415
...
let weight: @Sendable (VectorN<Double>) -> Double = { dimensions in
...
let beamResult = try! beamOptimizer.minimize(
    weight,
...
print("  Width: \(beamResult.solution[0].number(1)) mm")
print("  Height: \(beamResult.solution[1].number(1)) mm")
print("  Weight: \(beamResult.value.number(2)) kg")
```
...
// Or use the high-performance preset. `.highPerformance` leaves `seed` nil, so spell
// the same settings out with a seed whenever the printed result has to reproduce.
let fastOptimizer = DifferentialEvolution<VectorN<Double>>(
    config: DifferentialEvolutionConfig(
        populationSize: 1000,   // .highPerformance: 1000 population, 500 generations
        generations: 500,
        mutationFactor: 0.7,
        crossoverRate: 0.95,
        seed: 31415
...
let fastSphere: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }
let fastResult = try! fastOptimizer.minimize(fastSphere, from: VectorN([5.0, 5.0]))
print("GPU-accelerated result: \(fastResult.value)")
```
...
let gpuCheckOptimizer = DifferentialEvolution<VectorN<Double>>(
    config: DifferentialEvolutionConfig(
        populationSize: 1000,   // .highPerformance, with a seed added
        generations: 500,
        mutationFactor: 0.7,
        crossoverRate: 0.95,
        seed: 31415
...
let simpleResult = try! gpuCheckOptimizer.minimize(simple, from: VectorN([2.0]))
print("Result: \(simpleResult.value)")
```
...
let result1 = try! rand1.minimize(comparisonRosenbrock, from: VectorN([0.0, 0.0]))
...
let result2 = try! best1.minimize(comparisonRosenbrock, from: VectorN([0.0, 0.0]))
...
let result3 = try! ctb1.minimize(comparisonRosenbrock, from: VectorN([0.0, 0.0]))

print("Strategy Comparison on Rosenbrock:")
print("  rand/1: \(result1.value.number(6)) in \(result1.iterations) iterations")
print("  best/1: \(result2.value.number(6)) in \(result2.iterations) iterations")
print("  currentToBest1: \(result3.value.number(6)) in \(result3.iterations) iterations")
```
...
let benchmarkSearchSpace = Array(repeating: (-5.0, 5.0), count: 10)

let benchmarkSphere: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }
...
let benchmarkCPUOptimizer = DifferentialEvolution<VectorN<Double>>(
    config: cpuConfig,
    searchSpace: benchmarkSearchSpace
)

let cpuStart = Date()
let cpuResult = try! benchmarkCPUOptimizer.minimize(benchmarkSphere, from: VectorN(Array(repeating: 2.0, count: 10)))
...
let benchmarkGPUOptimizer = DifferentialEvolution<VectorN<Double>>(
    config: gpuConfig,
    searchSpace: benchmarkSearchSpace
)

let gpuStart = Date()
let gpuResult = try! benchmarkGPUOptimizer.minimize(benchmarkSphere, from: VectorN(Array(repeating: 2.0, count: 10)))
let gpuTime = Date().timeIntervalSince(gpuStart)

print("Performance Comparison (10D Sphere):")
print("CPU (100 pop): \(cpuResult.iterations) generations, result: \(cpuResult.value.number(6))")
print("GPU (1000 pop): \(gpuResult.iterations) generations, result: \(gpuResult.value.number(6))")

// Wall-clock elapsed time is the one thing here that cannot be made reproducible:
// it depends on the machine, its thermal state, and what else is running, so it is
// measured but not published as a figure. Print it on your own hardware:
//   print("CPU: \(cpuTime.number(3))s   GPU: \(gpuTime.number(3))s")
_ = (cpuTime, gpuTime)
```

**Read this comparison carefully — it is not a speedup measurement.**

The two configurations do not do the same work. At 1,000 individuals the second run
evaluates the objective ten times as often per generation as the first, so dividing one
elapsed time by the other gives a ratio of two different workloads, not an
acceleration factor. A true CPU-versus-GPU comparison would hold the population fixed and
switch only the execution path, and the public API has no way to do that: the GPU is
selected automatically at `populationSize >= 1000` and there is no override. Treat the
numbers you measure here as throughput on your own hardware, not as a speedup.

Both runs also stop at **generation 10 of 100** with the objective still near 9.68 — the
true minimum of a 10-D sphere is 0. Neither has found the optimum; both have stalled, and
the early-stopping rule reports `converged = true` for a search that stopped improving
rather than one that arrived. This is the premature convergence described under
[Troubleshooting](#problem-premature-convergence), and it is a good illustration of why
`converged` is not the same claim as "optimal".
...
let prematureConfig1 = DifferentialEvolutionConfig(
    mutationFactor: 0.5,  // Smaller steps
    crossoverRate: 0.7
)

// 2. Increase population diversity
let prematureConfig2 = DifferentialEvolutionConfig(
...
let goodConstraint = MultivariateConstraint<VectorN<Double>>.inequality { v in
	let x = v[0], y = v[1]
    return x + y - 10.0  // Correct: x + y ≤ 10 → x + y - 10 ≤ 0
}

// 2. Increase generations to allow convergence
let constraintConfig = DifferentialEvolutionConfig(
    populationSize: 100,
...
let seededConfig = DifferentialEvolutionConfig(
	populationSize: 50,
	generations: 100,
...
let seededResult1 = try! optimizer1.minimize({ v in v[0] * v[0] }, from: VectorN([5.0]))
let seededResult2 = try! optimizer2.minimize({ v in v[0] * v[0] }, from: VectorN([5.0]))

print("Result 1: \(seededResult1.value)")
print("Result 2: \(seededResult2.value)")
