...
> ```
> In an Xcode playground, wrap the examples below with the `PlaygroundSupport`
> import and the two `PlaygroundPage` calls shown above; elsewhere they run as written.

```swift
import BusinessMath

// Configure individual GAs for each island.
// Tutorial-scale settings: small enough to finish instantly, big enough to converge.
let gaConfig = GeneticAlgorithmConfig(
    populationSize: 200,
    generations: 200,
    seed: 42  // Fixed seed so this example prints the same numbers every run
)
...
// Solution: [0.9429229609668255, 0.8896684041246772]
// Objective value: 0.0032896762942741715
// (Rosenbrock's minimum is 0 at [1, 1] — the seeded run gets close, not exact.)
```
...
let ringGAConfig = GeneticAlgorithmConfig(
    populationSize: 60,
    generations: 50,
    seed: 42
)

let ringIslandConfig = IslandModelConfig(
    numberOfIslands: 4,
...
let ringOptimizer = IslandModel<VectorN<Double>>(
    gaConfig: ringGAConfig,
    islandConfig: ringIslandConfig,
...
let ringResult = try! ringOptimizer.minimize(rastrigin, from: VectorN([0.0, 0.0]))
print("Ring topology result: \(ringResult.value.number(3))")
```
...
let fullyConnectedGAConfig = GeneticAlgorithmConfig(
    populationSize: 60,
    generations: 50,
    seed: 42
)

let fullyConnectedIslandConfig = IslandModelConfig(
    numberOfIslands: 5,
...
let fullyConnectedOptimizer = IslandModel<VectorN<Double>>(
    gaConfig: fullyConnectedGAConfig,
    islandConfig: fullyConnectedIslandConfig,
    searchSpace: [(-10.0, 10.0), (-10.0, 10.0)]
)
...
let fullyConnectedResult = try! fullyConnectedOptimizer.minimize(sphere, from: VectorN([5.0, 5.0]))
print("Fully connected result: \(fullyConnectedResult.value.number(3))")
```
...
let randomGAConfig = GeneticAlgorithmConfig(
    populationSize: 60,
    generations: 50,
    seed: 42
)

let randomIslandConfig = IslandModelConfig(
    numberOfIslands: 6,
...
let randomOptimizer = IslandModel<VectorN<Double>>(
    gaConfig: randomGAConfig,
    islandConfig: randomIslandConfig,
    searchSpace: [(-5.0, 5.0), (-5.0, 5.0)]
)
...
let randomResult = try! randomOptimizer.minimize(ackley, from: VectorN([0.0, 0.0]))
print("Random topology result: \(randomResult.value)")
```
...
let islandCountGAConfig = GeneticAlgorithmConfig(
    populationSize: 50,
    generations: 40,
    seed: 42
)
...
let islandCountRosenbrock: @Sendable (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
    return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}

let opt1 = IslandModel<VectorN<Double>>(
    gaConfig: islandCountGAConfig,
    islandConfig: fewIslands,
    searchSpace: searchSpace
)
let result1 = try! opt1.minimize(islandCountRosenbrock, from: VectorN([0.0, 0.0]))
...
let result2 = try! opt2.minimize(islandCountRosenbrock, from: VectorN([0.0, 0.0]))
...
let migrationIntervalGAConfig = GeneticAlgorithmConfig(
    populationSize: 50,
    generations: 60,
    seed: 42
)
...
let migrationIntervalSearchSpace = [(-5.0, 5.0), (-5.0, 5.0)]
let migrationIntervalSphere: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }

let freqOpt = IslandModel<VectorN<Double>>(
    gaConfig: migrationIntervalGAConfig,
    islandConfig: frequentConfig,
    searchSpace: migrationIntervalSearchSpace
)
let freqResult = try! freqOpt.minimize(migrationIntervalSphere, from: VectorN([3.0, 3.0]))
...
let infreqResult = try! infreqOpt.minimize(migrationIntervalSphere, from: VectorN([3.0, 3.0]))
...
let migrationSizeGAConfig = GeneticAlgorithmConfig(
    populationSize: 100,
    generations: 50,
    seed: 42
)
...
let migrationSizeSearchSpace = [(-10.0, 10.0), (-10.0, 10.0)]
let migrationSizeRosenbrock: @Sendable (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
    return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}

let smallOpt = IslandModel<VectorN<Double>>(
    gaConfig: migrationSizeGAConfig,
    islandConfig: smallMigration,
    searchSpace: migrationSizeSearchSpace
)
let smallResult = try! smallOpt.minimize(migrationSizeRosenbrock, from: VectorN([0.0, 0.0]))
...
let largeResult = try! largeOpt.minimize(migrationSizeRosenbrock, from: VectorN([0.0, 0.0]))
...
let tspOptimizer = IslandModel<VectorN<Double>>(
...
let tourDistance: @Sendable (VectorN<Double>) -> Double = { weights in
...
let tspResult = try! tspOptimizer.minimize(tourDistance, from: VectorN(Array(repeating: 0.5, count: cities.count)))

print("Best tour distance: \(String(format: "%.2f", tspResult.value))")

// Decode tour
let weights = tspResult.solution.toArray()
...
let featureObjective_mlFS: @Sendable (VectorN<Double>) -> Double = { selection in
...
// 5 assets portfolio
let portfolioOptimizer = IslandModel<VectorN<Double>>(
	gaConfig: GeneticAlgorithmConfig(
		populationSize: 100,
		generations: 100,
		crossoverRate: 0.85,
...
let portfolioWeights = portfolioResult.solution.toArray()
let sum = portfolioWeights.reduce(0.0, +)
for (i, w) in portfolioWeights.enumerated() {
	print("  Asset \(i+1): \((w/sum).percent())")
}

// Calculate final metrics
let normalized = portfolioWeights.map { $0 / sum }
...
let structureWeight_truss: @Sendable (VectorN<Double>) -> Double = { areas in
...
The trigger for the GPU path is the **per-island population**, not the number of
islands or generations. So a tutorial can keep the 1000-individual population that
switches the GPU on while running only a handful of generations. That is what the
example below does: `GeneticAlgorithmConfig.highPerformance` is 1000 individuals over
500 generations, which is a research-scale budget and takes far too long for a
tutorial, so we spell out the same population with 30 generations instead.

```swift
import BusinessMath

// Each island with population ≥ 1000 uses GPU.
// Population is what selects the GPU path; generations only set how long we run.
let gpuConfig = GeneticAlgorithmConfig(
	populationSize: 1000,  // ≥ 1000 → GPU
	generations: 30,       // short run: highPerformance would use 500
	eliteCount: 10,
	tournamentSize: 5,
	seed: 42
)

let gpuIslandConfig = IslandModelConfig(
	numberOfIslands: 2,      // 2 islands is enough to show the pattern
	migrationInterval: 10,
	migrationSize: 5,
	topology: .fullyConnected
)

let gpuOptimizer = IslandModel<VectorN<Double>>(
	gaConfig: gpuConfig,     // Each island: 1000 individuals (GPU)
	islandConfig: gpuIslandConfig,
	searchSpace: Array(repeating: (-5.0, 5.0), count: 4)
)

// Total: 2 islands × 1000 = 2000 individuals
// Each island runs GPU-accelerated GA independently

let gpuSphere: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }
let gpuResult = try! gpuOptimizer.minimize(gpuSphere, from: VectorN(Array(repeating: 2.0, count: 4)))

// Note: on the GPU path the exact fitness differs from run to run even with a seed
// (see the reproducibility caveat below), so report convergence rather than digits.
print("GPU-accelerated island model converged below 1e-2: \(gpuResult.value < 1e-2)")
print("Generations run: \(gpuResult.iterations)")
```

In production you would use `.highPerformance` directly, or raise `generations` until
the fitness stops improving. Scaling up costs time linearly in
`numberOfIslands × populationSize × generations`.

> **Reproducibility caveat on the GPU path:** `seed` makes a run repeatable only while
> the GA stays on the CPU. Once `populationSize` reaches 1000 the Metal kernels take
> over, and their per-individual random seeds are drawn from the system generator
> rather than from `config.seed` — so two runs of the same seeded configuration
> produce slightly different fitnesses. The examples in this section therefore print
> whether the run converged rather than the evolved values. If you need bit-exact
> repeatability, keep `populationSize` below 1000 so the CPU path is used.
...
// Same 1000-individual population as `.highPerformance`, but 20 generations
// rather than 500 — this block is about detecting the GPU, not about converging.
let gpuStatusOptimizer = IslandModel<VectorN<Double>>(
    gaConfig: GeneticAlgorithmConfig(populationSize: 1000, generations: 20, seed: 42),
    islandConfig: IslandModelConfig(numberOfIslands: 2, migrationInterval: 10, migrationSize: 5),
    searchSpace: [(-5.0, 5.0)]
)

let simple: @Sendable (VectorN<Double>) -> Double = { v in v[0] * v[0] }
let gpuStatusResult = try! gpuStatusOptimizer.minimize(simple, from: VectorN([2.0]))
print("Converged below 1e-3: \(gpuStatusResult.value < 1e-3)")
```

### Optimal GPU Configuration

The shape of a GPU-friendly configuration is: large population per island, few
islands, infrequent migration. The example keeps that shape at tutorial scale — a
6-dimensional problem with 2 islands of 1000 for 25 generations. In production the
same code with `populationSize: 2000`, `numberOfIslands: 4` and a 20-dimensional
search space is the real target; it costs roughly fifty times as much time.
...
let gpuGA = GeneticAlgorithmConfig(
	populationSize: 1000,  // Large population per island (≥ 1000 → GPU)
	generations: 25,
	seed: 42
)

let gpuIslands = IslandModelConfig(
	numberOfIslands: 2,     // 2 islands = 2000 total individuals
	migrationInterval: 25,
	migrationSize: 10,      // 1% migration
	topology: .fullyConnected
)

let highDimOptimizer = IslandModel<VectorN<Double>>(
	gaConfig: gpuGA,
	islandConfig: gpuIslands,
	searchSpace: Array(repeating: (-10.0, 10.0), count: 6)  // 6D problem
)

let highdim: @Sendable (VectorN<Double>) -> Double = { v in v.dot(v) }
let highDimResult = try! highDimOptimizer.optimizeDetailed(objective: highdim)

// Again on the GPU path, so the winning island and its exact fitness vary per run.
print("High-dimensional optimization:")
print("  Converged below 1.0: \(highDimResult.bestFitness < 1.0)")
print("  Generations run: \(highDimResult.generations)")
print("  Islands: \(highDimResult.islandFitnesses.count)")
```
...
let analysisGAConfig = GeneticAlgorithmConfig(
	populationSize: 60,
	generations: 50,
	seed: 42
)

let analysisIslandConfig = IslandModelConfig(
	numberOfIslands: 5,
	migrationInterval: 10,
	migrationSize: 2,
	topology: .ring
...
let analysisOptimizer = IslandModel<VectorN<Double>>(
	gaConfig: analysisGAConfig,
	islandConfig: analysisIslandConfig,
	searchSpace: [(-5.0, 5.0), (-5.0, 5.0)]
)

let analysisRosenbrock: @Sendable (VectorN<Double>) -> Double = { v in
	let x = v[0], y = v[1]
	return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}

let analysisResult = try! analysisOptimizer.optimizeDetailed(objective: analysisRosenbrock)

print("Island Model Analysis:")
print("  Number of islands: \(analysisResult.islandFitnesses.count)")
print("  Best island: \(analysisResult.bestIslandIndex)")
print("  Best fitness: \(analysisResult.bestFitness.number(6))")
print("  Migrations performed: \(analysisResult.migrationCount)")
print("\nIndividual island fitnesses:")
for (i, fitness) in analysisResult.islandFitnesses.enumerated() {
	let marker = i == analysisResult.bestIslandIndex ? " ⭐" : ""
...
let comparisonSearchSpace = [(-5.0, 5.0), (-5.0, 5.0)]
let comparisonRosenbrock: @Sendable (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
    return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}
...
let singleGA = GeneticAlgorithm<VectorN<Double>>(
    ...
    searchSpace: comparisonSearchSpace
)

let singleResult = try! singleGA.minimize(comparisonRosenbrock, from: VectorN([0.0, 0.0]))
...
let islandResult = try! islandModel.minimize(comparisonRosenbrock, from: VectorN([0.0, 0.0]))
...
Both spend the same evaluation budget — 200 individuals × 50 generations — so the
interesting number is solution quality, not elapsed time. The island model splits that
budget across four independently-seeded populations, which is why it usually lands
lower on a function like Rosenbrock that punishes a single population for converging
early. Timings are deliberately not printed here: they vary with machine and load, and
the point of the comparison is the search behaviour rather than the clock.
...
let convergenceConfig1 = IslandModelConfig(
    numberOfIslands: 4,
...
let convergenceConfig2 = IslandModelConfig(
    numberOfIslands: 5,
    migrationInterval: 10,
    migrationSize: 3,
...
let convergenceGAConfig = GeneticAlgorithmConfig(
...
let costConfig1 = IslandModelConfig(
...
let costGPUConfig = GeneticAlgorithmConfig(
    populationSize: 1000,  // GPU-accelerated
    generations: 100
)

let costIslandConfig = IslandModelConfig(
    numberOfIslands: 4,
    migrationInterval: 20,
    migrationSize: 5,
    topology: .fullyConnected
...
// Keep populationSize below 1000: at 1000 and above the GA runs on the GPU, whose
// random seeds do not come from `seed`, and runs stop being bit-for-bit repeatable.
let reproducibleGAConfig = GeneticAlgorithmConfig(
    populationSize: 50,
    generations: 50,
    seed: 12345  // Fixed seed
)

let reproducibleIslandConfig = IslandModelConfig(
    numberOfIslands: 4,
    migrationInterval: 10,
    migrationSize: 2,
    topology: .ring
...
let reproducibleOptimizer = IslandModel<VectorN<Double>>(
    gaConfig: reproducibleGAConfig,
    islandConfig: reproducibleIslandConfig,
    searchSpace: [(-10.0, 10.0)]
)

// Results will now be reproducible
let reproducibleResult1 = try! reproducibleOptimizer.minimize({ v in v[0] * v[0] }, from: VectorN([5.0]))
let reproducibleResult2 = try! reproducibleOptimizer.minimize({ v in v[0] * v[0] }, from: VectorN([5.0]))

print("Result 1: \(reproducibleResult1.value)")
print("Result 2: \(reproducibleResult2.value)")
// Both identical, because populationSize (50) keeps this on the CPU path
```
