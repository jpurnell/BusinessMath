...
let islandCountRosenbrock = { (v: VectorN<Double>) -> Double in
    let x = v[0], y = v[1]
    return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}
...
let migrationIntervalSphere = { (v: VectorN<Double>) -> Double in v.dot(v) }
...
let migrationSizeRosenbrock = { (v: VectorN<Double>) -> Double in
    let x = v[0], y = v[1]
    return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}
...
let gpuSphere = { (v: VectorN<Double>) -> Double in v.dot(v) }
...
let analysisRosenbrock = { (v: VectorN<Double>) -> Double in
	let x = v[0], y = v[1]
	return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}
...
let comparisonRosenbrock = { (v: VectorN<Double>) -> Double in
    let x = v[0], y = v[1]
    return (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x)
}
