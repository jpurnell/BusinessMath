...
```swift
let efficientFrontier = try optimizer.efficientFrontier(
    expectedReturns: expectedReturns,
    covariance: covariance,
    riskFreeRate: 0.02,
    numberOfPoints: 20
...
for portfolio in efficientFrontier.portfolios {
	let vol = portfolio.volatility
	let ret = portfolio.expectedReturn
	let sharpe = portfolio.sharpeRatio

	print("\(vol.percent(1).paddingLeft(toLength: 10)) | \(ret.percent(2).paddingLeft(toLength: 8)) | \(sharpe.number(2).paddingLeft(toLength: 6))")
}
```
...
let minVol = efficientFrontier.minimumVariancePortfolio
print("\nMinimum Volatility:")
...
let maxSharpeF = efficientFrontier.maximumSharpePortfolio
print("\nMaximum Sharpe:")
...
let fourAssetExpectedReturns = [0.08, 0.10, 0.12, 0.15]  // 8%, 10%, 12%, 15%
...
let result = try constrainedOptimizer.minimize(
	...
	subjectTo: [
		...
		// Target return ≥ 12%
		.inequality { w in
			let ret = w.dot(VectorN(fourAssetExpectedReturns))
...
let optimalReturn = result.solution.dot(VectorN(fourAssetExpectedReturns))
...
let fiveAssetExpectedReturns = VectorN([0.10, 0.12, 0.11, 0.04, 0.09])

let fiveAssetCovariance = [
...
let fiveAssetOptimizer = PortfolioOptimizer()

// Conservative investor (minimum variance)
let conservative = try fiveAssetOptimizer.minimumVariancePortfolio(
	expectedReturns: fiveAssetExpectedReturns,
	covariance: fiveAssetCovariance,
