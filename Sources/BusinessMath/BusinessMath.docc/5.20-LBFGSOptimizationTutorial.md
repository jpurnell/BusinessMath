...
> }
> ```
> Examples throughout this tutorial show the `Task { ... }` wrapper. In an Xcode
> playground, add the `PlaygroundSupport` import and the two `PlaygroundPage` calls
> shown above around them; elsewhere the examples run as written.
...
```swift
let convergenceOptimizer = AsyncLBFGSOptimizer(
    memorySize: 10,
    tolerance: 1e-6,
    maxIterations: 1000,
