...
let restartOptimizer = AsyncConjugateGradientOptimizer(
    method: .polakRibiere,
    tolerance: 1e-6,
    maxIterations: 200,
...
let restartResult = try await restartOptimizer.optimize(
    objective: objective,
    constraints: [],
    initialGuess: 5.0,
    bounds: nil
...
```swift
let fixedIntervalOptimizer = AsyncConjugateGradientOptimizer(
    method: .polakRibiere,
    tolerance: 1e-6,
    maxIterations: 100,
...
let fixedIntervalResult = try await fixedIntervalOptimizer.optimizeWithProgress(
    objective: objective,
    constraints: [],
    initialGuess: 0.0,
    bounds: nil
...
```swift
let backoffOptimizer = AsyncConjugateGradientOptimizer(
    method: .daiYuan,
    tolerance: 1e-6,
    maxIterations: 1000,
...
```swift
let convergenceBasedOptimizer = AsyncConjugateGradientOptimizer(
    method: .hestenesStiefel,
    tolerance: 1e-6,
    maxIterations: 1000,
...
let earlyStoppingOptimizer = AsyncConjugateGradientOptimizer(
    method: .polakRibiere,
...
let earlyStoppingResult = try await earlyStoppingOptimizer.optimize(
    objective: objective,
    constraints: [],
    initialGuess: 0.0,
    bounds: nil
...
```swift
let streamingOptimizer = AsyncConjugateGradientOptimizer(
    method: .daiYuan,
    tolerance: 1e-6,
    maxIterations: 100,
...
let stream = streamingOptimizer.optimizeWithProgressStream(
    objective: objective,
    constraints: [],
    initialGuess: 10.0,
    bounds: nil
...
let rosenbrockOptimizer = AsyncConjugateGradientOptimizer(
...
let rosenbrockResult = try await rosenbrockOptimizer.optimize(
...
print("Found minimum at: \(rosenbrockResult.optimalValue)")  // ≈ 1.0
```
...
```swift
let comparisonObjective: @Sendable (Double) -> Double = { x in
    (x - 5.0) * (x - 5.0)
}
...
```swift
let highPrecisionOptimizer = AsyncConjugateGradientOptimizer(
    method: .daiYuan,
...
let highPrecisionResult = try await highPrecisionOptimizer.optimize(
...
print("High-precision result: \(highPrecisionResult.optimalValue)")  // Very close to 7.0
print("Function value: \(highPrecisionResult.objectiveValue)")  // ≈ 0.0 within 1e-12
```
