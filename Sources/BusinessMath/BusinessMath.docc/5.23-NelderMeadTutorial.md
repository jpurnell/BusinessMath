...
let rosenbrockOptimizer = NelderMead<VectorN<Double>>(
    config: NelderMeadConfig(
        tolerance: 1e-6,
...
let rosenbrockResult = try rosenbrockOptimizer.minimize(
    rosenbrock,
    from: VectorN([0.0, 0.0])
)

print("Found minimum at: \(rosenbrockResult.solution)")  // ≈ [1.0, 1.0]
print("Objective value: \(rosenbrockResult.value)")  // ≈ 0.0
```
...
let absoluteSum: @Sendable (VectorN<Double>) -> Double = { v in
    abs(v[0]) + abs(v[1])
}

let nonSmoothOptimizer = NelderMead<VectorN<Double>>(config: .default)

let nonSmoothResult = try nonSmoothOptimizer.minimize(
    absoluteSum,
    from: VectorN([3.0, 3.0])
)

// Successfully finds non-differentiable minimum
print("Solution: \(nonSmoothResult.solution)")  // ≈ [0.0, 0.0]
```
...
```swift
let highPrecisionOptimizer = NelderMead<VectorN<Double>>(
    config: NelderMeadConfig(
...
let highPrecisionResult = try highPrecisionOptimizer.minimize(
    quadratic,
    from: VectorN([0.0, 0.0])
)

print("High-precision result: \(highPrecisionResult.solution)")  // Very close to [7.0, 3.0]
```
...
```swift
let detailedOptimizer = NelderMead<VectorN<Double>>(config: .default)
...
print("Value: \(detailedResult.value)")
...
print("Convergence reason: \(detailedResult.convergenceReason)")
print("Final simplex size: \(detailedResult.finalSimplexSize)")
...
let constrainedOptimizer = NelderMead<VectorN<Double>>(config: .default)
...
let inequalityConstraint: MultivariateConstraint<VectorN<Double>> = .inequality(
    function: { v in v[0] + v[1] - 4.0 },  // g(x) ≤ 0
    gradient: { _ in VectorN([1.0, 1.0]) }
)

let inequalityOptimizer = NelderMead<VectorN<Double>>(config: .default)
...
print("Solution: \(inequalityResult.solution)")
```
...
```swift
// Simple 1D case
let objective1D: @Sendable (VectorN<Double>) -> Double = { v in
    (v[0] - 7.0) * (v[0] - 7.0)
}

let optimizer1D = NelderMead<VectorN<Double>>(config: .default)
let result1D = try optimizer1D.minimize(objective1D, from: VectorN([0.0]))

// Converges quickly in 1D
print("Solution: \(result1D.solution[0])")  // ≈ 7.0
```
...
let highDimOptimizer = NelderMead<VectorN<Double>>(
    config: NelderMeadConfig(
...
let highDimResult = try highDimOptimizer.minimize(highDimObjective, from: initialGuess)
```
...
```swift
let simplexSizeOptimizer = NelderMead<VectorN<Double>>(
    config: NelderMeadConfig(
...
let convergenceResult = highDimOptimizer.optimizeDetailed(
    objective: highDimObjective,
    initialGuess: initialGuess
)

print("Convergence reason: \(convergenceResult.convergenceReason)")
...
```swift
let evaluationResult = optimizer.optimizeDetailed(
    objective: sphere,
    initialGuess: VectorN([5.0, 5.0])
)
print("Iterations: \(evaluationResult.iterations)")
print("Evaluations: \(evaluationResult.evaluations)")
...
```swift
let monitoredResult = optimizer.optimizeDetailed(
    objective: sphere,
    initialGuess: VectorN([5.0, 5.0])
)

if let history = monitoredResult.convergenceHistory {
