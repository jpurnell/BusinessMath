...
Introducing several core enhancements that make BusinessMath more accessible and Swift-idiomatic.

---

## Overview

This introduces several core enhancements that make BusinessMath more accessible and Swift-idiomatic:
...
These enhancements lay the foundation for all subsequent optimization work.
...
| Uses: `goalSeek()` or        | Uses: `NewtonRaphsonOptimizer` |
| `GoalSeekOptimizer`          |                                |
...
// Output: Need to sell 2,000 units
```
...
print("Solution: x = \(solution.number(6).paddingLeft(toLength: 8))")
// Verify: exp(x) - 2x - 3 ≈ 0
...
let goalSeekResult = optimizer.optimize(
...
print("Solution: \(goalSeekResult.optimalValue)")
print("Converged: \(goalSeekResult.converged)")
print("Iterations: \(goalSeekResult.iterations)")
```
...
func constrainedProfitFunction(price: Double) -> Double {
	let quantity = 10000 - 1000 * price  // Demand function
	let revenue = price * quantity
	let fixedCosts = 2000.0
	let variableCost = 5.0
	...
	let constrainedGoalSeekOptimizer = GoalSeekOptimizer<Double>(target: 0.0)
	...
	let constrainedGoalSeekResult = constrainedGoalSeekOptimizer.optimize(
		objective: constrainedProfitFunction,
		constraints: [maxPriceConstraint],
		initialGuess: 1.0,
		bounds: (lower: 0.0, upper: 100.0)
	)

	if constrainedGoalSeekResult.converged {
		print("Breakeven price: \(constrainedGoalSeekResult.optimalValue.currency())")
	} else {
...
2. **Try multiple guesses:**
   ...
   for guess in guesses {
       if let result = try? goalSeek(function: profit, target: 0.0, guess: guess) {
...
3. **Start near expected solution:**
   ...
   let lastMonthBreakeven = 10.0
   let lastMonthGuess = lastMonthBreakeven
   ```

4. **Avoid problematic points:**
   ```swift
   // Don't start where derivative is zero
   let nonZeroDerivativeGuess = 1.0  // Not 0.0 for f(x) = x²
   ```
...
```swift
let swiftIdiomaticBounds: ClosedRange<Double> = 0.0...100.0
```
...
The shape of `BusinessMathError` as the library declares it (abridged — the real enum
carries many more cases):

<!-- docs:illustrative -->
```swift
enum BusinessMathError: Error {
...
let estimatedBreakevenPrice = 5.44  // Last quarter's breakeven
let educatedGuess = estimatedBreakevenPrice
```
...
let priceTolerance = 0.01  // Within 1 cent for prices
```

### 4. Use Bounds to Constrain Search

```swift
// The objective being solved, as a closure the optimizer can take
let f: @Sendable (Double) -> Double = { price in profit(price: price) }

// ❌ Bad: Unbounded search
let unboundedResult = optimizer.optimize(
    objective: f,
    constraints: [],
    initialGuess: 10,
    bounds: nil
...
let boundedResult = optimizer.optimize(
    objective: f,
    constraints: [],
    initialGuess: 10,
...
```swift
let target = 0.0
let verifiedResult = try goalSeek(function: f, target: target, guess: educatedGuess)

// Verify the solution
let actualValue = f(verifiedResult)
let verificationError = abs(actualValue - target)

assert(verificationError < priceTolerance, "Solution not accurate enough")
```
...
let goalSeekBreakevenPrice = try goalSeek(
    function: profit,
    target: 0.0,
    guess: 10.0
)
...
let optimalPrice = try NewtonRaphsonOptimizer<Double>().optimize(
    objective: { price in -profit(price: price) },  // Minimize negative profit
    constraints: [],
    initialGuess: 10.0,
    bounds: (0.0, 100.0)
).optimalValue
...
```swift
let customStepOptimizer = GoalSeekOptimizer<Double>(
    target: 0.0,
    tolerance: 0.0001,
    maxIterations: 1000,
...
```swift
let historyResult = customStepOptimizer.optimize(
    objective: f,
    constraints: [],
    initialGuess: 1.0,
    bounds: (lower: 0.0, upper: 100.0)
...
for (i, iteration) in historyResult.history.enumerated() {
    print("Iteration \(i):")
...
This module introduces fundamental enhancements that are the basis for much of our optimization capabilities. These enhancements provide:
...
**Next Steps:**
- **Phase 2**: Learn about <doc:5.4-VectorOperations> VectorSpace protocol for multivariate problems
- **Phase 3**: Explore <doc:5.5-MultivariateOptimization> multivariate optimization algorithms
- **Phase 4**: Add <doc:5.6-ConstrainedOptimization> constraints to optimization
- **Phase 5**: Apply <doc:5.7-BusinessOptimization> to business problems
