...
```swift
let vArithmetic = VectorN([1.0, 2.0, 3.0])
let wArithmetic = VectorN([4.0, 5.0, 6.0])

// Addition and subtraction
let sumArithmetic = vArithmetic + wArithmetic     // [5, 7, 9]
let diff = vArithmetic - wArithmetic              // [-3, -3, -3]

// Scalar multiplication
let scaledArithmetic = 3.0 * vArithmetic          // [3, 6, 9]
let divided = vArithmetic / 2.0                   // [0.5, 1.0, 1.5]

// Negation
let negated = -vArithmetic                        // [-1, -2, -3]
```

### Norms and Distances

```swift
let vNorms = VectorN([3.0, 4.0])

// Norms
print(vNorms.norm)                 // 5.0 (Euclidean: √(3² + 4²))
print(vNorms.squaredNorm)          // 25.0 (faster for comparisons)

// Distance metrics
let wNorms = VectorN([0.0, 0.0])
print(vNorms.distance(to: wNorms))              // 5.0 (Euclidean distance)
print(vNorms.manhattanDistance(to: wNorms))     // 7.0 (|3| + |4|)
print(vNorms.chebyshevDistance(to: wNorms))     // 4.0 (max(|3|, |4|))
```

### Dot Products and Angles

```swift
let vDotProduct = VectorN([1.0, 0.0, 0.0])
let wDotProduct = VectorN([0.0, 1.0, 0.0])

// Dot product
print(vDotProduct.dot(wDotProduct))               // 0.0 (perpendicular)

// Cosine similarity
print(vDotProduct.cosineSimilarity(with: wDotProduct))  // 0.0 (orthogonal)

// Angle between vectors
let angle = vDotProduct.angle(with: wDotProduct)  // π/2 radians (90 degrees)
```

### Projections

```swift
let vProjection = VectorN([3.0, 4.0])
let wProjection = VectorN([1.0, 0.0])

// Project v onto w
let projection = vProjection.projection(onto: wProjection)  // [3.0, 0.0]

// Rejection (component perpendicular to w)
let rejection = vProjection.rejection(from: wProjection)    // [0.0, 4.0]

// Verify: v = projection + rejection
assert(vProjection == projection + rejection)
```

### Normalization

```swift
let vNormalization = VectorN([3.0, 4.0])

// Normalize to unit length
let unit = vNormalization.normalized()     // [0.6, 0.8]
print(unit.norm)                           // 1.0

// Verify direction preserved
print(vNormalization.cosineSimilarity(with: unit))  // 1.0 (same direction)
```
...
// Zero vector
let v3Construction = VectorN<Double>.zero
...
```swift
var vManipulation = VectorN([1.0, 2.0, 3.0])

// Access elements
print(vManipulation[0])                   // 1.0
vManipulation[1] = 5.0                    // v = [1, 5, 3]

// Append/remove
let v2Manipulation = vManipulation.appending(4.0)     // [1, 5, 3, 4]
let v3Manipulation = v2Manipulation.removingLast()    // [1, 5, 3]

// Concatenate
let wManipulation = VectorN([6.0, 7.0])
let combined = vManipulation.concatenated(with: wManipulation)  // [1, 5, 3, 6, 7]
...
```swift
let vFunctional = VectorN([-2.0, -1.0, 0.0, 1.0, 2.0, 3.0])

// Map (element-wise transform)
let squared = vFunctional.map { $0 * $0 }  // [4, 1, 0, 1, 4, 9]

// Filter
let positive = vFunctional.filter { $0 > 0 }  // [1, 2, 3]

// Reduce
let sumFunctional = vFunctional.reduce(0.0, +)  // 3.0

// Zip with another vector
let wFunctional = VectorN([4.0, 5.0, 6.0, 7.0, 8.0, 9.0])
let productFunctional = vFunctional.zipWith(wFunctional, *)  // [-8.0, -5.0, 0.0, 7.0, 16.0, 27.0]
```

### Orthogonality and Parallelism

```swift
let vOrthogonality = VectorN([1.0, 0.0, 0.0])
let wOrthogonality = VectorN([0.0, 1.0, 0.0])
let u = VectorN([2.0, 0.0, 0.0])

// Check orthogonality
print(vOrthogonality.isOrthogonal(to: wOrthogonality))  // true
print(vOrthogonality.isOrthogonal(to: u))               // false

// Check parallelism
print(vOrthogonality.isParallel(to: u))                 // true (same direction)
print(vOrthogonality.isParallel(to: wOrthogonality))    // false
```
...
let budgetConstraint: MultivariateConstraint<VectorN<Double>> = .equality { weights in
    ...
    // For example: if weights = [0.3, 0.4, 0.3], sum = 1.0, result = 0 ✓
    // If weights = [0.2, 0.5, 0.2], sum = 0.9, result = -0.1 ✗
...
// We want: weight[0] ≥ 0 (can't invest negative amount)
// Convert to standard form (≤ 0): -weight[0] ≤ 0
...
// We want: weight[1] ≤ 50% (can't put more than half in one asset)
// Convert to standard form: weight[1] - 0.5 ≤ 0
...
// We want: weight[2] ≥ 10% (must invest at least 10% in bonds for safety)
// Convert to standard form: -weight[2] + 0.1 ≤ 0, or 0.1 - weight[2] ≤ 0
...
print("No short-selling check (≤ 0): \(-testWeights[0])")  // -0.3 ✓ (satisfied)
print("Max allocation check (≤ 0): \(testWeights[1] - 0.5)")  // -0.1 ✓ (satisfied)
print("Min bonds check (≤ 0): \(0.1 - testWeights[2])")  // -0.2 ✓ (satisfied)
```
...
let budgetConstraintWithGradient: MultivariateConstraint<VectorN<Double>> = .equality(
    function: { weights in
        // Constraint function: returns 0 when satisfied
        weights.sum - 1.0
    },
    gradient: { weights in
        // Gradient: ∇(sum(w) - 1) = [1, 1, 1, ...]
...
func createNoShortSellingConstraint(for assetIndex: Int, totalAssets: Int) -> MultivariateConstraint<VectorN<Double>> {
    .inequality(
        function: { weights in
            // Constraint: -weight[i] ≤ 0, which means weight[i] ≥ 0
            -weights[assetIndex]
        },
        gradient: { weights in
            // Gradient: ∇(-w[i]) = unit vector pointing in direction i
...
This is what lets one generic entry point serve both scalars and vectors:
...
// This computes: (0.3 × 10%) + (0.4 × 15%) + (0.3 × 12%)
...
for i in 0..<3 {
    ...
    print("  \(assetNames[i]): \(weights[i].percent(0)) → \(contribution.percent(2)) return")
}
```
...
print("\n🎯 Optimal warehouse location (weighted centroid):")
...
for (name, location) in customers.sorted(by: { $0.key < $1.key }) {
	...
	print("  \(name): \(distance.number(2)) km × \(volume.number(0)) parcels = \(weightedDist.number(1)) km·parcels")
}
...
print("\n💰 Improvement: \(improvement.number(1)) km·parcels/day (\(percentImprovement.percent(1)) reduction)")
...
for iteration in 0..<maxIterations {
	...
	currentLocation = currentLocation - iterativeLearningRate * gradient
...
print("Customer A ↔ Customer B: \(similarityAB.number(3))")  // 0.888 (somewhat similar)
print("Customer A ↔ Customer C: \(similarityAC.number(3))")  // 0.485 (less similar)
print("Customer B ↔ Customer C: \(similarityBC.number(3))")  // 0.516 (moderately similar)

// Interpretation
print("\n=== Recommendation ===")
if similarityAB > similarityAC {
	print("Customer A is most similar to Customer B")
	print("→ Recommend Customer A products that Customer B likes (books)")
} else {
	print("Customer A is most similar to Customer C")
	print("→ Recommend Customer A products that Customer C likes (sports, clothing)")
}
...
print("\nℹ️  Euclidean normalization makes the LENGTH = 1, not the sum")
...
print("\nℹ️  Sum normalization creates percentages/proportions that sum to 100%")
...
1. **Use fixed-dimension vectors when possible:**
   ...
   let fixedDimensionVector: Vector2D<Double> = Vector2D(x: 1.0, y: 2.0)

   // ❌ Slower for known 2D problems
   let dynamicDimensionVector: VectorN<Double> = VectorN([1.0, 2.0])
   ```

2. **Use squared norms for comparisons:**
   ```swift
   let displacement = Vector2D(x: 0.3, y: 0.4)

   // ❌ Slower (unnecessary sqrt)
   if displacement.norm < 1.0 { print("inside the unit circle") }

   // ✅ Faster (no sqrt)
   if displacement.squaredNorm < 1.0 { print("inside the unit circle") }
   ```

3. **Provide analytical gradients when possible:**
   ```swift
   // ❌ Slower (numerical gradient)
   let constraint: MultivariateConstraint<VectorN<Double>> = .equality { w in w.sum - 1.0 }

   // ✅ Faster (analytical gradient)
   let analyticalGradientConstraint: MultivariateConstraint<VectorN<Double>> = .equality(
...
let vectorIntegrationOptimizer: any MultivariateOptimizer<VectorN<Double>> = MultivariateGradientDescent(
	learningRate: 0.01,
	maxIterations: 1000,
	tolerance: 0.0001
)
...
let vectorIntegrationObjective: @Sendable (VectorN<Double>) -> Double = { x in
	x.squaredNorm  // Sum of squares
}

let vectorIntegrationResult = try vectorIntegrationOptimizer.minimize(vectorIntegrationObjective, from: VectorN([1.0, 2.0, 3.0]))

print("Solution: \(vectorIntegrationResult.solution.toArray())")  // Near [0, 0, 0]
print("Converged: \(vectorIntegrationResult.converged)")
print("Iterations: \(vectorIntegrationResult.iterations)")
```
...
for algoSwapOptimizer in algorithms {
	let result = try algoSwapOptimizer.minimize(rosenbrock, from: VectorN([0.0, 0.0]))
	print("\(type(of: algoSwapOptimizer)): \(result.solution.toArray().map({ $0.number(1) })) \(result.iterations) iterations")
}
```
...
let portfolioWeights = [0.3, 0.4, 0.3]
let variableDimensionWeights: VectorN<Double> = VectorN(portfolioWeights)

// ❌ Bad: Using VectorN for fixed 2D
let pointAsVectorN: VectorN<Double> = VectorN([1.0, 2.0])  // Slower!
```

### 2. Check Dimensions

```swift
let vDimensions = VectorN([1.0, 2.0, 3.0])
let wDimensions = VectorN([4.0, 5.0, 6.0])

// ❌ Bad: Assuming dimensions match
let result = vDimensions.dot(wDimensions)

// ✅ Good: Verify dimensions
guard vDimensions.dimension == wDimensions.dimension else {
    throw OptimizationError.dimensionMismatch(
        message: "Dot product requires equal dimensions, got \(vDimensions.dimension) and \(wDimensions.dimension)"
    )
}
let checkedResult = vDimensions.dot(wDimensions)
```
...
let validationPoint = VectorN([1.0, 2.0, 3.0])
let gradient = try numericalGradient({ (p: VectorN<Double>) in p.squaredNorm }, at: validationPoint)
guard gradient.isFinite else {
    throw OptimizationError.numericalInstability(
        message: "Gradient has non-finite components; the objective is ill-conditioned at this point"
    )
}
```
...
```swift
let vFunctionalOps = VectorN([1.0, 2.0, 3.0])

// ❌ Verbose: Manual loop
var manualResult = VectorN<Double>([])
for i in 0..<vFunctionalOps.dimension {
    manualResult = manualResult.appending(vFunctionalOps[i] * vFunctionalOps[i])
}

// ✅ Concise: Functional style
let functionalResult = vFunctionalOps.map { $0 * $0 }
```
