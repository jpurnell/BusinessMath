...
- `epsilon`: Finite difference step (default: 1e-6)
...
let gradientDescentFunction: @Sendable (VectorN<Double>) -> Double = { v in
    v[0]*v[0] + 4*v[1]*v[1]
}
...
var gradientDescentOptimizer = MultivariateGradientDescent<VectorN<Double>>(learningRate: 0.01)
	let gradientDescentResult = try gradientDescentOptimizer.minimize(
		...
		gradient: { x in try numericalGradient(gradientDescentFunction, at: x) },
...
<!-- docs:illustrative -->
```swift
struct MultivariateGradientDescent<V: VectorSpace> where V.Scalar: Real {
    init(
...
**Parameters:**
- `learningRate`: Step size (required). Larger = faster but less stable
- `maxIterations`: Maximum iterations before giving up (default: 1000)
- `tolerance`: Convergence threshold on the gradient norm (default: 1e-6)
- `momentum`: Momentum coefficient (default: 0, i.e. plain gradient descent)

**Example:**
```swift
// Minimize sum of squares: f(x₁,...,xₙ) = Σxᵢ²
let sumOfSquares: @Sendable (VectorN<Double>) -> Double = { v in
	v.map { $0 * $0 }.reduce(0, +)
}
...
<!-- docs:illustrative -->
```swift
struct GradientDescentOptimizer<T: Real & Sendable & Codable>: Optimizer {
    // Generic form: every numeric parameter is explicit
    init(
        learningRate: T,
        tolerance: T,
        maxIterations: Int = 1000,
        momentum: T,               // Momentum coefficient
        useNesterov: Bool = false, // Use Nesterov acceleration
        stepSize: T                // Step size for the numerical gradient
    )
}

extension GradientDescentOptimizer where T == Double {
    // Double-only convenience form used by the examples below
    init(
        learningRate: Double = 0.01,
        tolerance: Double = 0.0001,
        maxIterations: Int = 1000,
        momentum: Double = 0.797734375,
...
- `0.797734375`: Default value, and the ceiling — anything higher is clamped to it
- `0.5`–`0.79`: Typical working range for smooth landscapes
- `0.9`: Requested here for illustration; the initializer clamps it to `0.797734375`
...
let deepValley: @Sendable (Double) -> Double = { x in
    return x*x
}
...
<!-- docs:illustrative -->
```swift
struct AdaptiveOptimizer<V: VectorSpace> where V.Scalar == Double {
    init(
...
**Example:**
```swift
let adaptiveRosenbrock: @Sendable (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
...
<!-- docs:illustrative -->
```swift
struct MultivariateNewtonRaphson<V: VectorSpace> where V.Scalar: Real {
    init(
...
let convexQuadratic: @Sendable (VectorN<Double>) -> Double = { v in
...
let bfgsMethodRosenbrock: @Sendable (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
    return (1 - x) * (1 - x) + 100 * (y - x*x) * (y - x*x)
}

let bfgsMethodOptimizer = MultivariateNewtonRaphson<VectorN<Double>>(
    maxIterations: 500
)

let result = try bfgsMethodOptimizer.minimizeBFGS(
    function: bfgsMethodRosenbrock,
    gradient: { try numericalGradient(bfgsMethodRosenbrock, at: $0) },
...
struct PortfolioOptimizer {
    init(strategy: OptimizationStrategy = .automatic)
...
```swift
let minVarReturns = VectorN([0.08, 0.10, 0.12, 0.15])
let minVarCovariance = [
    [0.04, 0.01, 0.00, 0.02],
    [0.01, 0.09, 0.02, 0.03],
    [0.00, 0.02, 0.16, 0.04],
    [0.02, 0.03, 0.04, 0.25]
...
let minVarOptimizer = PortfolioOptimizer()

// Long-only minimum variance
let minVarResult = try minVarOptimizer.minimumVariancePortfolio(
	expectedReturns: minVarReturns,
	covariance: minVarCovariance,
...
```swift
let returnsMS = VectorN([0.08, 0.10, 0.12, 0.15])
let covarianceMS = [
    [0.04, 0.01, 0.00, 0.02],
    [0.01, 0.09, 0.02, 0.03],
    [0.00, 0.02, 0.16, 0.04],
...
let optimizerMS = PortfolioOptimizer()

// Risk-free rate = 2%
let resultMS = try optimizerMS.maximumSharpePortfolio(
	expectedReturns: returnsMS,
	covariance: covarianceMS,
	riskFreeRate: 0.02,
...
```swift
let frontierReturns = VectorN([0.08, 0.10, 0.12, 0.15])
let frontierCovariance = [
    [0.04, 0.01, 0.00, 0.02],
    [0.01, 0.09, 0.02, 0.03],
    [0.00, 0.02, 0.16, 0.04],
    [0.02, 0.03, 0.04, 0.25]
...
let frontierOptimizer = PortfolioOptimizer()
let frontier = try frontierOptimizer.efficientFrontier(
	expectedReturns: frontierReturns,
	covariance: frontierCovariance,
	numberOfPoints: 25
)
...
```swift
let returnsRiskParity = VectorN([0.08, 0.10, 0.12])
let covarianceRiskParity = [
	[0.04, 0.01, 0.02],
	[0.01, 0.09, 0.03],
	[0.02, 0.03, 0.16]
]

let optimizerRiskParity = PortfolioOptimizer()
let resultRiskParity = try optimizerRiskParity.riskParityPortfolio(
	expectedReturns: returnsRiskParity,
	covariance: covarianceRiskParity,
	constraintSet: .longOnly
)
...
```swift
let conservativeReturns = VectorN([0.08, 0.10, 0.12])
let conservativeCovariance = [
    [0.04, 0.01, 0.02],
    [0.01, 0.09, 0.03],
    [0.02, 0.03, 0.16]
]
let conservativeOptimizer = PortfolioOptimizer()

// Minimize variance with long-only constraints
let conservative = try conservativeOptimizer.minimumVariancePortfolio(
    expectedReturns: conservativeReturns,
    covariance: conservativeCovariance,
...
let aggressive = try conservativeOptimizer.maximumSharpePortfolio(
    expectedReturns: conservativeReturns,
    covariance: conservativeCovariance,
    riskFreeRate: 0.02,
...
let constrained = try conservativeOptimizer.maximumSharpePortfolio(
    expectedReturns: conservativeReturns,
    covariance: conservativeCovariance,
    riskFreeRate: 0.02,
...
let costFunction: @Sendable (VectorN<Double>) -> Double = { params in
    // Your cost calculation: fixed overhead plus a quadratic penalty per unit
    let units = params[0], staffing = params[1]
    let cost = 1_000.0 + (units - 40) * (units - 40) + 2 * (staffing - 12) * (staffing - 12)
    return cost
}

// Use gradient descent
let costOptimizer = MultivariateGradientDescent<VectorN<Double>>(learningRate: 0.05)
let costResult = try costOptimizer.minimize(
    function: costFunction,
    gradient: { try numericalGradient(costFunction, at: $0) },
    initialGuess: VectorN([1.0, 1.0])  // initial params
)
print("Lowest cost: \(costResult.objectiveValue.number(2))")
```
...
let data: [(x: Double, y: Double)] = [(1.0, 3.1), (2.0, 8.9), (3.0, 19.2), (4.0, 33.0)]

let leastSquares: @Sendable (VectorN<Double>) -> Double = { params in
...
let leastSquaresOptimizer = MultivariateNewtonRaphson<VectorN<Double>>()
let leastSquaresResult = try leastSquaresOptimizer.minimizeBFGS(
    function: leastSquares,
    gradient: { try numericalGradient(leastSquares, at: $0) },
    initialGuess: VectorN([1.0, 1.0, 1.0])
)

print("Best fit parameters: \(leastSquaresResult.solution)")
```
...
let simple = MultivariateGradientDescent<VectorN<Double>>(learningRate: 0.01)
...
let assetCount = 3
let portfolioGuess = VectorN(Array(repeating: 1.0/Double(assetCount), count: assetCount))
```

### 4. Monitor Convergence

```swift
let monitorOptimizer = MultivariateGradientDescent<VectorN<Double>>(learningRate: 0.05)
let monitorResult = try monitorOptimizer.minimize(
    function: gradientDescentFunction,
    gradient: { try numericalGradient(gradientDescentFunction, at: $0) },
    initialGuess: goodGuess
)

if !monitorResult.converged {
    print("Warning: Did not converge in \(monitorResult.iterations) iterations")
    print("Final value: \(monitorResult.objectiveValue)")
    print("Gradient magnitude: \(monitorResult.gradientNorm)")
}
```
...
```swift
do {
    let result = try monitorOptimizer.minimize(
        function: gradientDescentFunction,
        gradient: { try numericalGradient(gradientDescentFunction, at: $0) },
        initialGuess: goodGuess
    )
