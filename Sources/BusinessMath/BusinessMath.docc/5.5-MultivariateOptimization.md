...
let grad = numericalGradient(rosenbrock, at: VectorN([0.0, 0.0]))
...
let optimizer = MultivariateGradientDescent<VectorN<Double>>(learningRate: 0.01)
...
print("Minimum at: \(result.solution.toArray().map({ $0.rounded() }))")  // ≈ [0, 0]
	print("Value: \(result.objectiveValue.rounded())")          // ≈ 0
...
print("Solution: \(result.solution.toArray().map({ $0.rounded() }))")
...
print("Found minimum at: \(result.solution.toArray().map({ $0.rounded() }))")
...
print("Expected Return: \(result.expectedReturn.percent())")
print("Risk (Std Dev): \(result.volatility.percent())")
```
...
print("Risk: \(resultMS.volatility.percent())")
print()
print("Weights:")
...
for portfolio in frontier.portfolios {
	...
	print("\(risk.percent()) | \(ret.percent())")
}

// Access optimal portfolios
let minVar = frontier.minimumVariancePortfolio
let maxSharpe = frontier.maximumSharpePortfolio
print("\nMinimum Risk: \(minVar.volatility.percent())")
print("Maximum Sharpe: \(maxSharpe.sharpeRatio.number(3))")
```
