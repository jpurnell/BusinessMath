...
let gradientDescentFunction: (VectorN<Double>) -> Double = { v in
    v[0]*v[0] + 4*v[1]*v[1]
}
...
**Example:**
```swift
let adaptiveRosenbrock: (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
...
let bfgsMethodRosenbrock: (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
    return (1 - x) * (1 - x) + 100 * (y - x*x) * (y - x*x)
}
