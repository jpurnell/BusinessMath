# Phase 3: Multivariate Optimization - Complete Tutorial
...
Phase 3 extends BusinessMath from scalar optimization (single variable) to **multivariate optimization** (multiple variables). This enables solving real-world problems like portfolio optimization, cost minimization, and parameter fitting.
...
let grad = numericalGradient(of: rosenbrock, at: VectorN([0.0, 0.0]))
...
print("Minimum at: \(result.solution.map({ $0.rounded() })")  // ≈ [0, 0]
print("Value: \(result.value.rounded())")          // ≈ 0
...
print(result.solution.map({ $0.rounded() }))
// Result: ≈ [0.0, 0.0, 0.0]
```
...
print("Solution: \(result.solution.map({ $0.rounded() }))")
...
print("Found minimum at: \(result.solution)")
...
print("Weights: \(minVar.weights.toArray())")
print("Risk: \(minVar.volatility)")
print("Return: \(minVar.expectedReturn)")
...
print("Optimal weights: \(maxSharpe.weights.toArray())")
...
for (i, weight) in result.weights.toArray().enumerated() {
    if weight > 0.01 {
        print("  Asset \(i+1): \(Int(weight * 100))%")
    }
}
print("Expected Return: \(result.expectedReturn * 100)%")
print("Risk (Std Dev): \(result.volatility * 100)%")
```
...
print("Sharpe Ratio: \(String(format: "%.2f", result.sharpeRatio))")
print("Expected Return: \(result.expectedReturn * 100)%")
print("Risk: \(result.volatility * 100)%")
print()
print("Weights:")
for (i, weight) in result.weights.toArray().enumerated() {
    print("  Asset \(i+1): \(Int(weight * 100))%")
...
print("Weights:")
for (i, weight) in result.weights.toArray().enumerated() {
    print(String(format: "  Asset %d: %5.1f%%", i+1, weight * 100))
}
```
...
```swift
// No position > 30%, no position < 5%
let constrained = try optimizer.maximumSharpePortfolio(
    expectedReturns: returns,
    covariance: covariance,
    riskFreeRate: 0.02,
    constraintSet: .boxConstrained(min: 0.05, max: 0.30)
)
```
...
Phase 3 provides a complete multivariate optimization toolkit:
...
4. Proceed to Phase 4 for constrained optimization
