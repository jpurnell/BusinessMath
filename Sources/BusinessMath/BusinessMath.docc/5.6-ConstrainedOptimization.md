...
// Enahnced: Proper constraints during optimization
...
let multipliers = result.lagrangeMultipliers
...
print("Portfolio risk: \(sqrt(result.objectiveValue))")
...
print("Portfolio volatility: \(sqrt(result.objectiveValue).percent())")
```
...
let optimizer = PortfolioOptimizer(strategy: .automatic)
...
for (i, w) in result.weights.toArray().enumerated() {
	print("  Asset \(i+1): \(w.percent())")
}
print("Portfolio volatility: \(result.volatility.percent())")
