...
print("Total Value: \(capitalResult.totalValue.currency(0))")
...
for option in capitalResult.selectedOptions {
	...
	print("    NPV: \(option.expectedValue.currency(0))")
	print("    Budget: \((option.resourceRequirements["budget"] ?? 0).currency(0))")
}

print()
print("Resource Usage:")
for (resource, used) in capitalResult.totalResourcesUsed {
	print("  • \(resource): \(used.number(1))")
}
```
...
for option in departmentResult.selectedOptions {
	...
	print("    NPV: \(option.expectedValue.currency(0))")
	print("    Budget: \((option.resourceRequirements["budget"] ?? 0).currency(0))")
	print("    Funded: \(((option.resourceRequirements["budget"] ?? 0) * allocation ).currency(0))")
	print()
}
```
...
```swift
let revenuePlan = try manufacturingOptimizer.optimize(
    products: manufacturingProducts,
    resources: resources,
...
let utilizationPlan = try manufacturingOptimizer.optimize(
    products: manufacturingProducts,
    resources: resources,
    objective: .maximizeUtilization
)

// `resources` above is keyed by assembly/testing/materials, so ask for one of those.
print("Assembly utilization: \((utilizationPlan.resourceUtilization["assembly_hours"] ?? 0) * 100)%")
```
...
for (name, value) in revenueResult.optimizedDrivers {
...
let amountsAllocated = allocation.selectedOptions.map { option in
	...
	return "\(option.name):\t\((option.resourceRequirements["budget"]! * fraction).currency())"
}.joined(separator: "\n")
...
print("  Capital Allocated: \(allocation.totalValue.currency(0))")
