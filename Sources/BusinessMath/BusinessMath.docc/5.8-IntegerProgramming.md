...
for i in 0..<3 {
	if vars[i] > 0.5 {
		print("  Warehouse \(i): OPEN (cost: \(fixedCosts[i].currency(0)))")
		var totalShipped = 0.0
		for j in 0..<5 {
			let flowIdx = 3 + i * 5 + j
		...
		print("    Total: \(totalShipped.number(0))/\(warehouseCapacities[i].number(0)) capacity")
	}
}
print("\nTotal cost: \(result.objectiveValue.currency(0))")
```
