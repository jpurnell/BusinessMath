...
let tunedSolver = BranchAndBoundSolver<VectorN<Double>>(
    ...
    timeLimit: .seconds(300),              // Stop after 5 minutes
...
> **What this example does and does not show.** All three rules explore the same
> 32 nodes here, and that is not a defect: the LP relaxation of a knapsack puts at
> most **one** variable at a fractional value, so there is only ever one variable
> to branch on and every rule picks it. A branching rule can only differ where
> there is a choice — several fractional variables at once, which needs more than
> one constraint. The comparison below therefore demonstrates the mechanism and
> the cost of running it, not a reduction in tree size. An earlier version of this
> page claimed 243, 127 and 41 nodes for the three rules; those numbers could not
> have come from this problem.
...
// Test problem. Ten items rather than fifteen: this runs as documentation, so it
// has to finish quickly on a loaded machine, and the point it makes does not need
// a bigger instance.
let numVars = 10
let values_branchSel = (0..<numVars).map { Double($0 + 1) * 7.0 }
let weights_branchSel = (0..<numVars).map { Double($0 + 1) * 3.5 }
let capacity_branchSel = 86.625
...
func configureLargeProblemSolver() -> BranchAndCutSolver<VectorN<Double>> {
    return BranchAndCutSolver<VectorN<Double>>(
        ...
        enableMIRCuts: true,
        timeLimit: .seconds(1800),                  // 30 minutes
        relativeGapTolerance: 0.05,         // 5% gap acceptable
        nodeSelection: .bestBound,
        branchingRule: .pseudoCost
    )
...
func configureHeuristicSolver() -> BranchAndBoundSolver<VectorN<Double>> {
    return BranchAndBoundSolver<VectorN<Double>>(
        ...
        timeLimit: .seconds(10),                    // 10 seconds
