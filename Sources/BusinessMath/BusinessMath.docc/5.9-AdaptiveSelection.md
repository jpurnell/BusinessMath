...
The sketch below is pseudocode for the selection logic inside
``AdaptiveOptimizer``; it is not compilable Swift.
...
**Example:**
```swift
let inequalityOptimizer = AdaptiveOptimizer<VectorN<Double>>()

let inequalityResult = try inequalityOptimizer.optimize(
	objective: { v in
		let x = v[0], y = v[1]
		return x * x + y * y
	},
...
print("[\(inequalityResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(inequalityResult.algorithmUsed)  // "Inequality Optimizer"
print(inequalityResult.selectionReason)  // "Problem has inequality constraints"
```
...
**Example:**
```swift
let constrainedOptimizer = AdaptiveOptimizer<VectorN<Double>>()

let constrainedResult = try constrainedOptimizer.optimize(
...
print("[\(constrainedResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(constrainedResult.algorithmUsed)  // "Constrained Optimizer"
print(constrainedResult.selectionReason)  // "Problem has equality constraints"
```
...
**Example:**
```swift
let largeProblemOptimizer = AdaptiveOptimizer<VectorN<Double>>()

let dimension = 150
let initial = VectorN(Array(repeating: 0.5, count: dimension))

let largeProblemResult = try largeProblemOptimizer.optimize(
	objective: { x in x.toArray().reduce(0.0) { $0 + $1 * $1 } },
	initialGuess: initial
)

print("[\(largeProblemResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(largeProblemResult.algorithmUsed)  // "Gradient Descent"
print(largeProblemResult.selectionReason)  // "Large problem (150 variables)"
```
...
**Example:**
```swift
let optimizer_accuracy = AdaptiveOptimizer<VectorN<Double>>()

let result_accuracy = try optimizer_accuracy.optimize(
	objective: { v in
		let x = v[0], y = v[1]
		return x * x + y * y },
	initialGuess: VectorN([1.0, 1.0])
...
print("[\(result_accuracy.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(result_accuracy.algorithmUsed)  // "Newton-Raphson"
print(result_accuracy.selectionReason)  // "Accuracy preference with small problem"
```
...
**Example:**
```swift
let smallProblemOptimizer = AdaptiveOptimizer<VectorN<Double>>()
...
let smallProblemResult = try smallProblemOptimizer.optimize(
	objective: rosenbrock,
	initialGuess: VectorN([0.0, 0.0])
)

print("[\(smallProblemResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(smallProblemResult.algorithmUsed)  // "Newton-Raphson"
print(smallProblemResult.selectionReason)  // "Small problem (2 variables)"
```
...
**Example:**
```swift
let defaultOptimizer = AdaptiveOptimizer<VectorN<Double>>()

let defaultResult = try defaultOptimizer.optimize(
	objective: { x in
...
print("[\(defaultResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(defaultResult.algorithmUsed)  // "Gradient Descent"
print(defaultResult.selectionReason)  // "Unconstrained problem - using gradient descent"
```
...
```swift
let preferSpeedOptimizer = AdaptiveOptimizer<VectorN<Double>>(preferSpeed: true)

let preferSpeedResult = try preferSpeedOptimizer.optimize(
	objective: { v in
		let x = v[0], y = v[1]
		return x * x + y * y
	},
...
print("[\(preferSpeedResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(preferSpeedResult.algorithmUsed)  // "Gradient Descent"
// Faster algorithms prioritized, even for small problems
print(preferSpeedResult.selectionReason)  // "Unconstrained problem - using gradient descent"
```
...
```swift
let preferAccuracyOptimizer = AdaptiveOptimizer<VectorN<Double>>(preferAccuracy: true)

let preferAccuracyResult = try preferAccuracyOptimizer.optimize(
	objective: { v in
		let x = v[0], y = v[1]
		return x * x + y * y
	},
...
print("[\(preferAccuracyResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print(preferAccuracyResult.algorithmUsed)  // "Newton-Raphson"
							 // Most accurate algorithms chosen
print(preferAccuracyResult.selectionReason)  // "Accuracy preference with small problem - using full Newton-Raphson"
```
...
```swift
let toleranceOptimizer = AdaptiveOptimizer<VectorN<Double>>(
    maxIterations: 5000,   // More iterations (default: 1000)
	tolerance: 1e-8        // Very tight (default: 1e-6)
)

let complexFunction: @Sendable (VectorN<Double>) -> Double = { v in
    let x = v[0], y = v[1]
    return (x - 3) * (x - 3) + (y + 1) * (y + 1)
}
let complexInitialGuess = VectorN([0.0, 0.0])

let toleranceResult = try toleranceOptimizer.optimize(
    objective: complexFunction,
    initialGuess: complexInitialGuess
)

print(toleranceResult.objectiveValue)  // Very precise result
```
...
let portfolioOptimizer = AdaptiveOptimizer<VectorN<Double>>()
...
let portfolioResult = try portfolioOptimizer.optimize(
...
print("Algorithm: \(portfolioResult.algorithmUsed)")  // "Inequality Optimizer"
print("[\(portfolioResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print("Variance: \(portfolioResult.objectiveValue.percent(3))")

// Check constraints
let sum = portfolioResult.solution.toArray().reduce(0, +)
...
let productionOptimizer = AdaptiveOptimizer<VectorN<Double>>()

let productionResult = try productionOptimizer.optimize(
	objective: { x in
...
let product1 = productionResult.solution[0]
let product2 = productionResult.solution[1]
let profit = -productionResult.objectiveValue

print("Algorithm: \(productionResult.algorithmUsed)")  // "Inequality Optimizer"
...
let calibrationOptimizer = AdaptiveOptimizer<VectorN<Double>>(preferAccuracy: true)

let calibrationResult = try calibrationOptimizer.optimize(
	objective: { params in
...
print("Algorithm: \(calibrationResult.algorithmUsed)")  // "Newton-Raphson"
print("Reason: \(calibrationResult.selectionReason)")  // "Accuracy preference with small problem"
print("Parameters: [\(calibrationResult.solution.toArray().map({ $0.number(2) }).joined(separator: ", "))]")
print("Fit error: \(calibrationResult.objectiveValue.number(2))")
```
...
let largeScaleDimension = 200

// `maxIterations` is small here only so this article runs inside the documentation
// runner's budget. The example is about *which* algorithm the selector picks for a
// 200-variable problem and why, not about converging one — and a numerical gradient in
// 200 dimensions costs 400 objective calls per step, each of which loops 200 times.
let largeScaleOptimizer = AdaptiveOptimizer<VectorN<Double>>(maxIterations: 10)

let largeScaleResult = try largeScaleOptimizer.optimize(
    objective: { x in
        // Complex high-dimensional function
        let values = x.toArray()
        var runningSum = 0.0
        for i in 0..<largeScaleDimension {
            runningSum += values[i] * values[i]
            if i > 0 {
                runningSum += 0.1 * values[i-1] * values[i]
            }
        }
        return runningSum
    },
    initialGuess: VectorN(Array(repeating: 0.5, count: largeScaleDimension))
)

print("Algorithm: \(largeScaleResult.algorithmUsed)")  // "Gradient Descent"
print("Reason: \(largeScaleResult.selectionReason)")  // "Large problem (200 variables)"
print("Converged in \(largeScaleResult.iterations) iterations")
```
...
Excerpted from ``AdaptiveOptimizer``: `Result` is nested inside the generic
optimizer, so `V` is the optimizer's vector type. The excerpt cannot be
compiled on its own.
...
let x0 = VectorN([0.0, 0.0])

let convergenceResult = try optimizer.optimize(objective: f, initialGuess: x0)

if convergenceResult.converged {
    print("✅ Optimization succeeded!")
    print("Solution: \(convergenceResult.solution)")
    print("Objective: \(convergenceResult.objectiveValue)")
    print("Used \(convergenceResult.algorithmUsed) (\(convergenceResult.iterations) iterations)")
    print("Reason: \(convergenceResult.selectionReason)")

    if let violation = convergenceResult.constraintViolation {
...
```swift
if !convergenceResult.converged {
    print("⚠️ Did not converge after \(convergenceResult.iterations) iterations")
    print("Algorithm tried: \(convergenceResult.algorithmUsed)")
    print("Best objective found: \(convergenceResult.objectiveValue)")
...
1. **Increase Iterations**
   ```swift
   let moreIterationsOptimizer = AdaptiveOptimizer<VectorN<Double>>(maxIterations: 5000)
   ```

2. **Relax Tolerance**
   ```swift
   let relaxedToleranceOptimizer = AdaptiveOptimizer<VectorN<Double>>(tolerance: 1e-4)
   ```

3. **Better Initial Guess**
   ```swift
   // Instead of zeros:
   let zerosInitial = VectorN([0.0, 0.0])  // Bad

   // Use domain knowledge:
   let domainKnowledgeInitial = VectorN([1.0, 2.0])  // Better
   ```

4. **Override Algorithm (if needed)**
   ```swift
   // Force accuracy for difficult problems
   let forcedAccuracyOptimizer = AdaptiveOptimizer<VectorN<Double>>(preferAccuracy: true)
   ```
...
let slowConvergenceOptimizer = AdaptiveOptimizer<VectorN<Double>>(preferAccuracy: true)
```
...
```swift
let lowMemoryOptimizer = AdaptiveOptimizer<VectorN<Double>>(preferSpeed: true)
...
let autoOptimizer = AdaptiveOptimizer<VectorN<Double>>()
let autoResult = try autoOptimizer.optimize(objective: f, initialGuess: x0)

// ❌ BAD: Don't manually choose unless you have specific needs
let manualOptimizer = MultivariateGradientDescent<VectorN<Double>>(learningRate: 0.001)
```
...
```swift
let reasoningResult = try optimizer.optimize(objective: f, initialGuess: x0)

print("Used: \(reasoningResult.algorithmUsed)")
print("Why: \(reasoningResult.selectionReason)")
...
let objective: @Sendable (VectorN<Double>) -> Double = { w in
    w.toArray().reduce(0.0) { $0 + $1 * $1 }
}
let validationInitial = VectorN([0.5, 0.5])
let constraints: [MultivariateConstraint<VectorN<Double>>] = [
    .equality(function: { x in x.toArray().reduce(0, +) - 1.0 }, gradient: nil)
]

let validatedResult = try optimizer.optimize(
    objective: objective,
    initialGuess: validationInitial,
    constraints: constraints
)

// Always check:
assert(validatedResult.converged, "Optimization did not converge")

// Verify constraints for constrained problems
if let violation = validatedResult.constraintViolation {
    assert(violation < 1e-6, "Constraints violated")
}

// Verify solution makes business sense
assert(validatedResult.solution.toArray().allSatisfy { $0 >= 0 }, "Negative values unexpected")
```

### 6. Use Problem Analysis for Debugging

```swift
let analysisOptimizer = AdaptiveOptimizer<VectorN<Double>>()
let analysisInitial = VectorN([0.5, 0.5])

// Analyze before optimizing
let analysis = analysisOptimizer.analyzeProblem(
    initialGuess: analysisInitial,
    constraints: constraints,
    hasGradient: true
)
...
let analysisResult = try analysisOptimizer.optimize(
    objective: objective,
    initialGuess: analysisInitial,
    constraints: constraints
)
