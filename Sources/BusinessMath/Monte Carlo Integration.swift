...
public func integrate<T: Real>(_ f: (T) -> T, iterations n: Int = 10000, seed: Double? = Double.random(in: 0...1)) -> T where T: BinaryFloatingPoint { // stochastic:exempt
	let randomSeed = seed ?? Double.random(in: 0...1) // stochastic:exempt
	var m = T(randomSeed)
    for i in 0..<n {
        m += ((f(distributionUniform()) - m)) / T((i + 1)) // fp-safety:disable — i + 1 >= 1
    }
    return m
}
