...
public func boxMuller<T: Real>(mean: T = T(0), stdDev: T = T(1), _ u1Seed: Double = openUnitUniform(), _ u2Seed: Double = openUnitUniform()) -> T where T: BinaryFloatingPoint { // stochastic:exempt — the uniform arguments default to fresh draws; pass them explicitly for reproducibility
...
///
/// - Requires: Appropriate implementation of the function `boxMullerSeed` to generate standard normal values.
public func boxMuller<T: Real>(mean: T, variance: T, _ u1Seed: Double = openUnitUniform(), _ u2Seed: Double = openUnitUniform()) -> T where T: BinaryFloatingPoint { // stochastic:exempt — the uniform arguments default to fresh draws; pass them explicitly for reproducibility
	// Validate parameters - return NaN for invalid inputs
