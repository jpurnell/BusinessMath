...
private func computeICC1CI<T: Real>(
	...
	guard let fUpper = try? fQuantile(p: T(1) - halfAlpha, df1: df1, df2: df2), // silent: fQuantile may fail for extreme df values — return full range as fallback
		  let fLower = try? fQuantile(p: halfAlpha, df1: df1, df2: df2), // silent: fQuantile may fail for extreme df values — return full range as fallback
