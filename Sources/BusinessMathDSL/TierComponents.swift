...
public struct CapitalReturn {
    ...
    public init(_ amount: Double) {
        guard amount >= 0 else {
            preconditionFailure("Capital return cannot be negative: \(amount)")
        }
        self.amount = amount
    }
}
...
public struct PreferredReturn {
    ...
    public init(_ rate: Double, years: Int) {
        guard rate >= 0 else {
            preconditionFailure("Preferred return rate cannot be negative: \(rate)")
        }
        guard years > 0 else {
            preconditionFailure("Years must be positive: \(years)")
        }
        self.rate = rate
        self.years = years
    }
...
public struct CatchUp {
    ...
    public init(to percentage: Double) {
        guard percentage >= 0, percentage <= 1.0 else {
            preconditionFailure("Catch-up percentage must be between 0 and 1: \(percentage)")
        }
...
public struct ProRata {
    ...
    public init(_ participants: [(String, Double)]) {
        guard !participants.isEmpty else {
            preconditionFailure("ProRata must have at least one participant")
        }

        let total = participants.reduce(0.0) { $0 + $1.1 }
        guard abs(total - 1.0) < 0.001 else {
            preconditionFailure("ProRata percentages must sum to 1.0, got \(total)")
        }
