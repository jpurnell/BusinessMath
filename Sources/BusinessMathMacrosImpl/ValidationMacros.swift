...
public struct ValidatedMacro: MemberMacro {
    ...
    public static func expansion(
                ...
                case "Range":
                    ...
                    if let arguments = attr.arguments?.as(LabeledExprListSyntax.self),
                       let firstArg = arguments.first {
                        ...
                        validationChecks.append("""
                                // Annotated, because the attribute's literal arrives here as
                                // source text: `0...1` written against a `ClosedRange<Double>`
                                // parameter re-infers as `ClosedRange<Int>` once copied into
                                // the generated body, and then cannot contain a `Double`.
                                let range_\(propertyName): ClosedRange<Double> = \(rangeExpr)
                                if !range_\(propertyName).contains(Double(\(propertyName))) {
                                    throw MacroValidationError(
                                        property: "\(propertyName)",
        ...
        /// - Throws: MacroValidationError if any validation rule fails
