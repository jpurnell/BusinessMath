...
struct DistributionSeedingTests {
	...
	// Note: SeededRNG is now in TestSupport/SeededRNG.swift for reuse across all test files
	...
	func boxMullerSeedValidNormal() {
		...
		let normals = seeds.map { pair in
			boxMullerSeed(pair.0, pair.1) as (z1: Double, z2: Double)
		}
