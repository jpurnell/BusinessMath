...
struct ExponentialDistributionTests {
	...
	static func seedsForExponential(count: Int) -> [Double] {
		let rng = SeededRNG(seed: 22222)  // Unique seed for Exponential
		var seeds: [Double] = []
	...
	func exponentialMemorylessReiterated() {
			...
			// Should observe the mean should still be lambda^-1 (memoryless property)
			if !conditionedSamples.isEmpty {
					let conditionedMean = conditionedSamples.reduce(0, +) / Double(conditionedSamples.count)
	...
	@Test("Exponential distribution struct statistical properties - seeded")
	func exponentialStructStatistics() {
		let λ = 2.0
		let expectedMean = 1.0 / λ
		...
		let empiricalMean = samples.reduce(0, +) / Double(samples.count)
		#expect(abs(empiricalMean - expectedMean) < 0.05, "Mean should be close to 1/λ")
	}
	...
	@Test("Exponential distribution stress test", .timeLimit(testHangGuard))
