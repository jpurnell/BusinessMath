...
struct GeometricDistributionTests {
	...
	@Test("Geometric distribution struct statistical properties - seeded")
	func geometricStructStatistics() {
		let p = 0.4
		let expectedMean = 1.0 / p
		...
		let empiricalMean = samples.reduce(0, +) / Double(samples.count)
		#expect(abs(empiricalMean - expectedMean) < 0.2, "Mean should be close to 1/p")
	}
