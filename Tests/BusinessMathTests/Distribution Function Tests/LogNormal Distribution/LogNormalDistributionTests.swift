...
struct LogNormalDistributionTests {
	...
	static func seedsForLogNormal(count: Int) -> [(u1: Double, u2: Double)] {
		let rng = SeededRNG(seed: 33333)  // Unique seed for LogNormal
	...
	@Test("LogNormal distribution struct random() method - bounds check")
	...
	@Test("LogNormal distribution struct statistical properties - seeded")
	func logNormalStructStatistics() {
		let mean = 0.0
		let stdDev = 1.0
