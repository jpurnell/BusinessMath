...
struct LogisticDistributionTests {
	...
	static func seedsForLogistic(count: Int) -> [Double] {
		let rng = SeededRNG(seed: 66666)  // Unique seed for Logistic
		var seeds: [Double] = []
	...
	@Test("Logistic distribution struct random() method - bounds check")
	...
	@Test("Logistic distribution struct statistical properties - seeded")
	func logisticStructStatistics() {
		let mean = 100.0
		let stdDev = 15.0
