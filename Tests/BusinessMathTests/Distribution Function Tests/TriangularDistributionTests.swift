...
struct TriangularDistributionTests {
	...
	static func seedsForTriangular(count: Int) -> [Double] {
		let rng = SeededRNG(seed: 55555)  // Unique seed for Triangular
		var seeds: [Double] = []
	...
	@Test("Triangular distribution struct random() method - bounds check")
	...
	@Test("Triangular distribution struct statistical properties - seeded")
	func triangularStructStatistics() {
		let low = 5.0
		let high = 15.0
		let base = 10.0
		...
		let empiricalMean = samples.reduce(0, +) / Double(samples.count)
		#expect(abs(empiricalMean - expectedMean) < 0.2, "Mean should be close to (a+b+c)/3")
	}
	...
	func triangularApproachesUniform() {
		...
		// For symmetric triangular, inner quarters should have more samples
		#expect(q2 > q1, "Inner quarters should have more samples")
		#expect(q3 > q4, "Inner quarters should have more samples")
	}

	@Test("Triangular distribution invalid parameters return NaN")
	func triangularInvalidParameters() {
		// Test low >= high
		let invalidRangeResult = triangularDistribution(low: 10.0, high: 5.0, base: 7.0, 0.5)
		#expect(invalidRangeResult.isNaN, "low >= high should return NaN")

		// Test base < low
		let baseBelowRangeResult = triangularDistribution(low: 5.0, high: 10.0, base: 3.0, 0.5)
		#expect(baseBelowRangeResult.isNaN, "base < low should return NaN")

		// Test base > high
		let baseAboveRangeResult = triangularDistribution(low: 5.0, high: 10.0, base: 12.0, 0.5)
		#expect(baseAboveRangeResult.isNaN, "base > high should return NaN")

		// Test NaN parameters
		let nanLowResult = triangularDistribution(low: Double.nan, high: 10.0, base: 7.0, 0.5)
		#expect(nanLowResult.isNaN, "NaN low should return NaN")

		let nanHighResult = triangularDistribution(low: 5.0, high: Double.nan, base: 7.0, 0.5)
		#expect(nanHighResult.isNaN, "NaN high should return NaN")

		let nanBaseResult = triangularDistribution(low: 5.0, high: 10.0, base: Double.nan, 0.5)
		#expect(nanBaseResult.isNaN, "NaN base should return NaN")
	}
}
