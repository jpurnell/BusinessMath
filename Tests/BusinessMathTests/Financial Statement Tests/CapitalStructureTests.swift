...
struct CapitalStructureTests {
    ...
    func optimalCapitalStructure() throws {
        ...
        for debtRatio in stride(from: 0.0, through: 0.8, by: 0.1) {
            ...
            // Cost of equity increases with leverage (using quadratic to model increasing financial distress)
            // Higher quadratic term to model accelerating risk at higher leverage
            let costOfEquity = baseCostOfEquity + (debtRatio * 0.03) + (debtRatio * debtRatio * 0.15)

            // Cost of debt increases with leverage (using quadratic)
            let costOfDebt = baseCostOfDebt + (debtRatio * 0.01) + (debtRatio * debtRatio * 0.08)
    ...
    func afterTaxCostOfDebtTest() throws {
		...
		let afterTax = structure.afterTaxCostOfDebt
    ...
    func afterTaxCostOfDebtZeroTaxTest() throws {
		let structure = CapitalStructure(
			debtValue: 400_000.0,
			equityValue: 600_000.0,
			costOfDebt: 0.07,
    ...
    func equityComponentWACC() throws {
        ...
        let equityComponent = structure.equityRatio * structure.costOfEquity
    ...
    func debtComponentWACC() throws {
        ...
        let debtComponent = structure.debtRatio * structure.afterTaxCostOfDebt
