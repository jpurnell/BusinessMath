...
struct IncomeStatementAggregationTests {
	...
	func testEmptyAccountsArray() throws {
		...
		#expect(abs(try #require(incomeStmt.totalRevenue[testPeriods[0]]) - 0.0) < 1e-6)
	}
	...
	func testMultipleRevenueAggregation() throws {
		...
		#expect(abs(try #require(incomeStmt.totalRevenue[testPeriods[0]]) - 175.0) < 1e-6)  // 100 + 50 + 25
		#expect(abs(try #require(incomeStmt.totalRevenue[testPeriods[1]]) - 195.0) < 1e-6)  // 110 + 55 + 30
		#expect(abs(try #require(incomeStmt.totalRevenue[testPeriods[2]]) - 215.0) < 1e-6)  // 120 + 60 + 35
		#expect(abs(try #require(incomeStmt.totalRevenue[testPeriods[3]]) - 235.0) < 1e-6)  // 130 + 65 + 40
	}
	...
	func testMultipleExpenseAggregation() throws {
		...
		let q1OpEx = try #require(incomeStmt.operatingExpenses[testPeriods[0]])
	...
	func testGrossProfitComputation() throws {
		...
		#expect(abs(try #require(incomeStmt.grossProfit[testPeriods[0]]) - 600.0) < 1e-6)  // 1000 - 400
		#expect(abs(try #require(incomeStmt.grossProfit[testPeriods[1]]) - 660.0) < 1e-6)  // 1100 - 440
	}
	...
	func testOperatingIncomeComputation() throws {
		...
		// Q1: 1000 - 300 - 200 = 500
		#expect(abs(try #require(incomeStmt.operatingIncome[testPeriods[0]]) - 500.0) < 1e-6)
	}
	...
	func testEBITDAComputation() throws {
		...
		// EBITDA = 600 + 100 = 700
		#expect(abs(try #require(incomeStmt.ebitda[testPeriods[0]]) - 700.0) < 1e-6)
	}
	...
	func testNetIncomeComputation() throws {
		...
		#expect(abs(try #require(incomeStmt.netIncome[testPeriods[0]]) - 600.0) < 1e-6)
	}
