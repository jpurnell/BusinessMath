...
struct GeneticAlgorithmTests {
    ...
    func testSphereFunctionHighDim() throws {
        ...
        // For 10D sphere with objective < 1.0: each |x_i| < sqrt(1.0/10) ≈ 0.316
        for i in 0..<dimension {
            #expect(abs(result.solution[i]) < 0.35)
        }
    }
    ...
    func testGPUVsCPUConsistency() throws {
        ...
        #expect(gpuResult.value <= cpuResult.value + 1e-6)
        #endif
    }
    ...
    func benchmarkGPUEfficiency() throws {
        ...
        // Large GPU population (2000 individuals × 20 generations × 5D)
        ...
        // Verify solution quality (time limit enforced by test trait)
    ...
    func benchmarkGPUAdvantage() throws {
        ...
        // Large GPU problem (2000 individuals × 30 generations × 10D)
        let gpuConfig = GeneticAlgorithmConfig(
            populationSize: 2000,
            generations: 30,
            seed: 888
