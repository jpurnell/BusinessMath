...
struct IslandModelTests {
    ...
    func testResultProperties() throws {
        ...
        guard let minIslandFitness = result.islandFitnesses.min() else {
            Issue.record("Island fitnesses should not be empty after optimization")
            return
        }
    ...
    func testMigrationCount() throws {
        ...
        // Migrations occur every `interval` generations, not on the final generation
        let expectedMigrations = (gaConfig.generations - 1) / islandConfig.migrationInterval
        #expect(result.migrationCount == expectedMigrations)
    }
