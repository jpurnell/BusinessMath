...
//		print("Test result trajectory:")
		for (t, xₜ) in result.trajectory.enumerated() {
			...
			print("  Period \(t): \(weights.map { $0.number(4) }.joined(separator: ", "))")
		}
...
//		print("Turnover: \(turnover.percent())")
	}
...
//		print("Average allocation to asset 0: \(average.percent())")
	}
...
//		print("Final allocation: \(result.terminalState.toArray().map { $0.number() })")
	}
...
//			print("Q\(t+1) production: Product1=\(d[0].number(0)), Product2=\(d[1].number(0))")
		}
	}
...
//		print("Total investment: \(totalInvestment.currency(0))k over 5 periods")
//		print("Investments: \(result.trajectory.map { $0.toArray()[0].number(0) })")
	}
