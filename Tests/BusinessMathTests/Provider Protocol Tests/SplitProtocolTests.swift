...
struct SplitProtocolTests {
	...
	func metricsProviderGoldenPath() async throws {
		...
		let measured0 = try #require(metrics.priceToEarnings)
		#expect(abs(measured0 - 15.5) < 1e-10)
		let measured1 = try #require(metrics.beta)
		#expect(abs(measured1 - 1.2) < 1e-10)
		let measured2 = try #require(metrics.additionalMetrics["debtToEquity"])
		#expect(abs(measured2 - 1.87) < 1e-10)
		#expect(provider.callCount == 1)
	}
	...
	func marketMetricsCodableRoundTrip() throws {
		...
		let measured10 = try #require(decoded.additionalMetrics["roe"])
		#expect(abs(measured10 - 0.157) < 1e-10)
	}
	...
	func marketMetricsLargeValues() throws {
		...
		let measured0 = try #require(decoded.earningsPerShare)
		#expect(abs(measured0 - 73_298.0) < 1e-6)
		let measured1 = try #require(decoded.fiftyTwoWeekHigh)
		#expect(abs(measured1 - 641_435.0) < 1e-6)
	}
}
