...
struct PercentilesTests {
	...
	func percentilesOrdering() throws {
		// Generate deterministic data (ordering is mathematical invariant, no need for randomness)
		let values = (0..<100).map { i in Double(i) * 10.07 }
		let percentiles = try Percentiles(values: values)
...
struct PercentilesAdditionalTests {
	...
	@Test("Percentile argument outside [0, 1] is clamped")
	func percentileOutOfRange() throws {
		...
		#expect(p.percentile(-0.1) == p.min, "Negative percentile should be clamped to min")
		// percentile > 1 should return max
		#expect(p.percentile(1.1) == p.max, "Percentile > 1 should be clamped to max")
	}
