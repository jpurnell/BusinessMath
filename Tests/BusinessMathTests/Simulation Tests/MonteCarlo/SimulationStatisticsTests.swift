...
struct SimulationStatisticsTests {
	...
	func simulationStatisticsSingleValue() {
		...
		// Use tolerance to guard against future summation algorithm changes
		#expect(abs(stats.mean - 42.0) < 1e-10, "Mean should be exactly 42")
		#expect(abs(stats.median - 42.0) < 1e-10, "Median should be exactly 42")
		#expect(abs(stats.min - 42.0) < 1e-10, "Min should be exactly 42")
		#expect(abs(stats.max - 42.0) < 1e-10, "Max should be exactly 42")
		#expect(abs(stats.stdDev - 0.0) < 1e-10, "StdDev should be 0 for single value")
	...
	func simulationStatisticsAllSameValues() {
		...
		// Use tolerance to guard against future summation algorithm changes
		#expect(abs(stats.mean - 100.0) < 1e-10, "Mean should be exactly 100")
		#expect(abs(stats.median - 100.0) < 1e-10, "Median should be exactly 100")
		#expect(abs(stats.min - 100.0) < 1e-10, "Min should be exactly 100")
		#expect(abs(stats.max - 100.0) < 1e-10, "Max should be exactly 100")
		#expect(abs(stats.stdDev - 0.0) < 1e-10, "StdDev should be 0 for constant values")
		#expect(abs(stats.variance - 0.0) < 1e-10, "Variance should be 0 for constant values")
		#expect(stats.skewness.isNaN, "Skewness is undefined (NaN) when variance is 0")
	}
	...
	func simulationStatisticsNegativeValues() {
		...
		#expect(abs(stats.min - (-50.0)) < 1e-10, "Min should be exactly -50")
		#expect(abs(stats.max - 100.0) < 1e-10, "Max should be exactly 100")
		#expect(abs(stats.median - 25.0) < 1e-10, "Median should be 25")

		// Mean = sum / count = 175 / 7 = 25
		#expect(abs(stats.mean - 25.0) < 1e-10, "Mean should be 25")
	}
