...
`TimeSeries.fitETS()` lets a Holt-Winters model search for its own alpha, beta and gamma;
`ETSSeasonality` names the three cases Excel encodes in one numeric argument and closes
`FORECAST.ETS.SEASONALITY` over detection that already existed; `smape` joins the error
metrics with the halved denominator **measured** in Excel rather than chosen. `Complex` gains
`notation` and `init?(notation:)` as members rather than a conformance, so `description` is
untouched.

The fix is worth naming because of its shape. A Metal read-back appended only on a successful
vector conversion, so a failure returned a batch shorter than the population — which the
caller then indexed by population size. An out-of-range crash rather than a wrong answer,
unreachable through the public API today and one conformance away from reachable. Batch
conversion now returns every element or `nil`, never a prefix.

**Two branches are parked, both breaking, and together they are 3.0.0.**
`feature/stage-6-template-delegation` carries the `SaaSModel`/`SubscriptionBoxModel` LTV
delegation plus `CLVDefinition.perpetuityDue` and a parametric `customerLifetimeValue`.
`feature/gpu-seeded-acceleration` makes `optimizeDetailed` throw on DE and PSO, which is what
lets GPU and a seed work together again. The remaining breaking item — deleting the
`sampleSize` deprecated in 2.7.0 — is not started. Next step is a review of the two branches
against feature-completeness, then **v3.0.0a**.
