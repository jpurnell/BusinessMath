...
| # | Defect | Status | Notes |
|---|---|---|---|
| L1 | **`bayes` divides by an unguarded denominator.** `pT = s·p + f·(1−p)`; at `(0, s, 0)` and `(1, 0, f)` it is exactly zero and the result is NaN by accident rather than by contract. | ✅ **FIXED** — B4 of the alpha.5 blockers: contract stated, pinned, paired with `bayesChecked` | `Sources/BusinessMath/Bayes/Bayes.swift:30`. No validation of any of the three probabilities either. Same open-contract shape the statistics review decided for the free-function/checked pair. |
| L2 | **`bayes` is `Double`-only.** | ✅ **FIXED** — generic over `T: Real`, landed with L1 | Not generic over `T: Real`, against the project's own rule in CLAUDE.md ("Use `<T: Real>` generics for numeric functions"). The review asked whether it was generic; it is not. Fixing L1 is the moment to fix this. |
| L3 | **`runFinancialSimulation` takes no `seed:`.** | ✅ **FIXED** — A1, additively; the randomness was in the caller's builder | `FinancialSimulation.swift:498`. ~15 tests draw fresh values per run as a result. This is the coverage ceiling for the whole scenario suite, and the same defect the simulation review recorded for `ScenarioAnalysis` and `SensitivityAnalysis` (its Phase 3, breaking). |
...
| L5 | ~~**`ConstrainedDriver` clamping distorts the distribution, unmeasured.**~~ | **RESOLVED 2026-09-14 — question answered, the review's number refuted** | **It censors.** `sample` draws once and applies `constraint(rawValue)`, a pure function — no rejection loop, so an out-of-range draw is moved to the boundary rather than redrawn. Measured over 200,000 seeded draws of `Normal(1000,100)` clamped to ±1σ: **sd 71.86** (closed form 71.8372), **boundary mass 0.3166** (closed form 0.3173105), min and max sitting exactly on the bounds. **The review's boundary mass is exact; its sd of 60.6 matches neither convention** — censoring gives 71.84, truncating gives 53.96 — so this row's old summary, "arithmetic is right", was right about the half that identifies the behaviour and wrong about the half a modeller would quote. Both closed forms were confirmed against a two-million-draw simulation before the implementation was measured. **Censoring is the correct choice and is now documented as such:** rejection sampling consumes an unpredictable number of draws, and `SumDriver`/`ProductDriver` interleave operands on one generator, so a driver taking a variable number of draws makes the aggregate stream irreproducible. `ClampingDistributionTests` pins sd, boundary mass and the symmetric-clamp mean. |
...
| L19 | ~~**A negative lower bound hidden in an opaque constraint closure is silently truncated to zero.**~~ | ✅ **FIXED 2026-09-14** | Two changes, because two places refused a closure. **`extractVariableShift` now recovers the bound by evaluating rather than inspecting** — constant from the origin, each coefficient from a unit step, and affinity confirmed at three witness points away from both, which is exactly the probe `validateLinearModel` already uses. A closure is opaque to inspection, not to evaluation. **`transformConstraint` now shifts a closure by composition** (`g(y + shift)`) instead of throwing "Variable shifting only supports linear constraints" — it had been refusing the very model whose bound the extractor had just recovered. The probe declines anything it cannot verify: non-affine, non-finite anywhere probed, more than one variable, or a positive coefficient (which bounds from above). The `withKnownIssue` tripwire fired on the fix — *"Known issue was not recorded"* — and has been replaced with a direct assertion. |
| L15 | **Three days-per-year conventions in one package.** | ✅ **FIXED** — A4, routed through `DayCountConvention` | `FinancialRatios` uses 365; `TimeSeriesAnalytics:181` and `BondPricing` use **365.25** ("to account for leap years over long periods"); `DayCountConvention` implements the ISDA set properly. None is wrong alone, and a CAGR over decades has a real case for 365.25. But the choice is undecided and undocumented, and it sits directly next to L11. Decide once, document, and route through `DayCountConvention` where a convention is genuinely at stake. |
| L13 | **`sharpeRatio` returns 0 when risk is 0.** | ✅ **FIXED** — the zero-risk guard is gone; IEEE answers | `Portfolio.swift:182` is `guard risk > T(0) else { return T(0) }`. The review could not tell whether the implementation guarded or the test failed; it guards. But **0 is a plausible-but-wrong answer**: a Sharpe of 0 means "no excess return per unit of risk", and the fixture earns 7.5%/yr of excess return at *exactly zero risk* — infinitely good, reported as mediocre. The guard inverts the best possible case. Same family as `a * 0 → 0` and the antithetic SE: a guard returning a plausible number where the answer is undefined. **Decide +∞, NaN or throw, then pin it.** Consequence meanwhile: `sharpeFinite` asserts `.isFinite` on a constant 0, and `optimizerBeatsEqualWeights` reduces to `0 >= 0 - 1e-6`. |
| L14 | **A shared test helper ships a generator with two contract bugs.** | ✅ **FIXED** — A2, the helper is deleted | `Tests/.../Stochastic/StochasticTestHelpers.swift:13` is `Double(state) / Double(UInt64.max)` — **closed** on both ends, returning exactly 1.0 at `UInt64.max` and exactly 0.0 at 0, while its own doc says "(0, 1)". Its `nextNormal` guards `max(u1, 1e-15)`, the shape `BoxMullerPoleGuardTests` calls wrong, and does not guard u₁ = 1.0 where `log(1) = 0` collapses the draw. It duplicates `MMIXSeededRNG`, already in TestSupport. **This is the Nth inline Box-Muller in the corpus and the first found in a *shared* helper, so it seeds several files at once.** Delete it; use `DeterministicRNG` and the TestSupport transform. |
...
| L11 | **DSO / DIO / DPO are wrong on any non-annual statement.** | ✅ **FIXED** — B1 of the alpha.5 blockers, `4b666601` | `daysInventoryOutstanding` is `365 / inventoryTurnover`, and `inventoryTurnover` is `COGS / averageInventory` over the **period**, un-annualised. On the quarterly documentation fixture, measured: turnover 4.0 turns *per quarter*, DIO **91.25** days where the answer is 91/4 = **22.75** — **4.01× too large**. DSO returns 73.0 where it should be 18.2. The financial-ratio review presents "90-day quarter or 365-day year" as an open convention; it is neither. It is a dimensional error: an annual day count divided by a quarterly turn rate. **And 91.25 is within a quarter-day of the 91 days in the quarter, so a reader sanity-checking "is DIO about one quarter?" sees agreement.** The fix is `DayCountConvention.days(in: period)`, which already exists and returned the correct 91.0 in the same probe. |
| L8 | ~~**`MonthDay` accepts February 30.**~~ | **FIXED 2026-09-14 — confirmed and broader than filed** | The two guards checked `month ∈ 1…12` and `day ∈ 1…31` **independently**, so every impossible date in the 12×31 box constructed: February 30, April 31 and June 31 all measured as accepted. **Nothing downstream caught it** — a February-30 year-end produced a *plausible* fiscal year (15 March 2025 → fiscal 2026), identical to February-28's, because `FiscalCalendar` orders on (month, day) rather than resolving a date, so the caller had no way to learn their year-end does not exist. Now validated against a per-month table. **The row's open sub-question is answered: a February-29 year-end is well-defined**, precisely because the comparison is an ordering — measured, 15 March 2024 → fiscal 2025 and 15 March 2025 → fiscal 2026, the same answers February 28 gives — so February keeps 29 and a leap-day year-end stays expressible. Also removed four stray `try MonthDay(…)` in doc examples and one in a test: the initialiser's contract is a *precondition* and it does not throw, which was producing a compiler warning. |
| L9 | ~~**`npvExcel` discounts the first element by one period.**~~ | **RESOLVED 2026-09-14 — design correct, documentation already complete, gap was executability** | The implementation is `flow / (1+r)^(index+1)`, correct Excel semantics. **The remediation this row prescribed — "documentation, not a code change" — was already done:** the doc comment carries a comparison table, a when-to-use for each convention, a worked Method 1 versus Method 2 example, and an explicit "**do not** include the initial investment in the cashFlows array". **And it was still got wrong twice in this repository**, both found the day before: `TVMReferenceTests`' header and `npvExcelDocumentationExample`. Prose that thorough failing twice is the argument for making the distinction executable, which nothing did. Two tests now state it, with a sharper identity than the prose had: putting the outlay inside the array does not shift one term, it divides the **entire NPV** by (1+r) — 1307.29 becomes 1188.44, a 9% understatement, with nothing in the result to say so. |
| L10 | ~~**Mixed period types unsupported, recorded only in a commented-out test.**~~ | **REFUTED 2026-09-14** | The comment claimed the capability "currently trigger[s] a Strideable issue when Swift's stdlib tries to optimize operations". **`Period` deliberately does not conform to `Strideable` and cannot** — `PeriodArithmetic` says so where `PeriodRange` is defined: *"Since `distance(to:)` throws and we can't conform to Strideable with a throwing method, we provide a custom sequence."* There is no conformance for the standard library to optimise through, so the described failure has no mechanism. Meanwhile a **live** test in `TimeSeriesBuilderTests` had been exercising mixed period types through the builder throughout. The commented-out test is restored and passes, with its `Date()` replaced by a fixed instant — a series keyed on "now" is a different series every run. **Fifth refuted claim in the programme.** |
...
| ~~**B2**~~ | ~~The **3 stale integer-programming assertions**~~ | ✅ **DONE — and the third one is the finding the done-when anticipated.** All three capabilities did ship, all three as **opt-in flags defaulting to `false`**, which is why the tests could not simply be uncommented. `detectsQuadraticObjective` and `detectsBilinearConstraint` now build a second solver with `validateLinearity: true` and assert `.nonlinearModel` **by case, not by type** (G2's rule), while also pinning the documented default — that the unchecked path accepts the nonlinear model rather than diverging. **`handlesNegativeLowerBounds` found L19.** Measured, `minimize x` s.t. `x ≥ -3`, `x ≤ 5`: default solver → **0**; `enableVariableShifting: true` with the bound as an opaque `.inequality` closure → **0**; same flag with the bound as `.linearInequality` → **-3, correct**. `extractVariableShift` reads the constraint list for detectable bounds, and an opaque closure states nothing — so no shift is applied and the simplex's implicit `x ≥ 0` truncates the answer to 0, **silently**, which violates the package's own fail-silent principle. Recorded as `withKnownIssue` so that fixing shift detection breaks the build, the pattern that worked for L17. |
| ~~**B3**~~ | ~~**`Phase1_CutValidityTests`** — flip 16 of 17 to `minimize: false`~~ | ✅ **DONE, and the diagnosis needed correcting.** Measured first: **14 of 16 tests never generated a single cut.** Flipping to maximisation was necessary and **nowhere near sufficient** — flipped, still only 1 of 15 fired. The real cause is geometric, not directional. A Gomory cut is built from the **fractional parts** of a tableau row's non-basic coefficients, and the fixtures were single constraints with all-1 coefficients (`x + y ≤ 5.5`), whose optimal basis gives `x = 5.5 - y - s` with both non-basic coefficients **1** and both fractional parts **0**. The cut degenerates to `0 ≥ 0.5` and is correctly rejected as weak. **No direction, tolerance or round count could ever have made those fixtures cut.** Fixtures are now two constraints whose coefficients do not divide evenly (`2x+3y ≤ 11, 4x+y ≤ 10`; a richer `5x+4y ≤ 22, 3x+7y ≤ 25`), which pivots fractional entries into the optimal tableau. Every test now asserts its cut count explicitly — `> 0` where cuts should fire, **`== 0` where the point is that they should not** (`nonViolatingCutsRejected` is exactly the degenerate-row case and now says so). Integer optima are pinned rather than bounded. **`cutsReduceTreeSize` finally compares tree sizes** — it never did, and could not have: the fixture produced no cuts *and* every cut was infeasible until L18. Measured: **17 nodes without cuts, 5 with them.** |
| ~~**B4**~~ | ~~**`PerformanceOptimizationTests`** — ~17 wall-clock assertions in the regular suite~~ | ✅ **DONE.** 16 timing assertions found, tightest 50 ms. One test (`Performance_MemoryEfficiency`) asserted **only** elapsed time and was gated at test level; the file's own `Benchmark_TimeSeriesCreationThroughput` was already gated. The other **14 each carry a correctness assertion**, so per `benchmarkOnly`'s own guidance — it is for tests with "no correctness statement underneath that survives removing the timer" — the suite was split rather than moved: each timed test keeps its correctness assertions in the regular suite and gains a `Benchmark_`-prefixed twin in a new `PerformanceOptimizationBenchmarks` suite marked `.benchmarkOnly`. All 16 timers now run only under `RUN_BENCHMARKS`; verified both ways, and the same filter takes 0.03s gated against 0.54s enabled. |
...
| ~~**C4**~~ | ~~**Fix the checker's nested-scope bug**~~ | ✅ **DONE — and it needed no change to the tool.** Reproduced against the shipping binary: removing the marker from `testMonteCarloIntegration`, a test with two local `func` declarations followed by four real `#expect`s, produces **no** `missing-assertion` diagnostic. The auditor already handles it, and says so in its own source — *"A helper declared inside the test body. Its `return` is its own."* — while `currentTestHasAssertion` is set by any `#expect`/`#require` anywhere in the subtree, so nesting depth never suppressed it. **All 18 `// checker workaround for nested struct scope` markers removed; the checker reports 0 errors without them.** If the bug was ever real it was fixed, and the workarounds outlived it — which is the lesson: a workaround with no expiry outlives its cause. |
...
| | Item | Done when |
|---|---|---|
| ~~**D1**~~ | ~~**53 of the 75 `#expect(true)`** → `#expect(throws: Never.self)`~~ | 🟡 **PARTIAL — 45 of 72 resolved, 53 markers remain in the corpus.** 27 converted (10 interpolator equivalence tests, 2 error-handling boundary tests, 1 Holt-Winters training test, 14 attribution sites); **18 deleted outright as stale C4 workarounds — see C4.** What remains and why: `LoggerTests` 13 need a recording sink and were always the documented remainder; `MatrixBackendBenchmarks` 9 and `MonteCarloGPUPerformanceTests` 5 sit in suites already gated `.benchmarkOnly` and never run normally; the rest wrap their throwing call in `try?`, which swallows the error and makes a no-throw claim meaningless — they want a different assertion, not this one. |
| ~~**D2**~~ | ~~**225 `?? 0`** → `try #require`~~ | ✅ **DONE — 211 of 211.** The roadmap said 225; measured, it was 211 across 24 files. Swept file by file with a run between each, not by one regex: the shapes vary (`abs((x ?? 0) - v) < t` dominates, but there are bare comparisons, `?? .infinity` used as a deliberate poison value, `?? 0` on the *right* of a subtraction, and `?? []` inside string interpolation where the coalescing formats a failure message and is harmless). **A compile constraint that shapes the whole item:** `try #require` **cannot be inlined into `#expect`** — `#expect(abs(try #require(x) - v) < t)` fails with "errors thrown from here are not handled", because the macro expands its condition into a non-throwing closure. Every site therefore needs a real binding *and* its enclosing function marked `throws`. |
| ~~**D3**~~ | ~~**~152 ambient-time sites** → the shared fixed calendar~~ | ✅ **DONE for `Calendar.current`: 65 of 91 swept, 26 deliberately kept.** Measured 91 sites, not 152. Swept to `gregorianUTC`. **The 26 kept are the point of the carve-out:** `ZoneInvariance.swift`, `CalendarZoneInvarianceTests`, `ThirtyThreeSixtyVariantTests`, `CouponPeriodCalendarTests` and `BondClockZoneInvarianceTests` use `Calendar.current` *deliberately*, to prove behaviour is invariant under it — sweeping those would delete the tests that caught the original defect. The remainder are comment-only mentions. **The ~210 `Date()` population is not swept and should not be until C2's bracketing carve-out exists** — two `Date()` readings compared to each other assert ordering and are correct; `WallClockAdoptionTests` has nine. |
| ~~**D4**~~ | ~~Ratio fixture deduplication and its 20 `guard let` → `try #require`~~ | 🟡 **The `guard let` half is DONE; the fixture deduplication is not.** All 20 blocks of `guard let x = … else { Issue.record("…"); return }` in `RatioConvenienceFunctionsTests` became one-line `try #require(…, "…")` carrying the same message. **941 → 881 lines**, the ~60 the item predicted. The duplicated ~115-line fixture itself remains. |
...
| | Item | Done when |
|---|---|---|
| ~~**E1**~~ | ~~**The gradient certificate**~~ for L-BFGS, gradient descent, Newton-Raphson, multi-start. Replaces ~40 distance-to-known-minimum assertions with `‖∇f(x*)‖ < tol` — the condition that *defines* a minimum, and one that extends to problems with no known answer. | Every smooth problem asserts the gradient norm against the optimizer's own tolerance. `gradientNorm < 0.1` already exists in two files at the wrong bound — promote it. | ✅ **DONE.** 25 certificates across L-BFGS, gradient descent and Newton-Raphson, each asserting `‖∇f(x*)‖` against **that test's own optimizer's declared `tolerance`** rather than a fitted constant — the optimizer documents that it stops when the gradient norm falls below it, so this checks the contract it advertises. Measured norms span 4.4e-17 (Newton-Raphson on a quadratic) to 9.9e-4 (Adam on Rosenbrock); all 25 hold. The distance-to-minimum assertions were kept alongside rather than replaced: they carry the known answer, the certificate carries the definition. |
| ~~**E2**~~ | ~~**The two degenerate portfolio fixtures.**~~ Three tests are vacuous *because of the fixture*; no assertion change rescues them. | Nonzero, unequal variances and a known covariance, so the Sharpe ratio and the frontier both exist. | ✅ **DONE — and the fixture was worse than 'degenerate'.** `makeTwoAssetReturns` was `Array(repeating:)` for both assets, so **both variances were exactly zero**: the covariance matrix was all zeros, the Sharpe ratio divided by zero, and the frontier had no frontier. The replacement is analytic rather than sampled, so moments are exact: `A = 0.10/12 + 0.02·sᵢ`, `B = 0.05/12 + 0.01·tᵢ`, with `t` flipping sign at index 42 — **not 45**, because 42 turns exactly 9 even and 9 odd indices and keeps `t` balanced. Flipping at 45 gives a tidier 0.5 correlation and moves B's mean to 0.0045 against an intended 0.0041667; measured, before the flip point was corrected. Result: means exact, variances 60/59·(4e-4, 1e-4), **correlation exactly 0.4**. The three vacuous tests now assert real things — the Sharpe *identity* plus a pinned nonzero risk (it was `sharpe >= 0.0 || sharpe < 0.0`, a literal tautology), frontier monotonicity in both return and risk, and a genuinely mixed optimum. **Separate finding:** with `Portfolio`'s default 3% *annual* risk-free rate against *monthly* returns every excess return is negative, and maximising a negative Sharpe ratio inverts — it seeks the riskiest asset, returning 100% of one holding. Documented at the call site. |
| ~~**E3**~~ | ~~**DEA units invariance**~~ for SBM and super-efficiency. CCR and BCC have it. | All four models covered. Cheapest high-value addition in that domain: a scaling error breaks it while leaving every score plausible. | ✅ **DONE — six configurations, and the first run was a false alarm worth recording.** The test covered `.ccr` alone, the one model whose invariance is easiest to believe without checking. Now parameterised over ccr, bcc, superEfficiency(ccr), superEfficiency(bcc), sbm(crs) and sbm(vrs). `superEfficiency(bcc)` failed with 13 violations reading **`inf became inf`** — not a units defect at all. Andersen-Petersen removes the evaluated unit from its own reference set, and under variable returns to scale that can leave no frontier to project onto, so `+∞` is the correct score and it is the only configuration theory predicts will reach it. `abs(inf - inf)` is NaN and fails every bound, so a closeness-only comparison reports a violation exactly where the property holds perfectly. The comparison is now equality-**or**-closeness, which accepts `inf == inf` and still fails on NaN. |
| ~~**E4**~~ | ~~**ETS exact-recovery oracle.**~~ Not the phase defect — ETS delegates to `HoltWintersModel` and inherits that fix — but the parameter *search* is untested. | A noiseless series in the model's span is recovered exactly by the fitted parameters. | ✅ **DONE.** Every other test in the file compares the fit to a baseline — a grid, the defaults, seasonal-naive — which a search broken in the same direction as its baseline still passes. This asks the question with a known answer: a noiseless `level + trend·t + seasonal[t mod m]` series lies in additive Holt-Winters' span, so the fit must recover it. **Recovery is asymptotic, not immediate** — the state is seeded from the first cycles — so the oracle is geometric decay of the residuals, not zero from the start. Measured, maximum |residual| per block of eight: **4.698, 0.979, 0.173, 0.0204, 0.00279** — roughly a factor of six per block over five blocks. Asserted as strict monotone decay, a final block under 0.01, and a first-to-last ratio above 100, none of which requires knowing the parameters the search lands on (a deterministic series does not have unique ones). A non-seasonal twin isolates the trend mechanism. |
| ~~**E5**~~ | ~~**Interval calibration**~~ for `EmpiricalIntervalsTests`. Same shape as the antithetic-SE defect: a reported uncertainty nothing measures against realised spread. | An 80% interval contains the truth about 80% of the time over a seeded synthetic series, bounded by the binomial standard error. | ✅ **DONE.** The four existing tests check the *shape* of an interval — symmetric, widens with horizon, wider at higher confidence — and **every one passes for an interval of the wrong size**. A band twice too wide satisfies all three. Calibration is the claim the number 0.80 actually makes. The new test builds the interval from a seeded sample of 2,000 residuals and measures coverage against **4,000 fresh draws from the same distribution**, at nominal 0.80, 0.90 and 0.95. The bound is the binomial standard error `sqrt(p(1-p)/n)` at three sigma — about 0.019 at n=4,000, p=0.8 — rather than a tolerance chosen by eye, which would either admit a miscalibrated interval or flake on a correct one. All three levels pass. |
| ~~**E6**~~ | ~~**Driver composition identities.**~~ The seeding contract already makes them cheap. | `identical(composite.sample(seed:), a.sample(seed:) op b.sample(seed:))` — catches a composite that re-draws its operands, which no band assertion can. | ✅ **DONE.** `compositesReproduce` already asserted that the same seed gives the same values — and **a composite that re-draws its operands passes it**, because re-drawing from a seeded generator is still deterministic: the stream repeats perfectly while being the wrong stream. Reproducibility compares a composite against itself, the one comparison that cannot catch this. The identity compares it against its parts: drawing `SumDriver` from one generator must consume exactly what drawing `lhs` then `rhs` consumes from an identically-seeded one, and equal their sum **bit-for-bit**. Checked at 16 successive draw positions rather than only the first, because a composite that re-seeds per call can coincide once and diverge after. Same for `ProductDriver`. |

#### Phase F — exactness

| | Item | Done when |
|---|---|---|
| ~~**F1**~~ | ~~**The operations/inventory batch.**~~ | ✅ **DONE — and the batch filed as "no defects" had one.** All 22 exact values from the review's table transcribed and **every one confirmed** by running them. The EOQ optimality identity now holds at **1e-12 relative** rather than ±1.0 — it is the first-order condition itself, so ordering cost *equals* holding cost at Q\* by construction (both 187.34993995195194), and the old window admitted a Q\* off by 1.3%. Stockout probability at the mean and NORMSINV(0.5) asserted as exact rather than ±0.05 and ±0.01. `optimalityProperty` widened from a ±1 bracket to a grid at ±1, 2, 5, 10. Error assertions matched **by case**, not by type. Finding 5 (`==`/`!=` on Doubles) → `identical`/`!identical` with a finiteness guard; finding 6 (inline Box-Muller) → the library's `boxMullerSeed`. **The defect: `extremeInputs` was named for overflow safety and used inputs where `2SD = 2e18`, comfortably inside Double's range — it was never at risk.** The review reasoned to the right test (`D = S = 1e200`) and filed it as a suggestion; writing it showed `EOQModel` formed `2·S·D` directly, overflowed to `+infinity`, and returned a **non-finite order quantity with no error**. Fixed by factoring the root, `√(2SD/H) = √(2S/H)·√D`; the 1e-12 fixtures proved the reassociation left ordinary answers bit-identical. Finding 3 also resolved: `reorderPointGoldenPath`'s comment derived `r = 91.76` from a σ_d of 5 while the fixture had σ_d = 0 — and **91.76 corresponds to no fixture in the file**; the constant case now asserts 70.0/0.0/10.0 exactly and the variable case is pinned at 78.85254006299331. |
| ~~**F2**~~ | ~~**The five comment-arithmetic errors**~~ | ✅ **DONE — all five, each verified independently before anything was changed.** (1) `TVMReferenceTests`' header cited `NPV(10%, 3000, 4200, 6800) - 10000 = 1188.44`; that expression is **1307.28775356875**, because it subtracts the outlay *outside* the call and therefore undiscounted. Excel's published example puts the outlay inside, discounted from t = 1. (2) `NPVTests.npvExcelDocumentationExample` — **the most instructive of the five.** Its comment's middle term read `7888.89` where `9200/1.08²` is `7887.52`. The three *rounded* figures summed self-consistently to the stated 23234.62, so the arithmetic looked checked, and the **1.373 error propagated into the expected value**: 13234.62 asserted against a true 13233.2469643855. **A ±2.0 tolerance held a wrong expected value, so the test passed while asserting the wrong number.** (3) `ClassifierEvaluationTests`' tie conventions: counted directly on the fixture (4 positives, 4 negatives, 16 pairs, 12 strict wins, 4 ties), the alternatives are **0.75** and **1.0** — the cited 0.8125 and 0.9375 are 14/16 ∓ 1/16 and correspond to no convention. (4) The H-model, filed by its review as *"the one candidate live defect in the batch"* at 27% apart — **it is not a defect.** `HModel` documents its own parameter: *"The full transition period is 2H years"*, so `halfLife: 10` means a twenty-year decline and using 10 directly as H is correct. The test's prose said "10 years", which made the code look wrong. Value pinned at 61.33333333333333; had H been 5 it would be exactly 48.0, and the old ±0.5 band could not tell the readings apart. **Sixth instance of "the code is right and the comment is wrong".** |
| ~~**F3**~~ | ~~**T2's remainder**, ~150 band assertions with verified reference values already supplied~~ | ✅ **DONE for every value the reviews supply — 35 across three tables, and all 35 confirmed.** Financial ratio §3 (19), time series Appendix A (14), Bayes §3 (2). **That every one held is itself the finding:** each of the other batches in this programme carried a materially wrong claim, and these three were right about every value — they are computed tables rather than recalled ones. The bands they replace were wide in a way easy to under-rate: `#expect(result > 0.95)` on a posterior of 36/37 passes for 0.951 and 0.999 alike; `0.01 < r < 0.02` spans a **factor of two**; `abs(irr - 0.143) < 0.01` admits a 7% error in a rate. None was vacuous — all would catch a grossly wrong answer — but none could distinguish a correct implementation from a subtly wrong one, which is the failure that ships. Three sub-cases handled as §T2 prescribes: **exact in binary** → `isEqual(to:)` (`workingCapital` 850,000, `marketCap` 50,000,000, `priceToSales` 50.0, `evToSales` 50.5, `inventoryTurnover` 2.0, `enterpriseValue` 50,500,000, `interestCoverage` 10.0, `grossMargin` 0.6); **exact rational** → ~1e-15 relative (currentRatio 20/3, quickRatio 16/3, cashRatio 10/3, assetTurnover 1/3, receivablesTurnover 10/3, Bayes 36/37); **everything else** → 1e-9 to 1e-12. Also added the partition identity `debtToAssets + equityRatio == 1`, which no single value can check. **F3 independently vindicated F2:** the time-series appendix lists `npvExcel(0.08, [8000, 9200, 10000])` = 23233.246964385507, the value F2 derived from first principles by a different route. **Remaining:** the T2 sites *without* supplied values, which need derivation rather than transcription — a different kind of work than this item described. |

#### Phase G — hygiene ✅ COMPLETE

| | Item | Done when |
|---|---|---|
| ~~**G1**~~ | ~~10 `.disabled` and 30 commented-out `@Test`~~ | ✅ **DONE `9d2ca2fa`. Both populations are empty; 7,757 tests to 7,815.** The count was 29 commented-out, not 30, and 17 of those were one file — `AdvancedMathTests.swift`, entire struct commented out, 1,240 lines that had never compiled. Nine of its seventeen call `VectorN.multiply(by:)`, which has never existed on any vector type here; the other eight exercise members already covered in `VectorSpaceTests`. Deleted. **Four of the ten `.disabled` traits carried no reason at all** — the argument was the display name the `@Test` never got, and all four pass as written. A fifth blamed "random variation" in a test whose every sample is the fractional part of *i·φ*, identical on every run. The two GPU traits blamed a "Metal initialization quirk on initial runs": that was `4470f7e`'s defect, a `try?` that swallowed a GPU failure and fell through to the CPU mid-run, and both pass on a cold device with the warm-up simulations deleted. The node-limit trait said "test problem too easy", which was the symptom — its knapsack was written `.inequality { 5 - (2x₀ + …) }` and that case is documented as feasible where **g(x) ≤ 0**, so the closure said weight *at least* 5, which all-ones satisfies. Rebuilt on `cutRich`, 17 nodes against a budget of 5. Two stubs asserting a test-local `ValidationError` from two non-`throws` initialisers were unsatisfiable by construction; the requirement is now a `withKnownIssue` that trips when validation lands (churn 1.2 on 100 customers ends month one at **−20 customers**). `throttle(interval:)` was public with zero test call sites — a real gap, now covered; its wall-clock half took `// TIMING:` + `.benchmarkOnly`, as did the disabled debounce test. **Re-enabling `deterministicProjection` exposed a flaky assertion the gate caught on the first full run** — ±20% bands on single unseeded draws from a 10%-uncertainty driver, two-sigma each, failing about one run in eleven. |
| ~~**G2**~~ | ~~**530 type-only error assertions** against 99 that pin a value~~ | ✅ **DONE across three commits — `12997d28` (the 54 `(any Error).self`), `741f786b` (90 by hand), `077562dd` (the remaining 390).** The count was **524**, not 530 — measured at `651c81e9`, counting only sites that actually assert nothing but a type: 54 `(any Error).self` and 470 typed. A further 24 are `throws: Never.self`, which assert that a call does *not* throw and are correct as written, and were left alone. **Method matters here.** Reading call sites back to their guards was the obvious approach and the wrong one — the first two batches produced seven cases where the source was read correctly and the string still came out wrong, because the guard that fires is not always the one the test is named for. Every remaining site was rewritten to `throws: Never.self` and the suite run once; Swift Testing then prints the error each site actually throws, with file and line, for all of them at once. The tree was restored and the assertions generated from that transcript, so **every expected value is a recording rather than a reconstruction**. Three shapes, measured across the three commits: **381** value pins where the type is `Equatable`, comparing the whole error with `==`; **126** case-and-payload matches in an `#expect { } throws: { }` closure where it is not; and twelve sites whose message embeds a *computed* float — the probe point in `OptimizationError.nonlinearModel`, the measured entropy in `BacktestError.unforecastableSeries` — pin the stable fields and bound the rest. **Found by the sweep:** `inventoryTurnover`'s "missing inventory account" test is refused for a missing *expense* (COGS, checked first) and has been named for the wrong account all along; `PeriodTypeTests` held the corpus's weakest assertion, `throws: Error.self`, which did not even pin the domain; the two custom-period decoding refusals share a case *and* a coding path and differ only in debug description; `EvaluationError` distinguishes six conditions that ten assertions accepted interchangeably. **The ledger, measured rather than recalled:** 524 → 522 (G1 incidentally removed two) → 470 → 404 → 14. The commit messages overstate two of those steps — `741f786b` says "ninety" where 66 were newly weak (the rest were five files redone from a looser helper form into the house one), and `077562dd` says 350 where the measured figure is 390. The numbers here are the ones to trust. Fourteen sites keep the type-only form because the type is the claim — `CancellationError` and a test-local `BuilderFault` carry no payload, and ten already capture and `guard case` below. |
