...
| L19 | **A negative lower bound hidden in an opaque constraint closure is silently truncated to zero.** | **OPEN — recorded as `withKnownIssue` in `handlesNegativeLowerBounds`** | Found by restoring B2's third stale assertion. `enableVariableShifting` works: `minimize x` s.t. `x ≥ -3`, `x ≤ 5` returns **-3** when the bound is a `.linearInequality`. It returns **0** when the identical bound is an `.inequality` carrying a closure, because `extractVariableShift` reads the constraint list for bounds it can recognise and a closure states nothing. No shift is applied, the simplex's implicit `x ≥ 0` truncates, and the caller gets a plausible-but-wrong optimum with no error — the exact shape the package's fail-silent principle forbids. **Two defensible fixes:** detect the bound numerically by probing the closure (the same technique `validateLinearModel` already uses), or refuse to solve when shifting is requested and a constraint is opaque. The second is cheap and honest; the first is better. Not attempted — it is a design decision, not a patch. |
...
| | Item | Done when |
|---|---|---|
| **D1** | **53 of the 75 `#expect(true)`** → `#expect(throws: Never.self)`. | 🟡 **PARTIAL — 27 of 52 done.** Converted: the 10 uniform interpolator equivalence tests (the construction is wrapped, since that is the claim the marker stood next to), 2 error-handling boundary tests, 1 Holt-Winters training test, and 14 attribution sites. **Remaining, and each for a stated reason:** `LoggerTests` 13 need a recording sink and were always the documented remainder; `MatrixBackendBenchmarks` 9 and `MonteCarloGPUPerformanceTests` 5 sit in suites already gated `.benchmarkOnly`, so they never run in a normal build; and a handful of singletons where the throwing call is wrapped in `try?`, which swallows the error and makes a no-throw claim meaningless — those need a different assertion, not this one. **Separately, 18 `#expect(true)` carry `// checker workaround for nested struct scope` and are C4's material, not D1's** — they come out when the checker is fixed. |
| **D2** | **225 `?? 0`** → `try #require`. | 🟡 **PARTIAL — 158 of 211 done, 53 remain.** Measured 211 sites in 24 files rather than 225. Swept the seven densest files: `TimeSeriesAnalyticsTests` 33, `TimeSeriesOperationsTests` 34, `ScenarioBuilderTests` 31, `TimeSeriesTests` 26, `EMSTableGeneratorTests` 16, `SplitProtocolTests` 16, `HeuristicAttributionTests` 14. **Not done as one regex, deliberately:** the shapes vary (`abs((x ?? 0) - v) < t`, bare comparisons, `?? .infinity`, `?? []` inside interpolation where it is harmless), and a sweeping substitution across 24 files unattended risks silent damage. Each file was transformed and run before the next. **Note `try #require` cannot be inlined into `#expect`** — `#expect(abs(try #require(x) - v) < t)` fails to compile with "errors thrown from here are not handled", so each site needs a real binding and its enclosing function needs `throws`. |
...
| **D4** | Ratio fixture deduplication and its 20 `guard let` → `try #require`. | 🟡 **PARTIAL — the `guard let` half is done.** All 20 `guard let x = … else { Issue.record(…); return }` blocks in `RatioConvenienceFunctionsTests` became one-line `try #require(…, "message")`, carrying the same message. **941 → 881 lines**, which is the ~60 the item predicted. The fixture deduplication itself (~115 lines × 2) is not done. |
